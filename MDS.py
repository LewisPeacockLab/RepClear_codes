...
def run_MDS(
    ...
    if data_source == "stim":
        ...
        trial_start_TRs = stim_on_TRs[
            ::2
        ]  # starting TRs of each trial. used for getting trial number & image_id & category

        # subsample & average bold
        avg_trial_bolds = full_data[stim_on_TRs]
        avg_trial_bolds = np.array(
            [
                avg_trial_bolds[i : i + 2].mean(axis=0)
                for i in range(0, len(avg_trial_bolds), 2)
            ]
        )
        ...
        out_fname = os.path.join(
            results_dir, "MDS", f"sub-{subID}_{task}_bold_vtc_mdss"
        )
...
def item_level_RSA(
    subID="002", phase="pre", weight_source="LSA", stats_test="tmap", dim_red="MDS"
):
    """
    ...
    mask_path = os.path.join(
        data_dir, "group_MNI_thresholded_VTC_mask.nii.gz"
    )  # voxels chosen with GLM contrast: stim vs non-stim
    ...
    bold_dir = os.path.join(
        data_dir, f"sub-{subID}", "item_representations_MDS"
    )  # for (sub)cate comparison
    ...
    if dim_red == "MDS":
        out_fname = os.path.join(
            results_dir, "MDS", f"sub-{subID}_{phase}_{stats_test}_{expt_tag}mdss"
        )
        ...
        out_fname = os.path.join(
            results_dir, "MDS", f"sub-{subID}_{phase}_{stats_test}_{expt_tag}_hc"
        )
        hc = hierarchical_clustering(item_repress, save=True, out_fname=out_fname)

        fig_fname = os.path.join(
            results_dir, "MDS", "figs", f"sub-{subID}_{phase}_{expt_tag}_hc"
        )
        plot_dendrogram(
            hc, labels, save=True, tag=f"sub{subID} {expt_tag}", out_fname=fig_fname
        )
