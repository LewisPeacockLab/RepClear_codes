...
def load_and_preprocess_bold_data(subID, brain_flag, phase, roi_name):
    ...
    concatenated_file_path = os.path.join(
        bold_path, f"sub-0{subID}_{brain_flag}_{phase}.nii.gz"
    )
    ...
    if os.path.exists(concatenated_file_path):
        ...
        localizer_files = find(
            f"sub*{phase}*{brain_flag}*preproc*bold*.nii.gz", bold_path
        )
    ...
    if brain_flag == "T1w":
        mask_path = os.path.join(
            container_path, "t1w_vtc_masks", f"sub-0{subID}_VTC_mask_T1w.nii.gz"
        )
    else:
        mask_path = os.path.join(
...
def item_representation(subID, phase, roi_name):
    ...
    brain_flags = ["T1w"]  # You can add other flags here as needed
    ...
    for brain_flag in brain_flags:
        ...
        events_path = f"/Volumes/zbg_eHD/Zachary_Data/processed/task-{phase}_events.tsv"  # Adjust this path
...
def check_missing_runs(subs, phases, rois):
    ...
    root_dir = "/Volumes/zbg_eHD/Zachary_Data/processed/"
        ...
        for phase in phases:
            for roi in rois:
                ...
                out_folder = os.path.join(
                    root_dir,
                    f"sub-0{subID}",
                    f"item_representations_{roi}_{brain_flag}",  # Assuming MNI as brain_flag
                )
