...
def item_level_1(subID):
    ...
    face_mask_path = os.path.join(
        "/scratch1/06873/zbretton/clearmem/group_model/group_category_lvl2/",
    ...
    temp_events = (
        events.copy()
    )  # copy the original events list, this list is set up where each "trial_type" is the image ID, so we dont need trial sorting
    ...
    for image_id in np.unique(events["trial_type"].values):
        if image_id == 0:
            ...
            item_contrast = np.full(
                np.unique(events["trial_type"].values).size, -1
            )  # start with an array of 0's
            item_contrast[0] = 0
            item_contrast[
                model.design_matrices_[0].columns[:55] == image_id
            ] = 53  # find all the indices of image_id and set to 54
    ...
    for contrast in contrasts:
        ...
        nib.save(
            z_map, os.path.join(out_folder, f"item{contrast}_{brain_flag}_zmap.nii.gz")
        )
        t_map = model.compute_contrast(
            contrasts[contrast], stat_type="t", output_type="stat"
        )
        nib.save(
            t_map, os.path.join(out_folder, f"item{contrast}_{brain_flag}_tmap.nii.gz")
        )
        file_data = model.generate_report(contrasts[contrast])
        file_data.save_as_html(
            os.path.join(out_folder, f"item{contrast}_{brain_flag}_report.html")
        )
