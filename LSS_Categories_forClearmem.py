...
def LSS_Category(subID):
    ...
    for image_id in np.unique(events["trial_type"].values):
        ...
        if image_id > 0:
            ...
            img_indx = np.where(temp_events["trial_type"] == image_id)[
                0
            ]  # find first occurance
            change_df = temp_events.index.isin(img_indx[:1])
            temp_events["trial_type"][
                ~change_df
            ] = "other"  # set the other trials to other
            temp_events["trial_type"][change_df] = (
                "image%s" % image_id
            )  # set our trial of interest to its image#
            ...
            contrast_array[
                np.where(model.design_matrices_[0].columns[:2].str.match("image"))[0][0]
            ] = 1
            contrast_array[
            ...
            contrasts = {
                "image%s" % (image_id): pad_contrast(contrast_array, n_columns)
            }
            ...
            for contrast in contrasts:
                ...
                nib.save(
                    t_map,
                    os.path.join(out_folder, f"{contrast}_{brain_flag}_tmap.nii.gz"),
                )
