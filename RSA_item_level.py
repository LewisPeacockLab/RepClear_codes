...
for num in range(len(subs)):
    ...
    fmri_img = concat_imgs(
        [localizer_run1, localizer_run2, localizer_run3, localizer_run4, localizer_run5]
    )  # unmasked
    background = concat_imgs(
        [localizer_run1, localizer_run2, localizer_run3, localizer_run4, localizer_run5]
    )  # take unmasked data to make background
    ...
    fmri_glm_fruit = FirstLevelModel(
        t_r=0.46,
        mask_img=fruit_mask,
        noise_model="ar1",
        standardize=False,
    ...
    fmri_glm_scene = FirstLevelModel(
        t_r=0.46,
        mask_img=scene_mask,
        noise_model="ar1",
        standardize=False,
    ...
    for x in range(1, 55):
        if x <= 18:
            stat_map_face = fmri_glm_face.compute_contrast(
                face_cont["%s vs other" % x], stat_type="t", output_type="stat"
            )
            z_map_face = fmri_glm_face.compute_contrast(
                face_cont["%s vs other" % x], stat_type="t", output_type="z_score"
            )
            # plot_stat_map(z_map_face, bg_img=mean_img_temp,
            #                display_mode='z', cut_coords=3, black_bg=True,
            #                title='Face item %s minus other' % x)
            # plt.show()
            z_map_face.to_filename(
                os.path.join(outdir, "item%s_v_other_face_z_map.nii.gz" % x)
            )
            stat_map_face.to_filename(
                os.path.join(outdir, "item%s_v_other_face_stat_map.nii.gz" % x)
            )
            del z_map_face, stat_map_face
        elif (x > 18) & (x <= 36):
            # fruit constrast
            stat_map_fruit = fmri_glm_fruit.compute_contrast(
                fruit_cont["%s vs other" % x], stat_type="t", output_type="stat"
            )
            z_map_fruit = fmri_glm_fruit.compute_contrast(
                fruit_cont["%s vs other" % x], stat_type="t", output_type="z_score"
            )
            # plot_stat_map(z_map_fruit, bg_img=mean_img_temp,
            #               display_mode='z', cut_coords=3, black_bg=True,
            #               title='Fruit item %s minus other' % x)
            # plt.show()
            z_map_fruit.to_filename(
                os.path.join(outdir, "item%s_v_other_fruit_z_map.nii.gz" % x)
            )
            stat_map_fruit.to_filename(
                os.path.join(outdir, "item%s_v_other_fruit_stat_map.nii.gz" % x)
            )
            del z_map_fruit, stat_map_fruit
        elif x > 36:
            # fruit constrast
            stat_map_scene = fmri_glm_scene.compute_contrast(
                scene_cont["%s vs other" % x], stat_type="t", output_type="stat"
            )
            z_map_scene = fmri_glm_scene.compute_contrast(
                scene_cont["%s vs other" % x], stat_type="t", output_type="z_score"
            )
            # plot_stat_map(z_map_scene, bg_img=mean_img_temp,
            #               display_mode='z', cut_coords=3, black_bg=True,
            #               title='Scene item %s minus other' % x)
            # plt.show()
            z_map_scene.to_filename(
                os.path.join(outdir, "item%s_v_other_scene_z_map.nii.gz" % x)
            )
            stat_map_scene.to_filename(
                os.path.join(outdir, "item%s_v_other_scene_stat_map.nii.gz" % x)
            )
