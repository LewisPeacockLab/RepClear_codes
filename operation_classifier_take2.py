...
def load_existing_data(
    subID, task, space, mask_ROIS, load=True
):  # try to find if the data we want has already been cleaned and saved...
...
def load_process_data(
    ...
    if mask_ROIS == ["wholebrain"]:  # 'wholebrain'
        ...
        mask_paths = [
            "/scratch/06873/zbretton/repclear_dataset/BIDS/derivatives/fmriprep/group_MNI_GM_mask.nii.gz"
        ]
    else:
    ...
    if mask_ROIS == ["wholebrain"]:  # 'wholebrain'
        ...
        temp = [
            sum(sum(sum(masks[0].get_fdata() == 1))),
            sum(sum(sum(masks[1].get_fdata() == 1))),
            sum(sum(sum(masks[2].get_fdata() == 1))),
        ]
...
def fit_model(X, Y, runs, save=False, out_fname=None, v=False):
    ...
    for train_inds, test_inds in ps.split():
        ...
        evidence = 1.0 / (1.0 + np.exp(-lr.decision_function(X_test_sub)))
...
def classification(subID):
    ...
    # model fitting
    score, auc_score, cm, evidence, roc_auc, tested_labels, y_scores = fit_model(
    ...
    auc_df.loc["Maintain"]["AUC"] = roc_auc.loc[
        :, 0
    ...
    evidence_df["maintain_evi"] = np.vstack(evidence)[
        :, 0
    ]  # add in the evidence values for maintain
    evidence_df["replace_evi"] = np.vstack(evidence)[
        :, 1
    ]  # add in the evidence values for replace
    evidence_df["suppress_evi"] = np.vstack(evidence)[
        :, 2
    ...
    for i in sub_condition_list:
        if (
            ...
            maintain_trials[temp_image] = sub_df[
                ["maintain_evi", "replace_evi", "suppress_evi"]
            ][sub_index[counter] : sub_index[counter] + x].values
            counter += 1
...
def organize_memory_evidence(subID, space, task, save=True):
    ...
    for i in sub_condition_list:
        if (
            ...
            temp_image = sub_images[counter]
            temp_memory = sub_df[sub_df["image_id"] == temp_image]["memory"].values[
                0
            ]  # this grabs the first index of the images, and checks the memory outcome

            if temp_memory == 1:
                maintain_remember_trials[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][sub_index[counter] : sub_index[counter] + x].values
            elif temp_memory == 0:
                maintain_forgot_trials[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][sub_index[counter] : sub_index[counter] + x].values
            counter += 1
            ...
            if temp_memory == 1:
                replace_remember_trials[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][sub_index[counter] : sub_index[counter] + x].values
            elif temp_memory == 0:
                replace_forgot_trials[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][sub_index[counter] : sub_index[counter] + x].values
            counter += 1
            ...
            if temp_memory == 1:
                suppress_remember_trials[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][sub_index[counter] : sub_index[counter] + x].values
            elif temp_memory == 0:
                suppress_forgot_trials[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][sub_index[counter] : sub_index[counter] + x].values
            counter += 1

    # now that the trials are sorted, we need to get the subject average for each condition and memory:
    avg_remember_maintain = pd.DataFrame(
        data=np.dstack(maintain_remember_trials.values()).mean(axis=2)
    )
    avg_remember_replace = pd.DataFrame(
        data=np.dstack(replace_remember_trials.values()).mean(axis=2)
    )
    avg_remember_suppress = pd.DataFrame(
        data=np.dstack(suppress_remember_trials.values()).mean(axis=2)
    )

    if maintain_forgot_trials:
        avg_forgot_maintain = pd.DataFrame(
            data=np.dstack(maintain_forgot_trials.values()).mean(axis=2)
        )
    else:
        avg_forgot_maintain = pd.DataFrame()
    ...
    if replace_forgot_trials:
        avg_forgot_replace = pd.DataFrame(
            data=np.dstack(replace_forgot_trials.values()).mean(axis=2)
        )
    else:
        avg_forgot_replace = pd.DataFrame()
    ...
    if suppress_forgot_trials:
        avg_forgot_suppress = pd.DataFrame(
            data=np.dstack(suppress_forgot_trials.values()).mean(axis=2)
        )
    else:
        avg_forgot_suppress = pd.DataFrame()

    # now I will have to change the structure to be able to plot in seaborn:
    avg_remember_maintain = (
        avg_remember_maintain.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_remember_maintain["sub"] = np.repeat(
        subID, len(avg_remember_maintain)
    )  # input the subject so I can stack melted dfs
    avg_remember_maintain["evidence_class"] = np.tile(
        ["maintain", "replace", "suppress"], x
    )  # add in the labels so we know what each data point is refering to
    avg_remember_maintain.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_remember_maintain[
        "condition"
    ...
    avg_forgot_maintain = (
        avg_forgot_maintain.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_forgot_maintain["sub"] = np.repeat(
        subID, len(avg_forgot_maintain)
    )  # input the subject so I can stack melted dfs
    avg_forgot_maintain["evidence_class"] = np.tile(
        ["maintain", "replace", "suppress"], x
    )  # add in the labels so we know what each data point is refering to
    avg_forgot_maintain.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_forgot_maintain[
        "condition"
    ...
    avg_remember_replace = (
        avg_remember_replace.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_remember_replace["sub"] = np.repeat(
        subID, len(avg_remember_replace)
    )  # input the subject so I can stack melted dfs
    avg_remember_replace["evidence_class"] = np.tile(
        ["maintain", "replace", "suppress"], x
    )  # add in the labels so we know what each data point is refering to
    avg_remember_replace.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_remember_replace[
        "condition"
    ...
    avg_forgot_replace = (
        avg_forgot_replace.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_forgot_replace["sub"] = np.repeat(
        subID, len(avg_forgot_replace)
    )  # input the subject so I can stack melted dfs
    avg_forgot_replace["evidence_class"] = np.tile(
        ["maintain", "replace", "suppress"], x
    )  # add in the labels so we know what each data point is refering to
    avg_forgot_replace.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_forgot_replace[
        "condition"
    ...
    avg_remember_suppress = (
        avg_remember_suppress.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_remember_suppress["sub"] = np.repeat(
        subID, len(avg_remember_suppress)
    )  # input the subject so I can stack melted dfs
    avg_remember_suppress["evidence_class"] = np.tile(
        ["maintain", "replace", "suppress"], x
    )  # add in the labels so we know what each data point is refering to
    avg_remember_suppress.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_remember_suppress[
        "condition"
    ...
    avg_forgot_suppress = (
        avg_forgot_suppress.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_forgot_suppress["sub"] = np.repeat(
        subID, len(avg_forgot_suppress)
    )  # input the subject so I can stack melted dfs
    avg_forgot_suppress["evidence_class"] = np.tile(
        ["maintain", "replace", "suppress"], x
    )  # add in the labels so we know what each data point is refering to
    avg_forgot_suppress.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_forgot_suppress[
        "condition"
    ...
    avg_remember_subject_df = pd.concat(
        [avg_remember_maintain, avg_remember_replace, avg_remember_suppress],
        ignore_index=True,
        sort=False,
    )

    avg_forgot_subject_df = pd.concat(
        [avg_forgot_maintain, avg_forgot_replace, avg_forgot_suppress],
        ignore_index=True,
        sort=False,
    )
    ...
    if save:
        sub_dir = os.path.join(data_dir, f"sub-{subID}")
        out_fname_template = (
            f"sub-{subID}_{space}_{task}_remember_evidence_dataframe.csv"
        )
        print(
        ...
        out_fname_template2 = (
            f"sub-{subID}_{space}_{task}_forgot_evidence_dataframe.csv"
        )
        print(
...
def visualize_evidence():
    ...
    ax = sns.lineplot(
        data=group_evidence_df.loc[
            (group_evidence_df["condition"] == "maintain")
            & (group_evidence_df["evidence_class"] == "maintain")
        ],
        x="TR",
        y="evidence",
        color="green",
    ...
    ax = sns.lineplot(
        data=group_evidence_df.loc[
            (group_evidence_df["condition"] == "replace")
            & (group_evidence_df["evidence_class"] == "replace")
        ],
        x="TR",
        y="evidence",
        color="blue",
    ...
    ax = sns.lineplot(
        data=group_evidence_df.loc[
            (group_evidence_df["condition"] == "suppress")
            & (group_evidence_df["evidence_class"] == "suppress")
        ],
        x="TR",
        y="evidence",
        color="red",
    ...
    ax.set(
        xlabel="TR",
        ylabel="Operation Classifier Evidence",
        title=f"{space} - Operation Classifier (group average)",
    )
    ax.set_ylim([0.3, 0.9])
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_decoding_during_removal.svg",
        )
    )
    plt.clf()
    ...
    for subID in subIDs:
        temp_remember_subject_df, temp_forgot_subject_df = organize_memory_evidence(
            subID, space, "study"
        )

        group_remember_evidence_df = pd.concat(
            [group_remember_evidence_df, temp_remember_subject_df],
            ignore_index=True,
            sort=False,
        )
        group_forgot_evidence_df = pd.concat(
            [group_forgot_evidence_df, temp_forgot_subject_df],
            ignore_index=True,
            sort=False,
        )

    ax = sns.lineplot(
        data=group_remember_evidence_df.loc[
            (group_remember_evidence_df["condition"] == "maintain")
            & (group_remember_evidence_df["evidence_class"] == "maintain")
        ],
        x="TR",
        y="evidence",
        color="green",
    ...
    ax = sns.lineplot(
        data=group_remember_evidence_df.loc[
            (group_remember_evidence_df["condition"] == "replace")
            & (group_remember_evidence_df["evidence_class"] == "replace")
        ],
        x="TR",
        y="evidence",
        color="blue",
    ...
    ax = sns.lineplot(
        data=group_remember_evidence_df.loc[
            (group_remember_evidence_df["condition"] == "suppress")
            & (group_remember_evidence_df["evidence_class"] == "suppress")
        ],
        x="TR",
        y="evidence",
        color="red",
    ...
    ax.set(
        xlabel="TR",
        ylabel="Operation Classifier Evidence",
        title=f"{space} - Operation Classifier (group average) - Remembered Items",
    )
    ax.set_ylim([0.3, 0.9])
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_decoding_during_removal_remember.png",
        )
    )
    plt.clf()

    ax = sns.lineplot(
        data=group_forgot_evidence_df.loc[
            (group_forgot_evidence_df["condition"] == "maintain")
            & (group_forgot_evidence_df["evidence_class"] == "maintain")
        ],
        x="TR",
        y="evidence",
        color="green",
    ...
    ax = sns.lineplot(
        data=group_forgot_evidence_df.loc[
            (group_forgot_evidence_df["condition"] == "replace")
            & (group_forgot_evidence_df["evidence_class"] == "replace")
        ],
        x="TR",
        y="evidence",
        color="blue",
    ...
    ax = sns.lineplot(
        data=group_forgot_evidence_df.loc[
            (group_forgot_evidence_df["condition"] == "suppress")
            & (group_forgot_evidence_df["evidence_class"] == "suppress")
        ],
        x="TR",
        y="evidence",
        color="red",
    ...
    ax.set(
        xlabel="TR",
        ylabel="Operation Classifier Evidence",
        title=f"{space} - Operation Classifier (group average) - Forgot Items",
    )
    ax.set_ylim([0.3, 0.9])
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_decoding_during_removal_forgot.png",
        )
    )
    plt.clf()
    ...
    group_diff_evidence_df["evidence"] = (
        group_remember_evidence_df["evidence"] - group_forgot_evidence_df["evidence"]
    )

    ax = sns.lineplot(
        data=group_diff_evidence_df.loc[
            (group_diff_evidence_df["condition"] == "maintain")
            & (group_diff_evidence_df["evidence_class"] == "maintain")
        ],
        x="TR",
        y="evidence",
        color="green",
    ...
    ax = sns.lineplot(
        data=group_diff_evidence_df.loc[
            (group_diff_evidence_df["condition"] == "replace")
            & (group_diff_evidence_df["evidence_class"] == "replace")
        ],
        x="TR",
        y="evidence",
        color="blue",
    ...
    ax = sns.lineplot(
        data=group_diff_evidence_df.loc[
            (group_diff_evidence_df["condition"] == "suppress")
            & (group_diff_evidence_df["evidence_class"] == "suppress")
        ],
        x="TR",
        y="evidence",
        color="red",
    ...
    ax.set(
        xlabel="TR",
        ylabel="Operation Classifier Evidence (Remember - Forgot)",
        title=f"{space} - Category Classifier (group average): Remember-Forgot",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_decoding_during_removal_difference.png",
        )
    )
    plt.clf()
    ...
    group_combined_evidence_df = pd.concat(
        [group_remember_evidence_df, group_forgot_evidence_df],
        ignore_index=True,
        sort=False,
    )

    ax = sns.lineplot(
        data=group_combined_evidence_df.loc[
            (group_combined_evidence_df["condition"] == "maintain")
            & (group_combined_evidence_df["evidence_class"] == "maintain")
        ],
        x="TR",
        y="evidence",
        style="memory",
    ...
    ax = sns.lineplot(
        data=group_combined_evidence_df.loc[
            (group_combined_evidence_df["condition"] == "replace")
            & (group_combined_evidence_df["evidence_class"] == "replace")
        ],
        x="TR",
        y="evidence",
        style="memory",
    ...
    ax = sns.lineplot(
        data=group_combined_evidence_df.loc[
            (group_combined_evidence_df["condition"] == "suppress")
            & (group_combined_evidence_df["evidence_class"] == "suppress")
        ],
        x="TR",
        y="evidence",
        style="memory",
    ...
    ax.set(
        xlabel="TR",
        ylabel="Operation Classifier Evidence",
        title=f"{space} - Operation Classifier (group average): Remember & Forgot",
    )
    ax.set_ylim([0.3, 0.9])
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_decoding_during_removal_RandF.png",
        )
    )
    plt.clf()
    ...
    ax.set_title(
        f"{space} - Operation Classifier during Maintain (group average): Remember-Forgot",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_decoding_during_maintain_difference.png",
        )
    )
    plt.clf()
    ...
    ax.set(xlabel="TR", ylabel="Operation Classifier Evidence (Remember - Forgot)")
    ax.set_title(
        f"{space} - Operation Classifier during Replace (group average): Remember-Forgot",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_decoding_during_replace_difference.png",
        )
    )
    plt.clf()
    ...
    ax.set(xlabel="TR", ylabel="Operation Classifier Evidence (Remember - Forgot)")
    ax.set_title(
        f"{space} - Operation Classifier during Suppress (group average): Remember-Forgot",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_decoding_during_suppress_difference.png",
        )
    )
    plt.clf()
...
def post_classification(subID):
    ...
    # need to then save the evidence:
    evidence_df = pd.DataFrame(
        columns=["stim_present", "operation", "predicts", "image_id"]
    )  # take the study DF for this subject
...
def visualize_post_evidence(space):
    ...
    ax.set(
        xlabel="TR",
        ylabel="Operation Classifier Evidence",
        title=f"{space} - Operation Classifier on Post-Removal Phase (group average)",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_decoding_during_postremoval.png",
        )
    )
    plt.clf()
    ...
    ax.set(
        xlabel="TR",
        ylabel="Operation Classifier Evidence",
        title=f"{space} - Operation Classifier on Post-Removal (group average) - Remembered Items",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_decoding_during_postremoval_remember.png",
        )
    )
    plt.clf()
    ...
    ax.set(
        xlabel="TR",
        ylabel="Operation Classifier Evidence",
        title=f"{space} - Operation Classifier on Post-Removal (group average) - Forgot Items",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_decoding_during_postremoval_forgot.png",
        )
    )
    plt.clf()
    ...
    ax.set(
        xlabel="TR",
        ylabel="Operation Classifier Evidence (Remember - Forgot)",
        title=f"{space} - Category Classifier on Post-Removal (group average): Remember-Forgot",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_decoding_during_postremoval_difference.png",
        )
    )
    plt.clf()
...
def organize_evidence_timewindow(subID, space, task, save=True):
    ...
    for i in sub_condition_list:
        if (
            ...
            temp_image = sub_images[counter]
            maintain_trials_early[temp_image] = sub_df[
                ["maintain_evi", "replace_evi", "suppress_evi"]
            ][sub_index[counter] : int(sub_index[counter] + (x / 2))].values.mean(
                axis=0
            )
            maintain_trials_late[temp_image] = sub_df[
                ["maintain_evi", "replace_evi", "suppress_evi"]
            ][
                int(sub_index[counter] + (x / 2)) : int(sub_index[counter] + x)
            ].values.mean(
                axis=0
            )
            maintain_trials_overall[temp_image] = sub_df[
                ["maintain_evi", "replace_evi", "suppress_evi"]
            ][int(sub_index[counter]) : int(sub_index[counter] + x)].values.mean(axis=0)
            counter += 1

        elif i == 2:
            temp_image = sub_images[counter]
            replace_trials_early[temp_image] = sub_df[
                ["maintain_evi", "replace_evi", "suppress_evi"]
            ][sub_index[counter] : int(sub_index[counter] + (x / 2))].values.mean(
                axis=0
            )
            replace_trials_late[temp_image] = sub_df[
                ["maintain_evi", "replace_evi", "suppress_evi"]
            ][
                int(sub_index[counter] + (x / 2)) : int(sub_index[counter] + x)
            ].values.mean(
            ...
            replace_trials_overall[temp_image] = sub_df[
                ["maintain_evi", "replace_evi", "suppress_evi"]
            ][int(sub_index[counter]) : int(sub_index[counter] + x)].values.mean(axis=0)
            counter += 1
            ...
            temp_image = sub_images[counter]
            suppress_trials_early[temp_image] = sub_df[
                ["maintain_evi", "replace_evi", "suppress_evi"]
            ][sub_index[counter] : int(sub_index[counter] + (x / 2))].values.mean(
                axis=0
            )
            suppress_trials_late[temp_image] = sub_df[
                ["maintain_evi", "replace_evi", "suppress_evi"]
            ][
                int(sub_index[counter] + (x / 2)) : int(sub_index[counter] + x)
            ].values.mean(
            ...
            suppress_trials_overall[temp_image] = sub_df[
                ["maintain_evi", "replace_evi", "suppress_evi"]
            ][int(sub_index[counter]) : int(sub_index[counter] + x)].values.mean(axis=0)
            counter += 1

    # now that the trials are sorted, we need to get the subject average for each condition:
    avg_maintain_early = pd.DataFrame(
        data=np.dstack(maintain_trials_early.values()).mean(axis=2)
    )
    avg_replace_early = pd.DataFrame(
        data=np.dstack(replace_trials_early.values()).mean(axis=2)
    )
    avg_suppress_early = pd.DataFrame(
        data=np.dstack(suppress_trials_early.values()).mean(axis=2)
    )

    avg_maintain_late = pd.DataFrame(
        data=np.dstack(maintain_trials_late.values()).mean(axis=2)
    )
    avg_replace_late = pd.DataFrame(
        data=np.dstack(replace_trials_late.values()).mean(axis=2)
    )
    avg_suppress_late = pd.DataFrame(
        data=np.dstack(suppress_trials_late.values()).mean(axis=2)
    )

    avg_maintain_overall = pd.DataFrame(
        data=np.dstack(maintain_trials_overall.values()).mean(axis=2)
    )
    avg_replace_overall = pd.DataFrame(
        data=np.dstack(replace_trials_overall.values()).mean(axis=2)
    )
    avg_suppress_overall = pd.DataFrame(
        data=np.dstack(suppress_trials_overall.values()).mean(axis=2)
    )

    maintain_early_df = pd.DataFrame(
        data=np.dstack(maintain_trials_early.values())[0],
        columns=maintain_trials_early.keys(),
    )
    maintain_early_df.drop(index=[1, 2], inplace=True)
    maintain_late_df = pd.DataFrame(
        data=np.dstack(maintain_trials_late.values())[0],
        columns=maintain_trials_late.keys(),
    )
    maintain_late_df.drop(index=[1, 2], inplace=True)
    maintain_overall_df = pd.DataFrame(
        data=np.dstack(maintain_trials_overall.values())[0],
        columns=maintain_trials_overall.keys(),
    )
    maintain_overall_df.drop(index=[1, 2], inplace=True)

    replace_early_df = pd.DataFrame(
        data=np.dstack(replace_trials_early.values())[0],
        columns=replace_trials_early.keys(),
    )
    replace_early_df.drop(index=[0, 2], inplace=True)
    replace_late_df = pd.DataFrame(
        data=np.dstack(replace_trials_late.values())[0],
        columns=replace_trials_late.keys(),
    )
    replace_late_df.drop(index=[0, 2], inplace=True)
    replace_overall_df = pd.DataFrame(
        data=np.dstack(replace_trials_overall.values())[0],
        columns=replace_trials_overall.keys(),
    )
    replace_overall_df.drop(index=[0, 2], inplace=True)

    suppress_early_df = pd.DataFrame(
        data=np.dstack(suppress_trials_early.values())[0],
        columns=suppress_trials_early.keys(),
    )
    suppress_early_df.drop(index=[0, 1], inplace=True)
    suppress_late_df = pd.DataFrame(
        data=np.dstack(suppress_trials_late.values())[0],
        columns=suppress_trials_late.keys(),
    )
    suppress_late_df.drop(index=[0, 1], inplace=True)
    suppress_overall_df = pd.DataFrame(
        data=np.dstack(suppress_trials_overall.values())[0],
        columns=suppress_trials_overall.keys(),
    )
    ...
    ### maintain
    avg_maintain_early = (
        maintain_early_df.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_early["sub"] = np.repeat(
        subID, len(avg_maintain_early)
    )  # input the subject so I can stack melted dfs
    avg_maintain_early[
        "evidence_class"
    ] = "maintain"  # add in the labels so we know what each data point is refering to
    avg_maintain_early.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_early[
        "condition"
    ] = "maintain"  # now I want to add in a condition label, since I can then stack all 3 conditions into 1 array per subject
    ...
    avg_maintain_late = (
        maintain_late_df.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_late["sub"] = np.repeat(
        subID, len(avg_maintain_late)
    )  # input the subject so I can stack melted dfs
    avg_maintain_late[
        "evidence_class"
    ] = "maintain"  # add in the labels so we know what each data point is refering to
    avg_maintain_late.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_late[
        "condition"
    ...
    avg_maintain_overall = (
        maintain_overall_df.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_overall["sub"] = np.repeat(
        subID, len(avg_maintain_overall)
    )  # input the subject so I can stack melted dfs
    avg_maintain_overall[
        "evidence_class"
    ] = "maintain"  # add in the labels so we know what each data point is refering to
    avg_maintain_overall.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_overall[
        "condition"
    ...
    ### replace
    avg_replace_early = (
        replace_early_df.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_replace_early["sub"] = np.repeat(
        subID, len(avg_replace_early)
    )  # input the subject so I can stack melted dfs
    avg_replace_early[
        "evidence_class"
    ] = "replace"  # add in the labels so we know what each data point is refering to
    avg_replace_early.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_replace_early[
        "condition"
    ...
    avg_replace_late = (
        replace_late_df.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_replace_late["sub"] = np.repeat(
        subID, len(avg_replace_late)
    )  # input the subject so I can stack melted dfs
    avg_replace_late[
        "evidence_class"
    ] = "replace"  # add in the labels so we know what each data point is refering to
    avg_replace_late.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_replace_late[
        "condition"
    ...
    avg_replace_overall = (
        replace_overall_df.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_replace_overall["sub"] = np.repeat(
        subID, len(avg_replace_overall)
    )  # input the subject so I can stack melted dfs
    avg_replace_overall[
        "evidence_class"
    ] = "maintain"  # add in the labels so we know what each data point is refering to
    avg_replace_overall.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_replace_overall[
        "condition"
    ...
    ### suppress
    avg_suppress_early = (
        suppress_early_df.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_early["sub"] = np.repeat(
        subID, len(avg_suppress_early)
    )  # input the subject so I can stack melted dfs
    avg_suppress_early[
        "evidence_class"
    ] = "suppress"  # add in the labels so we know what each data point is refering to
    avg_suppress_early.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_early[
        "condition"
    ...
    avg_suppress_late = (
        suppress_late_df.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_late["sub"] = np.repeat(
        subID, len(avg_suppress_late)
    )  # input the subject so I can stack melted dfs
    avg_suppress_late[
        "evidence_class"
    ] = "suppress"  # add in the labels so we know what each data point is refering to
    avg_suppress_late.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_late[
        "condition"
    ...
    avg_suppress_overall = (
        suppress_overall_df.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_overall["sub"] = np.repeat(
        subID, len(avg_suppress_overall)
    )  # input the subject so I can stack melted dfs
    avg_suppress_overall[
        "evidence_class"
    ] = "maintain"  # add in the labels so we know what each data point is refering to
    avg_suppress_overall.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_overall[
        "condition"
    ...
    ### combine
    avg_subject_early_df = pd.concat(
        [avg_maintain_early, avg_replace_early, avg_suppress_early],
        ignore_index=True,
        sort=False,
    )

    avg_subject_late_df = pd.concat(
        [avg_maintain_late, avg_replace_late, avg_suppress_late],
        ignore_index=True,
        sort=False,
    )

    avg_subject_df = pd.concat(
        [avg_maintain_overall, avg_replace_overall, avg_suppress_overall],
        ignore_index=True,
        sort=False,
    )
    ...
    if save:
        ...
        out_fname_template = (
            f"sub-{subID}_{space}_{task}_evidence_overall_dataframe.csv"
        )
        print(
...
def organize_memory_evidence_timewindow(subID, space, task, save=True):
    ...
    for i in sub_condition_list:
        if (
            ...
            if temp_memory == 1:
                maintain_trials_early_r[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][sub_index[counter] : int(sub_index[counter] + (x / 2))].values.mean(
                    axis=0
                )
                maintain_trials_late_r[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][
                    int(sub_index[counter] + (x / 2)) : int(sub_index[counter] + x)
                ].values.mean(
                ...
                maintain_trials_overall_r[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][int(sub_index[counter]) : int(sub_index[counter] + x)].values.mean(
                    axis=0
                )
            elif temp_memory == 0:
                maintain_trials_early_f[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][sub_index[counter] : int(sub_index[counter] + (x / 2))].values.mean(
                    axis=0
                )
                maintain_trials_late_f[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][
                    int(sub_index[counter] + (x / 2)) : int(sub_index[counter] + x)
                ].values.mean(
                ...
                maintain_trials_overall_f[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][int(sub_index[counter]) : int(sub_index[counter] + x)].values.mean(
                    axis=0
                )
            ...
            if temp_memory == 1:
                replace_trials_early_r[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][sub_index[counter] : int(sub_index[counter] + (x / 2))].values.mean(
                    axis=0
                )
                replace_trials_late_r[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][
                    int(sub_index[counter] + (x / 2)) : int(sub_index[counter] + x)
                ].values.mean(
                ...
                replace_trials_overall_r[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][int(sub_index[counter]) : int(sub_index[counter] + x)].values.mean(
                    axis=0
                )
            elif temp_memory == 0:
                replace_trials_early_f[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][sub_index[counter] : int(sub_index[counter] + (x / 2))].values.mean(
                    axis=0
                )
                replace_trials_late_f[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][
                    int(sub_index[counter] + (x / 2)) : int(sub_index[counter] + x)
                ].values.mean(
                ...
                replace_trials_overall_f[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][int(sub_index[counter]) : int(sub_index[counter] + x)].values.mean(
                    axis=0
                )
            ...
            if temp_memory == 1:
                suppress_trials_early_r[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][sub_index[counter] : int(sub_index[counter] + (x / 2))].values.mean(
                    axis=0
                )
                suppress_trials_late_r[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][
                    int(sub_index[counter] + (x / 2)) : int(sub_index[counter] + x)
                ].values.mean(
                ...
                suppress_trials_overall_r[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][int(sub_index[counter]) : int(sub_index[counter] + x)].values.mean(
                    axis=0
                )
            elif temp_memory == 0:
                suppress_trials_early_f[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][sub_index[counter] : int(sub_index[counter] + (x / 2))].values.mean(
                    axis=0
                )
                suppress_trials_late_f[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][
                    int(sub_index[counter] + (x / 2)) : int(sub_index[counter] + x)
                ].values.mean(
                ...
                suppress_trials_overall_f[temp_image] = sub_df[
                    ["maintain_evi", "replace_evi", "suppress_evi"]
                ][int(sub_index[counter]) : int(sub_index[counter] + x)].values.mean(
                    axis=0
                )
    ...
    ##maintain
    maintain_early_df_r = pd.DataFrame(
        data=np.dstack(maintain_trials_early_r.values())[0],
        columns=maintain_trials_early_r.keys(),
    )
    maintain_early_df_r.drop(index=[1, 2], inplace=True)
    if maintain_trials_early_f:
        maintain_early_df_f = pd.DataFrame(
            data=np.dstack(maintain_trials_early_f.values())[0],
            columns=maintain_trials_early_f.keys(),
        )
        maintain_early_df_f.drop(index=[1, 2], inplace=True)
    else:
        maintain_early_df_f = pd.DataFrame()

    maintain_late_df_r = pd.DataFrame(
        data=np.dstack(maintain_trials_late_r.values())[0],
        columns=maintain_trials_late_r.keys(),
    )
    maintain_late_df_r.drop(index=[1, 2], inplace=True)
    if maintain_trials_late_f:
        maintain_late_df_f = pd.DataFrame(
            data=np.dstack(maintain_trials_late_f.values())[0],
            columns=maintain_trials_late_f.keys(),
        )
        maintain_late_df_f.drop(index=[1, 2], inplace=True)
    else:
        maintain_late_df_f = pd.DataFrame()

    maintain_overall_df_r = pd.DataFrame(
        data=np.dstack(maintain_trials_overall_r.values())[0],
        columns=maintain_trials_overall_r.keys(),
    )
    maintain_overall_df_r.drop(index=[1, 2], inplace=True)
    if maintain_trials_overall_f:
        maintain_overall_df_f = pd.DataFrame(
            data=np.dstack(maintain_trials_overall_f.values())[0],
            columns=maintain_trials_overall_f.keys(),
        )
    ...
    ## replace
    replace_early_df_r = pd.DataFrame(
        data=np.dstack(replace_trials_early_r.values())[0],
        columns=replace_trials_early_r.keys(),
    )
    replace_early_df_r.drop(index=[0, 2], inplace=True)
    if replace_trials_early_f:
        replace_early_df_f = pd.DataFrame(
            data=np.dstack(replace_trials_early_f.values())[0],
            columns=replace_trials_early_f.keys(),
        )
        replace_early_df_f.drop(index=[0, 2], inplace=True)
    else:
        replace_early_df_f = pd.DataFrame()

    replace_late_df_r = pd.DataFrame(
        data=np.dstack(replace_trials_late_r.values())[0],
        columns=replace_trials_late_r.keys(),
    )
    replace_late_df_r.drop(index=[0, 2], inplace=True)
    if replace_trials_late_f:
        replace_late_df_f = pd.DataFrame(
            data=np.dstack(replace_trials_late_f.values())[0],
            columns=replace_trials_late_f.keys(),
        )
        replace_late_df_f.drop(index=[0, 2], inplace=True)
    else:
        replace_late_df_f = pd.DataFrame()

    replace_overall_df_r = pd.DataFrame(
        data=np.dstack(replace_trials_overall_r.values())[0],
        columns=replace_trials_overall_r.keys(),
    )
    replace_overall_df_r.drop(index=[0, 2], inplace=True)
    if replace_trials_overall_f:
        replace_overall_df_f = pd.DataFrame(
            data=np.dstack(replace_trials_overall_f.values())[0],
            columns=replace_trials_overall_f.keys(),
        )
    ...
    ## suppress
    suppress_early_df_r = pd.DataFrame(
        data=np.dstack(suppress_trials_early_r.values())[0],
        columns=suppress_trials_early_r.keys(),
    )
    suppress_early_df_r.drop(index=[0, 1], inplace=True)
    if suppress_trials_early_f:
        suppress_early_df_f = pd.DataFrame(
            data=np.dstack(suppress_trials_early_f.values())[0],
            columns=suppress_trials_early_f.keys(),
        )
        suppress_early_df_f.drop(index=[0, 1], inplace=True)
    else:
        suppress_early_df_f = pd.DataFrame()

    suppress_late_df_r = pd.DataFrame(
        data=np.dstack(suppress_trials_late_r.values())[0],
        columns=suppress_trials_late_r.keys(),
    )
    suppress_late_df_r.drop(index=[0, 1], inplace=True)
    if suppress_trials_late_f:
        suppress_late_df_f = pd.DataFrame(
            data=np.dstack(suppress_trials_late_f.values())[0],
            columns=suppress_trials_late_f.keys(),
        )
        suppress_late_df_f.drop(index=[0, 1], inplace=True)
    else:
        suppress_late_df_f = pd.DataFrame()

    suppress_overall_df_r = pd.DataFrame(
        data=np.dstack(suppress_trials_overall_r.values())[0],
        columns=suppress_trials_overall_r.keys(),
    )
    suppress_overall_df_r.drop(index=[0, 1], inplace=True)
    if suppress_trials_overall_f:
        suppress_overall_df_f = pd.DataFrame(
            data=np.dstack(suppress_trials_overall_f.values())[0],
            columns=suppress_trials_overall_f.keys(),
        )
    ...
    # now I will have to change the structure to be able to plot in seaborn:
    ### maintain - remember
    avg_maintain_early_r = (
        maintain_early_df_r.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_early_r["sub"] = np.repeat(
        subID, len(avg_maintain_early_r)
    )  # input the subject so I can stack melted dfs
    avg_maintain_early_r[
        "evidence_class"
    ] = "maintain"  # add in the labels so we know what each data point is refering to
    avg_maintain_early_r.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_early_r[
        "condition"
    ...
    avg_maintain_late_r = (
        maintain_late_df_r.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_late_r["sub"] = np.repeat(
        subID, len(avg_maintain_late_r)
    )  # input the subject so I can stack melted dfs
    avg_maintain_late_r[
        "evidence_class"
    ] = "maintain"  # add in the labels so we know what each data point is refering to
    avg_maintain_late_r.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_late_r[
        "condition"
    ...
    avg_maintain_overall_r = (
        maintain_overall_df_r.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_overall_r["sub"] = np.repeat(
        subID, len(avg_maintain_overall_r)
    )  # input the subject so I can stack melted dfs
    avg_maintain_overall_r[
        "evidence_class"
    ] = "maintain"  # add in the labels so we know what each data point is refering to
    avg_maintain_overall_r.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_overall_r[
        "condition"
    ...
    ### maintain - forgot
    avg_maintain_early_f = (
        maintain_early_df_f.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_early_f["sub"] = np.repeat(
        subID, len(avg_maintain_early_f)
    )  # input the subject so I can stack melted dfs
    avg_maintain_early_f[
        "evidence_class"
    ] = "maintain"  # add in the labels so we know what each data point is refering to
    avg_maintain_early_f.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_early_f[
        "condition"
    ...
    avg_maintain_late_f = (
        maintain_late_df_f.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_late_f["sub"] = np.repeat(
        subID, len(avg_maintain_late_f)
    )  # input the subject so I can stack melted dfs
    avg_maintain_late_f[
        "evidence_class"
    ] = "maintain"  # add in the labels so we know what each data point is refering to
    avg_maintain_late_f.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_late_f[
        "condition"
    ...
    avg_maintain_overall_f = (
        maintain_overall_df_f.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_overall_f["sub"] = np.repeat(
        subID, len(avg_maintain_overall_f)
    )  # input the subject so I can stack melted dfs
    avg_maintain_overall_f[
        "evidence_class"
    ] = "maintain"  # add in the labels so we know what each data point is refering to
    avg_maintain_overall_f.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_overall_f[
        "condition"
    ...
    ### replace - remember
    avg_replace_early_r = (
        replace_early_df_r.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_replace_early_r["sub"] = np.repeat(
        subID, len(avg_replace_early_r)
    )  # input the subject so I can stack melted dfs
    avg_replace_early_r[
        "evidence_class"
    ] = "replace"  # add in the labels so we know what each data point is refering to
    avg_replace_early_r.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_replace_early_r[
        "condition"
    ...
    avg_replace_late_r = (
        replace_late_df_r.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_replace_late_r["sub"] = np.repeat(
        subID, len(avg_replace_late_r)
    )  # input the subject so I can stack melted dfs
    avg_replace_late_r[
        "evidence_class"
    ] = "replace"  # add in the labels so we know what each data point is refering to
    avg_replace_late_r.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_replace_late_r[
        "condition"
    ...
    avg_replace_overall_r = (
        replace_overall_df_r.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_replace_overall_r["sub"] = np.repeat(
        subID, len(avg_replace_overall_r)
    )  # input the subject so I can stack melted dfs
    avg_replace_overall_r[
        "evidence_class"
    ] = "replace"  # add in the labels so we know what each data point is refering to
    avg_replace_overall_r.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_replace_overall_r[
        "condition"
    ...
    ### replace - forgot
    avg_replace_early_f = (
        replace_early_df_f.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_replace_early_f["sub"] = np.repeat(
        subID, len(avg_replace_early_f)
    )  # input the subject so I can stack melted dfs
    avg_replace_early_f[
        "evidence_class"
    ] = "replace"  # add in the labels so we know what each data point is refering to
    avg_replace_early_f.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_replace_early_f[
        "condition"
    ...
    avg_replace_late_f = (
        replace_late_df_f.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_replace_late_f["sub"] = np.repeat(
        subID, len(avg_replace_late_f)
    )  # input the subject so I can stack melted dfs
    avg_replace_late_f[
        "evidence_class"
    ] = "replace"  # add in the labels so we know what each data point is refering to
    avg_replace_late_f.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_replace_late_f[
        "condition"
    ...
    avg_replace_overall_f = (
        replace_overall_df_f.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_replace_overall_f["sub"] = np.repeat(
        subID, len(avg_replace_overall_f)
    )  # input the subject so I can stack melted dfs
    avg_replace_overall_f[
        "evidence_class"
    ] = "replace"  # add in the labels so we know what each data point is refering to
    avg_replace_overall_f.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_replace_overall_f[
        "condition"
    ...
    ### suppress - remember
    avg_suppress_early_r = (
        suppress_early_df_r.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_early_r["sub"] = np.repeat(
        subID, len(avg_suppress_early_r)
    )  # input the subject so I can stack melted dfs
    avg_suppress_early_r[
        "evidence_class"
    ] = "suppress"  # add in the labels so we know what each data point is refering to
    avg_suppress_early_r.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_early_r[
        "condition"
    ...
    avg_suppress_late_r = (
        suppress_late_df_r.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_late_r["sub"] = np.repeat(
        subID, len(avg_suppress_late_r)
    )  # input the subject so I can stack melted dfs
    avg_suppress_late_r[
        "evidence_class"
    ] = "suppress"  # add in the labels so we know what each data point is refering to
    avg_suppress_late_r.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_late_r[
        "condition"
    ...
    avg_suppress_overall_r = (
        suppress_overall_df_r.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_overall_r["sub"] = np.repeat(
        subID, len(avg_suppress_overall_r)
    )  # input the subject so I can stack melted dfs
    avg_suppress_overall_r[
        "evidence_class"
    ] = "suppress"  # add in the labels so we know what each data point is refering to
    avg_suppress_overall_r.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_overall_r[
        "condition"
    ...
    ### suppress - forgot
    avg_suppress_early_f = (
        suppress_early_df_f.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_early_f["sub"] = np.repeat(
        subID, len(avg_suppress_early_f)
    )  # input the subject so I can stack melted dfs
    avg_suppress_early_f[
        "evidence_class"
    ] = "suppress"  # add in the labels so we know what each data point is refering to
    avg_suppress_early_f.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_early_f[
        "condition"
    ...
    avg_suppress_late_f = (
        suppress_late_df_f.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_late_f["sub"] = np.repeat(
        subID, len(avg_suppress_late_f)
    )  # input the subject so I can stack melted dfs
    avg_suppress_late_f[
        "evidence_class"
    ] = "suppress"  # add in the labels so we know what each data point is refering to
    avg_suppress_late_f.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_late_f[
        "condition"
    ...
    avg_suppress_overall_f = (
        suppress_overall_df_f.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_overall_f["sub"] = np.repeat(
        subID, len(avg_suppress_overall_f)
    )  # input the subject so I can stack melted dfs
    avg_suppress_overall_f[
        "evidence_class"
    ] = "suppress"  # add in the labels so we know what each data point is refering to
    avg_suppress_overall_f.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_overall_f[
        "condition"
    ...
    ### combine
    avg_subject_early_remember_df = pd.concat(
        [avg_maintain_early_r, avg_replace_early_r, avg_suppress_early_r],
        ignore_index=True,
        sort=False,
    )
    avg_subject_early_forgot_df = pd.concat(
        [avg_maintain_early_f, avg_replace_early_f, avg_suppress_early_f],
        ignore_index=True,
        sort=False,
    )

    avg_subject_late_remember_df = pd.concat(
        [avg_maintain_late_r, avg_replace_late_r, avg_suppress_late_r],
        ignore_index=True,
        sort=False,
    )
    avg_subject_late_forgot_df = pd.concat(
        [avg_maintain_late_f, avg_replace_late_f, avg_suppress_late_f],
        ignore_index=True,
        sort=False,
    )

    avg_subject_remember_df = pd.concat(
        [avg_maintain_overall_r, avg_replace_overall_r, avg_suppress_overall_r],
        ignore_index=True,
        sort=False,
    )
    avg_subject_forgot_df = pd.concat(
        [avg_maintain_overall_f, avg_replace_overall_f, avg_suppress_overall_f],
        ignore_index=True,
        sort=False,
    )
    ...
    if save:
        sub_dir = os.path.join(data_dir, f"sub-{subID}")
        out_fname_template = (
            f"sub-{subID}_{space}_{task}_evidence_remember_early_dataframe.csv"
        )
        print(
        ...
        out_fname_template = (
            f"sub-{subID}_{space}_{task}_evidence_remember_late_dataframe.csv"
        )
        print(
        ...
        out_fname_template = (
            f"sub-{subID}_{space}_{task}_evidence_remember_overall_dataframe.csv"
        )
        print(
        ...
        out_fname_template = (
            f"sub-{subID}_{space}_{task}_evidence_forgot_early_dataframe.csv"
        )
        print(
        ...
        out_fname_template = (
            f"sub-{subID}_{space}_{task}_evidence_forgot_late_dataframe.csv"
        )
        print(
        ...
        out_fname_template = (
            f"sub-{subID}_{space}_{task}_evidence_forgot_overall_dataframe.csv"
        )
        print(
    ...
    return (
        avg_subject_early_remember_df,
        avg_subject_early_forgot_df,
        avg_subject_late_remember_df,
        avg_subject_late_forgot_df,
        avg_subject_remember_df,
        avg_subject_forgot_df,
    )
...
def coef_stim_operation(subID, save=True, content_oper=False):
    ...
    (
        avg_subject_early_df,
        avg_subject_late_df,
        avg_subject_overall_df,
    ) = organize_evidence_timewindow(subID, space, task2)
    ...
    for i in sub_condition_list:
        if (
            ...
            maintain_trials[temp_image] = sub_df[["rest_evi", "scene_evi", "face_evi"]][
                sub_index[counter] + 2 : sub_index[counter] + 6
            ].values.mean(axis=0)[
                1
            ]  # taking the average of the 4 operation TRs and then only pulling out the scene evidence
            counter += 1
    ...
    auc_df = pd.read_csv(
        os.path.join(sub_dir, f"sub-{subID}_{space}_{task2}_operation_auc.csv"),
        index_col=0,
    )

    # now that the trials are sorted, we need to get the subject average for each condition:
    avg_maintain_df = pd.DataFrame(
        data=np.dstack(maintain_trials.values())[0], columns=maintain_trials.keys()
    )
    avg_maintain_df = (
        avg_maintain_df.melt()
    )  # now you get 2 columns: variable and value (evidence)
    avg_maintain_df["sub"] = np.repeat(
        subID, len(avg_maintain_df)
    )  # input the subject so I can stack melted dfs
    avg_maintain_df[
        "evidence_class"
    ] = "scene"  # add in the labels so we know what each data point is refering to
    avg_maintain_df.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_df[
        "condition"
    ...
    avg_replace_df = pd.DataFrame(
        data=np.dstack(replace_trials.values())[0], columns=replace_trials.keys()
    )
    avg_replace_df = (
        avg_replace_df.melt()
    )  # now you get 2 columns: variable and value (evidence)
    avg_replace_df["sub"] = np.repeat(
        subID, len(avg_replace_df)
    )  # input the subject so I can stack melted dfs
    avg_replace_df[
        "evidence_class"
    ] = "scene"  # add in the labels so we know what each data point is refering to
    avg_replace_df.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_replace_df[
        "condition"
    ...
    avg_suppress_df = pd.DataFrame(
        data=np.dstack(suppress_trials.values())[0], columns=suppress_trials.keys()
    )
    avg_suppress_df = (
        avg_suppress_df.melt()
    )  # now you get 2 columns: variable and value (evidence)
    avg_suppress_df["sub"] = np.repeat(
        subID, len(avg_suppress_df)
    )  # input the subject so I can stack melted dfs
    avg_suppress_df[
        "evidence_class"
    ] = "scene"  # add in the labels so we know what each data point is refering to
    avg_suppress_df.rename(
        columns={"variable": "image_id", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_df[
        "condition"
    ...
    avg_subject_category_df = pd.concat(
        [avg_maintain_df, avg_replace_df, avg_suppress_df],
        ignore_index=True,
        sort=False,
    )

    maintain_early_operation_evi = avg_subject_early_df.loc[
        avg_subject_early_df["condition"] == "maintain"
    ]["evidence"].values.reshape(-1, 1)
    maintain_late_operation_evi = avg_subject_late_df.loc[
        avg_subject_late_df["condition"] == "maintain"
    ]["evidence"].values.reshape(-1, 1)
    maintain_overall_operation_evi = avg_subject_overall_df.loc[
        avg_subject_overall_df["condition"] == "maintain"
    ]["evidence"].values.reshape(-1, 1)

    maintain_category_evi = avg_maintain_df["evidence"].values.reshape(-1, 1)

    replace_early_operation_evi = avg_subject_early_df.loc[
        avg_subject_early_df["condition"] == "replace"
    ]["evidence"].values.reshape(-1, 1)
    replace_late_operation_evi = avg_subject_late_df.loc[
        avg_subject_late_df["condition"] == "replace"
    ]["evidence"].values.reshape(-1, 1)
    replace_overall_operation_evi = avg_subject_overall_df.loc[
        avg_subject_overall_df["condition"] == "replace"
    ]["evidence"].values.reshape(-1, 1)

    replace_category_evi = avg_replace_df["evidence"].values.reshape(-1, 1)

    suppress_early_operation_evi = avg_subject_early_df.loc[
        avg_subject_early_df["condition"] == "suppress"
    ]["evidence"].values.reshape(-1, 1)
    suppress_late_operation_evi = avg_subject_late_df.loc[
        avg_subject_late_df["condition"] == "suppress"
    ]["evidence"].values.reshape(-1, 1)
    suppress_overall_operation_evi = avg_subject_overall_df.loc[
        avg_subject_overall_df["condition"] == "suppress"
    ]["evidence"].values.reshape(-1, 1)

    suppress_category_evi = avg_suppress_df["evidence"].values.reshape(-1, 1)

    subject_coef_df = pd.DataFrame(
        columns=["sub", "condition", "beta", "timing"],
    ...
    maintain_early_lr = LinearRegression().fit(
        maintain_early_operation_evi, maintain_category_evi
    )
    subject_coef_df.loc[0, "beta"] = maintain_early_lr.coef_[0][0]
    subject_coef_df.loc[0, "timing"] = "early"
    maintain_late_lr = LinearRegression().fit(
        maintain_late_operation_evi, maintain_category_evi
    )
    subject_coef_df.loc[1, "beta"] = maintain_late_lr.coef_[0][0]
    subject_coef_df.loc[1, "timing"] = "late"
    maintain_overall_lr = LinearRegression().fit(
        maintain_overall_operation_evi, maintain_category_evi
    )
    subject_coef_df.loc[2, "beta"] = maintain_overall_lr.coef_[0][0]
    subject_coef_df.loc[2, "timing"] = "overall"

    replace_early_lr = LinearRegression().fit(
        replace_early_operation_evi, replace_category_evi
    )
    subject_coef_df.loc[3, "beta"] = replace_early_lr.coef_[0][0]
    subject_coef_df.loc[3, "timing"] = "early"
    replace_late_lr = LinearRegression().fit(
        replace_late_operation_evi, replace_category_evi
    )
    subject_coef_df.loc[4, "beta"] = replace_late_lr.coef_[0][0]
    subject_coef_df.loc[4, "timing"] = "late"
    replace_overall_lr = LinearRegression().fit(
        replace_overall_operation_evi, replace_category_evi
    )
    subject_coef_df.loc[5, "beta"] = replace_overall_lr.coef_[0][0]
    subject_coef_df.loc[5, "timing"] = "overall"

    suppress_early_lr = LinearRegression().fit(
        suppress_early_operation_evi, suppress_category_evi
    )
    subject_coef_df.loc[6, "beta"] = suppress_early_lr.coef_[0][0]
    subject_coef_df.loc[6, "timing"] = "early"
    suppress_late_lr = LinearRegression().fit(
        suppress_late_operation_evi, suppress_category_evi
    )
    subject_coef_df.loc[7, "beta"] = suppress_late_lr.coef_[0][0]
    subject_coef_df.loc[7, "timing"] = "late"
    suppress_overall_lr = LinearRegression().fit(
        suppress_overall_operation_evi, suppress_category_evi
    )
...
def coef_stim_memory_operation(subID, save=True):
    ...
    (
        ...
        avg_subject_late_remember_df,
        avg_subject_late_forgot_df,
        avg_subject_overall_remember_df,
        avg_subject_overall_forgot_df,
    ) = organize_memory_evidence_timewindow(subID, space, task2)
    ...
    ## maintain
    maintain_early_operation_remember_evi = avg_subject_early_remember_df.loc[
        avg_subject_early_remember_df["condition"] == "maintain"
    ]["evidence"].values.reshape(-1, 1)
    maintain_late_operation_remember_evi = avg_subject_late_remember_df.loc[
        avg_subject_late_remember_df["condition"] == "maintain"
    ]["evidence"].values.reshape(-1, 1)
    maintain_overall_operation_remember_evi = avg_subject_overall_remember_df.loc[
        avg_subject_overall_remember_df["condition"] == "maintain"
    ]["evidence"].values.reshape(-1, 1)

    maintain_early_operation_forgot_evi = avg_subject_early_forgot_df.loc[
        avg_subject_early_forgot_df["condition"] == "maintain"
    ]["evidence"].values.reshape(-1, 1)
    maintain_late_operation_forgot_evi = avg_subject_late_forgot_df.loc[
        avg_subject_late_forgot_df["condition"] == "maintain"
    ]["evidence"].values.reshape(-1, 1)
    maintain_overall_operation_forgot_evi = avg_subject_overall_forgot_df.loc[
        avg_subject_overall_forgot_df["condition"] == "maintain"
    ]["evidence"].values.reshape(-1, 1)

    maintain_remember_category_evi = avg_maintain_df[
        avg_maintain_df.image_id.isin(
            avg_subject_early_remember_df.loc[
                avg_subject_early_remember_df["condition"] == "maintain"
            ].image_id
        )
    ]["evidence"].values.reshape(-1, 1)
    maintain_forgot_category_evi = avg_maintain_df[
        avg_maintain_df.image_id.isin(
            avg_subject_early_forgot_df.loc[
                avg_subject_early_forgot_df["condition"] == "maintain"
            ].image_id
        )
    ]["evidence"].values.reshape(-1, 1)
    ...
    ## replace
    replace_early_operation_remember_evi = avg_subject_early_remember_df.loc[
        avg_subject_early_remember_df["condition"] == "replace"
    ]["evidence"].values.reshape(-1, 1)
    replace_late_operation_remember_evi = avg_subject_late_remember_df.loc[
        avg_subject_late_remember_df["condition"] == "replace"
    ]["evidence"].values.reshape(-1, 1)
    replace_overall_operation_remember_evi = avg_subject_overall_remember_df.loc[
        avg_subject_overall_remember_df["condition"] == "replace"
    ]["evidence"].values.reshape(-1, 1)

    replace_early_operation_forgot_evi = avg_subject_early_forgot_df.loc[
        avg_subject_early_forgot_df["condition"] == "replace"
    ]["evidence"].values.reshape(-1, 1)
    replace_late_operation_forgot_evi = avg_subject_late_forgot_df.loc[
        avg_subject_late_forgot_df["condition"] == "replace"
    ]["evidence"].values.reshape(-1, 1)
    replace_overall_operation_forgot_evi = avg_subject_overall_forgot_df.loc[
        avg_subject_overall_forgot_df["condition"] == "replace"
    ]["evidence"].values.reshape(-1, 1)

    replace_remember_category_evi = avg_replace_df[
        avg_replace_df.image_id.isin(
            avg_subject_early_remember_df.loc[
                avg_subject_early_remember_df["condition"] == "replace"
            ].image_id
        )
    ]["evidence"].values.reshape(-1, 1)
    replace_forgot_category_evi = avg_replace_df[
        avg_replace_df.image_id.isin(
            avg_subject_early_forgot_df.loc[
                avg_subject_early_forgot_df["condition"] == "replace"
            ].image_id
    ...
    ## suppress
    suppress_early_operation_remember_evi = avg_subject_early_remember_df.loc[
        avg_subject_early_remember_df["condition"] == "suppress"
    ]["evidence"].values.reshape(-1, 1)
    suppress_late_operation_remember_evi = avg_subject_late_remember_df.loc[
        avg_subject_late_remember_df["condition"] == "suppress"
    ]["evidence"].values.reshape(-1, 1)
    suppress_overall_operation_remember_evi = avg_subject_overall_remember_df.loc[
        avg_subject_overall_remember_df["condition"] == "suppress"
    ]["evidence"].values.reshape(-1, 1)

    suppress_early_operation_forgot_evi = avg_subject_early_forgot_df.loc[
        avg_subject_early_forgot_df["condition"] == "suppress"
    ]["evidence"].values.reshape(-1, 1)
    suppress_late_operation_forgot_evi = avg_subject_late_forgot_df.loc[
        avg_subject_late_forgot_df["condition"] == "suppress"
    ]["evidence"].values.reshape(-1, 1)
    suppress_overall_operation_forgot_evi = avg_subject_overall_forgot_df.loc[
        avg_subject_overall_forgot_df["condition"] == "suppress"
    ]["evidence"].values.reshape(-1, 1)

    suppress_remember_category_evi = avg_suppress_df[
        avg_suppress_df.image_id.isin(
            avg_subject_early_remember_df.loc[
                avg_subject_early_remember_df["condition"] == "suppress"
            ].image_id
        )
    ]["evidence"].values.reshape(-1, 1)
    suppress_forgot_category_evi = avg_suppress_df[
        avg_suppress_df.image_id.isin(
            avg_subject_early_forgot_df.loc[
                avg_subject_early_forgot_df["condition"] == "suppress"
            ].image_id
    ...
    ## combine
    subject_coef_remember_df = pd.DataFrame(
        columns=["sub", "condition", "beta", "timing"],
        index=[0, 1, 2, 3, 4, 5, 6, 7, 8],
    )
    subject_coef_remember_df["sub"] = subID
    subject_coef_remember_df["condition"] = np.repeat(
        ["maintain", "replace", "suppress"], 3
    )

    subject_coef_forgot_df = pd.DataFrame(
        columns=["sub", "condition", "beta", "timing"],
        index=[0, 1, 2, 3, 4, 5, 6, 7, 8],
    )
    subject_coef_forgot_df["sub"] = subID
    subject_coef_forgot_df["condition"] = np.repeat(
        ["maintain", "replace", "suppress"], 3
    )
...
def visualize_coef_dfs():
    ...
    for subID in subIDs:
        ...
        group_coef_df = pd.concat(
            [group_coef_df, temp_df], ignore_index=True, sort=False
        )

    ax = sns.barplot(data=group_coef_df, x="condition", y="beta", hue="timing")
    plt.legend()
    ax.set(
        xlabel="Operation on Item",
        ylabel="Beta",
        title=f"{space} - Operation prediction on Content Decoding",
    )
    ax.set_ylim([-0.3, 0.3])
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir, "figs", f"group_level_{space}_operation_category_prediction.png"
        )
    )
    plt.clf()
...
def bootstrap_auc():
    ...
    for i in range(0, 1000):
        ...
        maintain_bootstrap = np.append(
            maintain_bootstrap,
            LinearRegression().fit(maintain_itr_auc, maintain_itr_content).coef_[0][0],
        )
        replace_bootstrap = np.append(
            replace_bootstrap,
            LinearRegression().fit(replace_itr_auc, replace_itr_content).coef_[0][0],
        )
        suppress_bootstrap = np.append(
            suppress_bootstrap,
            LinearRegression().fit(suppress_itr_auc, suppress_itr_content).coef_[0][0],
        )

    true_maintain_beta = (
        LinearRegression()
        .fit(
            maintain_df["AUC"].values.reshape(-1, 1),
            maintain_df["Content"].values.reshape(-1, 1),
        )
        .coef_[0][0]
    )
    true_replace_beta = (
        LinearRegression()
        .fit(
            replace_df["AUC"].values.reshape(-1, 1),
            replace_df["Content"].values.reshape(-1, 1),
        )
        .coef_[0][0]
    )
    true_suppress_beta = (
        LinearRegression()
        .fit(
            suppress_df["AUC"].values.reshape(-1, 1),
            suppress_df["Content"].values.reshape(-1, 1),
        )
        .coef_[0][0]
    )
    ...
    ax.axvline(
        np.percentile(maintain_bootstrap, 97.5),
        0,
        ax.get_ylim()[1],
        color="k",
        linestyle="dashed",
    ...
    ax.set(
        xlabel="AUC vs. Content Betas",
        ylabel="Frequency",
        title=f"Super Subject - Maintian AUC predicting Content Decoding",
    )
    ax.axvline(
        true_maintain_beta, 0, ax.get_ylim()[1], color="g", label="True Maintain Beta"
    )
    ax.legend()
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir, "figs", "Super_subject_maintain_auc_content_prediction.png"
        )
    )
    plt.clf()
    ...
    ax.axvline(
        np.percentile(replace_bootstrap, 97.5),
        0,
        ax.get_ylim()[1],
        color="k",
        linestyle="dashed",
    ...
    ax.set(
        xlabel="AUC vs. Content Betas",
        ylabel="Frequency",
        title=f"Super Subject - Replace AUC predicting Content Decoding",
    )
    ax.axvline(
        true_suppress_beta, 0, ax.get_ylim()[1], color="b", label="True Replace Beta"
    )
    ax.legend()
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir, "figs", "Super_subject_replace_auc_content_prediction.png"
        )
    )
    plt.clf()
    ...
    ax.axvline(
        np.percentile(suppress_bootstrap, 97.5),
        0,
        ax.get_ylim()[1],
        color="k",
        linestyle="dashed",
    ...
    ax.axvline(
        true_suppress_beta, 0, ax.get_ylim()[1], color="r", label="True Suppress Beta"
    )
    ax.set(
        xlabel="AUC vs. Content Betas",
        ylabel="Frequency",
        title=f"Super Subject - Suppress AUC predicting Content Decoding",
    )
    ax.legend()
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir, "figs", "Super_subject_suppress_auc_content_prediction.png"
        )
    )
    plt.clf()
...
def visualize_coef_dfs_memory():
    ...
    for subID in subIDs:
        ...
        group_coef_remember_df = pd.concat(
            [group_coef_remember_df, temp_r_df], ignore_index=True, sort=False
        )
        group_coef_forgot_df = pd.concat(
            [group_coef_forgot_df, temp_f_df], ignore_index=True, sort=False
        )
    ...
    ax.set(
        xlabel="Operation on Item",
        ylabel="Beta",
        title=f"{space} - Operation prediction on Remembered Content Decoding",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_remembered_category_prediction.png",
        )
    )
    plt.clf()
    ...
    ax.set(
        xlabel="Operation on Item",
        ylabel="Beta",
        title=f"{space} - Operation prediction on Forgotten Content Decoding",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_operation_forgotten_category_prediction.png",
        )
    )
    plt.clf()
...
def permute_empericialnull():
    ...
    true_maintain_beta = (
        LinearRegression()
        .fit(
            maintain_auc_df["AUC"].values.reshape(-1, 1),
            maintain_content_df["Content"].values.reshape(-1, 1),
        )
        .coef_[0][0]
    )
    true_replace_beta = (
        LinearRegression()
        .fit(
            replace_auc_df["AUC"].values.reshape(-1, 1),
            replace_content_df["Content"].values.reshape(-1, 1),
        )
        .coef_[0][0]
    )
    true_suppress_beta = (
        LinearRegression()
        .fit(
            suppress_auc_df["AUC"].values.reshape(-1, 1),
            suppress_content_df["Content"].values.reshape(-1, 1),
        )
        .coef_[0][0]
    )
    ...
    ax.set_title(
        f"Emperical Null Distribution - Maintain AUC predicting Content Decoding",
        loc="center",
        wrap=True,
    )
    ax.axvline(
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(data_dir, "figs", "null_dist_maintain_auc_content_prediction.png")
    )
    plt.clf()
    ...
    ax.set(xlabel="AUC vs. Content Betas", ylabel="Frequency")
    ax.set_title(
        f"Emperical Null Distribution - Replace AUC predicting Content Decoding",
        loc="center",
        wrap=True,
    )
    ax.axvline(
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(data_dir, "figs", "null_dist_replace_auc_content_prediction.png")
    )
    plt.clf()
    ...
    ax.set(xlabel="AUC vs. Content Betas", ylabel="Frequency")
    ax.set_title(
        f"Emperical Null Distribution - Suppress AUC predicting Content Decoding",
        loc="center",
        wrap=True,
    )
    ax.legend()
    plt.tight_layout()
    plt.savefig(
        os.path.join(data_dir, "figs", "null_dist_suppress_auc_content_prediction.png")
    )
    plt.clf()
...
def bootstrap_content_oper():
    ...
    for subID in subIDs:
        maintain_df, replace_df, suppress_df = coef_stim_operation(
            subID, content_oper=True
        )
    ...
    for i in range(0, 1000):
        ...
        for x in range(0, 22):
            maintain_itr_operation = resample(
                maintain_operation_df.values, n_samples=30
            ).reshape(-1, 1)
            maintain_itr_content = resample(
                maintain_content_df.values, n_samples=30
            ).reshape(-1, 1)

            replace_itr_operation = resample(
                replace_operation_df.values, n_samples=30
            ).reshape(-1, 1)
            replace_itr_content = resample(
                replace_content_df.values, n_samples=30
            ).reshape(-1, 1)

            suppress_itr_operation = resample(
                suppress_operation_df.values, n_samples=30
            ).reshape(-1, 1)
            suppress_itr_content = resample(
                suppress_content_df.values, n_samples=30
            ).reshape(-1, 1)

            maintain_itr = resample(
                group_maintain_df[["content", "operation"]], n_samples=30
            )
            replace_itr = resample(
                group_replace_df[["content", "operation"]], n_samples=30
            )
            suppress_itr = resample(
                group_suppress_df[["content", "operation"]], n_samples=30
            )

            maintain_bootstrap_content = pd.concat(
                (
                    maintain_bootstrap_content,
                    maintain_itr["content"].reset_index(drop=True),
                ),
                axis=1,
                ignore_index=True,
            )
            replace_bootstrap_content = pd.concat(
                (
                    replace_bootstrap_content,
                    replace_itr["content"].reset_index(drop=True),
                ),
                axis=1,
                ignore_index=True,
            )
            suppress_bootstrap_content = pd.concat(
                (
                    suppress_bootstrap_content,
                    suppress_itr["content"].reset_index(drop=True),
                ),
                axis=1,
                ignore_index=True,
            )

            maintain_bootstrap_operation = pd.concat(
                (
                    maintain_bootstrap_operation,
                    maintain_itr["operation"].reset_index(drop=True),
                ),
                axis=1,
                ignore_index=True,
            )
            replace_bootstrap_operation = pd.concat(
                (
                    replace_bootstrap_operation,
                    replace_itr["operation"].reset_index(drop=True),
                ),
                axis=1,
                ignore_index=True,
            )
            suppress_bootstrap_operation = pd.concat(
                (
                    suppress_bootstrap_operation,
                    suppress_itr["operation"].reset_index(drop=True),
                ),
                axis=1,
                ignore_index=True,
            )

        maintain_itr_beta = np.append(
            maintain_itr_beta,
            LinearRegression()
            .fit(
                maintain_bootstrap_operation.values.reshape(-1, 1),
                maintain_bootstrap_content.values.reshape(-1, 1),
            )
            .coef_[0][0],
        )
        replace_itr_beta = np.append(
            replace_itr_beta,
            LinearRegression()
            .fit(
                replace_bootstrap_operation.values.reshape(-1, 1),
                replace_bootstrap_content.values.reshape(-1, 1),
            )
            .coef_[0][0],
        )
        suppress_itr_beta = np.append(
            suppress_itr_beta,
            LinearRegression()
            .fit(
                suppress_bootstrap_operation.values.reshape(-1, 1),
                suppress_bootstrap_content.values.reshape(-1, 1),
            )
            .coef_[0][0],
        )
    ...
    true_maintain_beta = (
        LinearRegression()
        .fit(
            group_maintain_df["operation"].values.reshape(-1, 1),
            group_maintain_df["content"].values.reshape(-1, 1),
        )
        .coef_[0][0]
    )
    true_replace_beta = (
        LinearRegression()
        .fit(
            group_replace_df["operation"].values.reshape(-1, 1),
            group_replace_df["content"].values.reshape(-1, 1),
        )
        .coef_[0][0]
    )
    true_suppress_beta = (
        LinearRegression()
        .fit(
            group_suppress_df["operation"].values.reshape(-1, 1),
            group_suppress_df["content"].values.reshape(-1, 1),
        )
        .coef_[0][0]
    )
    ...
    ax.set_title(
        f"SuperSub Bootstrap - Maintain Evidence predicting Content Decoding",
        loc="center",
        wrap=True,
    )
    ax.axvline(
    ...
    plt.savefig(
        os.path.join(
            data_dir, "figs", "supersub_maintain_operation_content_prediction.png"
        )
    )
    plt.clf()
    ...
    ax.set(xlabel="Operation vs. Content Betas", ylabel="Frequency")
    ax.set_title(
        f"SuperSub Bootstrap - Replace Evidence predicting Content Decoding",
        loc="center",
        wrap=True,
    )
    ax.axvline(
        true_replace_beta, 0, ax.get_ylim()[1], color="b", label="True Replace Beta"
    )
    ax.legend()
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir, "figs", "supersub_replace_operation_content_prediction.png"
        )
    )
    plt.clf()
    ...
    ax.axvline(0, 0, ax.get_ylim()[1], color="k", linestyle="dashed")
    ax.axvline(
        true_suppress_beta.mean(),
        0,
        ax.get_ylim()[1],
        color="r",
        label="True Suppress Beta",
    )
    ax.set(xlabel="Operation vs. Content Betas", ylabel="Frequency")
    ax.set_title(
        f"SuperSub Bootstrap - Suppress Evidence predicting Content Decoding",
        loc="center",
        wrap=True,
    )
    ax.legend()
    ...
    plt.savefig(
        os.path.join(
            data_dir, "figs", "supersub_suppress_operation_content_prediction.png"
        )
    )
    plt.clf()
    ...
    ax.set_title(
        "Proportion of bootstrapped iterations >0 Beta (Operation vs. Content Decoding)",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(data_dir, "figs", "proportion_bootstrapping_prediction.png")
    )
...
def sample_for_cross_sub(full_data_dir, label_df):
    ...
    for temp_sub in subIDs:
        temp_bold = full_data_dir[f"{temp_sub}"][
            oper_inds
        ]  # this pulls the data for the current subject

        first_half = np.concatenate(
            (temp_bold[0:150], temp_bold[300:450], temp_bold[600:750])
        )  # this is extracting the first half of each run for this current subject
        first_half_reg = np.concatenate(
            (
                sample_regressor[0:150],
                sample_regressor[300:450],
                sample_regressor[600:750],
            )
        )

        second_half = np.concatenate(
            (temp_bold[150:300], temp_bold[450:600], temp_bold[750:900])
        )  # this is extracting the second half of each run for this current subject
        second_half_reg = np.concatenate(
            (
                sample_regressor[150:300],
                sample_regressor[450:600],
                sample_regressor[750:900],
            )
        )
    ...
    sample_runs = np.repeat(
        range(0, 22), 450
    )  # this in reality isnt runs, but subjects
...
def fit_model_cross_sub(
    ...
    ps = PredefinedSplit(
        sample_runs
    )  # using the input subjects, we can train on everything BUT 1 subject
    for train_inds, test_inds in ps.split():
        if v:
            print("\n***** Training on 21 subjects and testing on 1")
        X_train, X_test, y_train, y_test = (
            testing_bold[train_inds],
            testing_bold[test_inds],
            testing_reg[train_inds],
            testing_reg[test_inds],
        )  # using the splits, I can now segment my "testing set", into training (all subs but 1) and testing (the held out sub)
...
def cross_sub():
    ...
    (
        ...
        testing_reg,
        sample_runs,
    ) = sample_for_cross_sub(full_data_dir, label_df)
    ...
    # need to then save the evidence:
    evidence_df = pd.DataFrame(
        columns=["subjects", "operation", "image_id"]
    )  # take the study DF for this subject
