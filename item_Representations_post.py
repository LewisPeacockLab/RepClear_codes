...
def item_representation_post(subID):
    ...
    if brain_flag == "T1w":
        mask_path = os.path.join(
            ...
            sub,
            "new_mask",
            "VVS_postremoval_%s_mask.nii.gz" % brain_flag,
        )
    ...
    if os.path.exists(
        os.path.join(bold_path, "sub-0%s_%s_postremoval.nii.gz" % (subID, brain_flag))
    ):
        img = nib.load(
            os.path.join(
                bold_path, "sub-0%s_%s_postremoval.nii.gz" % (subID, brain_flag)
            )
        )
        ...
        print(
            "%s Concatenated Post-Localizer BOLD...saved" % (brain_flag)
    ...
    if os.path.exists(
        os.path.join(
            bold_path,
            "sub-0%s_%s_postremoval_%s_cleaned.nii.gz" % (subID, brain_flag, mask_flag),
        )
    ):
        img_clean_scene = nib.load(
            os.path.join(
                bold_path,
                "sub-0%s_%s_postremoval_%s_cleaned.nii.gz"
                % (subID, brain_flag, mask_flag),
            )
        )

        print(
            "%s Concatenated, Cleaned and %s Masked Post-Localizer BOLD...LOADED"
            % (brain_flag, mask_flag)
        )
        del img
    else:
        print("Cleaning and Masking BOLD data...")
        img_clean_scene = clean_img(
            img,
            sessions=run_list,
            t_r=1,
            detrend=False,
        ...
        print(
            "%s Concatenated, Cleaned and %s Masked Post-Localizer BOLD...saved"
            % (brain_flag, mask_flag)
        )
        del img
        """load in the denoised bold data and events file"""
    events = pd.read_csv(
        "/work/06873/zbretton/ls6/repclear_dataset/BIDS/task-postremoval_events.tsv",
        sep="\t",
    )
    ...
    for trial in range(len(scene_index)):
        ...
        affine_mat = img_clean_scene.affine
...
Parallel(n_jobs=4)(delayed(item_representation_post)(i) for i in subs)
