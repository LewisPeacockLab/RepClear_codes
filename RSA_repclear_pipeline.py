...
for subID in subs:
    ...
    cate_repress_study = np.multiply(
        masked_bolds_arr_2, masked_cate_weights_arr
    )  # since there is only 1 cate_weight, this multiplies all of masked_bold_arr_2 with the cate_weights
    ...
    for trial in study_scene_order["trial_id"].values:
        ...
        if image_condition == 1:
            m_item_repress_study_comp[m_counter] = np.multiply(
                masked_bolds_arr_2[trial - 1, :],
                masked_weights_arr[pre_trial_num - 1, :],
            )
            m_item_repress_removal_comp[m_counter] = np.multiply(
                masked_bolds_arr_4[:, :, trial - 1],
                masked_weights_arr[pre_trial_num - 1, :].reshape(
                    (1, masked_weights_arr[pre_trial_num - 1, :].size)
                ),
            )
            m_item_repress_pre_comp[m_counter] = item_repress_pre[pre_trial_num - 1, :]
            m_counter = m_counter + 1
        elif image_condition == 2:
            r_item_repress_study_comp[r_counter] = np.multiply(
                masked_bolds_arr_2[trial - 1, :],
                masked_weights_arr[pre_trial_num - 1, :],
            )
            r_item_repress_removal_comp[r_counter] = np.multiply(
                masked_bolds_arr_4[:, :, trial - 1],
                masked_weights_arr[pre_trial_num - 1, :].reshape(
                    (1, masked_weights_arr[pre_trial_num - 1, :].size)
                ),
            ...
            s_item_repress_study_comp[s_counter] = np.multiply(
                masked_bolds_arr_2[trial - 1, :],
                masked_weights_arr[pre_trial_num - 1, :],
            )
            s_item_repress_removal_comp[s_counter] = np.multiply(
                masked_bolds_arr_4[:, :, trial - 1],
                masked_weights_arr[pre_trial_num - 1, :].reshape(
                    (1, masked_weights_arr[pre_trial_num - 1, :].size)
                ),
    ...
    m_item_pre_study_comp = np.corrcoef(
        m_item_repress_pre_comp, m_item_repress_study_comp
    )
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "maintain_item_weight_pre_study_RSA.csv",
        )
    )
    del temp_df

    r_item_pre_study_comp = np.corrcoef(
        r_item_repress_pre_comp, r_item_repress_study_comp
    )
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "replace_item_weight_pre_study_RSA.csv",
        )
    )
    del temp_df

    s_item_pre_study_comp = np.corrcoef(
        s_item_repress_pre_comp, s_item_repress_study_comp
    )
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "suppress_item_weight_pre_study_RSA.csv",
        )
    )
    del temp_df
    ...
    for trial in study_scene_order["trial_id"].values:
        ...
        item_repress_removal_comp[counter] = np.multiply(
            masked_bolds_arr_4[:, :, trial - 1],
            masked_weights_arr[pre_trial_num - 1, :].reshape(
                (1, masked_weights_arr[pre_trial_num - 1, :].size)
            ),
        ...
        item_repress_post_comp[counter] = np.multiply(
        ...
        pre_post_trial_iw_fidelity = np.corrcoef(
            item_repress_pre_comp[counter, :], item_repress_post_comp[counter, :]
        )
        # This is to get the fidelity of the current item/trial from pre to removal (item_weighted)
        pre_removal_trial_iw_fidelity = np.corrcoef(
            item_repress_pre_comp[counter, :], item_repress_removal_comp[counter]
        )
        # This is to get the fidelity of the current item/trial from study to post (item_weighted)
        study_post_trial_iw_fidelity = np.corrcoef(
            item_repress_study_comp[counter, :], item_repress_post_comp[counter, :]
        )
        # This is to get the fidelity of the current item/removal from study to post (item_weighted)
        removal_post_trial_iw_fidelity = np.corrcoef(
            item_repress_removal_comp[counter], item_repress_post_comp[counter, :]
        )
        ...
        pre_post_trial_uw_fidelity = np.corrcoef(
            non_weight_pre_comp[counter, :], non_weight_post_comp[counter, :]
        )
        # This is to get the fidelity of the current item/trial from study to post (unweighted)
        study_post_trial_uw_fidelity = np.corrcoef(
            non_weight_study_comp[counter, :], non_weight_post_comp[counter, :]
        )
        # This is to get the fidelity of the current item/trial from pre to study (unweighted)
        pre_study_trial_uw_fidelity = np.corrcoef(
            non_weight_pre_comp[counter, :], non_weight_study_comp[counter, :]
        )
        ...
        if brain_flag == "MNI":
            ...
            study_post_trial_cw_fidelity = np.corrcoef(
                category_weight_study_comp[counter, :],
                category_weight_post_comp[counter, :],
            )
            # This is to get the fidelity of the current item/trial from pre to study (scene-weighted)
            pre_study_trial_cw_fidelity = np.corrcoef(
                category_weight_pre_comp[counter, :],
                category_weight_study_comp[counter, :],
            )
        ...
        if image_condition == 1:
            ...
            ] = pre_post_trial_iw_fidelity[1][0]
            change_uw_maintain_dict[
                "image ID: %s" % study_image_id
            ] = pre_post_trial_uw_fidelity[1][0]

            modify_iw_maintain_dict[
                "image ID: %s" % study_image_id
            ] = study_post_trial_iw_fidelity[1][0]
            removal_iw_maintain_dict[
                "image ID: %s" % study_image_id
            ] = removal_post_trial_iw_fidelity[0][1:]
            preremoval_iw_maintain_dict[
                "image ID: %s" % study_image_id
            ] = pre_removal_trial_iw_fidelity[0][1:]

            modify_uw_maintain_dict[
                "image ID: %s" % study_image_id
            ] = study_post_trial_uw_fidelity[1][0]
            if brain_flag == "MNI":
                modify_cw_maintain_dict[
                    "image ID: %s" % study_image_id
                ] = study_post_trial_cw_fidelity[1][0]
                change_cw_maintain_dict[
                    "image ID: %s" % study_image_id
                ] = pre_post_trial_cw_fidelity[1][0]
                cw_maintain_dict[
                    "image ID: %s" % study_image_id
                ] = pre_study_trial_cw_fidelity[1][0]
            ...
            uw_maintain_dict[
                "image ID: %s" % study_image_id
            ] = pre_study_trial_uw_fidelity[1][0]
    ...
    for trial in pre_exp_df["trial_id"].values:
        ...
        post_trial_index = post_scene_order.index[
            post_scene_order["image_id"] == pre_img_id
        ].tolist()[0]
        post_trial_num = post_scene_order.loc[post_trial_index, "trial_id"]
        # this mean we now know both the trial #, the image id and we can also grab the condition to help sort
        ...
        item_preexp_post_comp[counter] = np.multiply(
            masked_bolds_arr_3[post_trial_num - 1, :], masked_weights_arr[trial - 1, :]
        )
        ...
        change_iw_preexp_dict["image ID: %s" % pre_img_id] = pre_post_trial_iw_fidelity[
            1
        ][0]
        change_uw_preexp_dict["image ID: %s" % pre_img_id] = pre_post_trial_uw_fidelity[
            1
        ][0]
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "unweighted_pre_post_RSA.csv",
        )
    )
    del temp_df

    unweighted_study_post_comp = np.corrcoef(
        non_weight_study_comp, non_weight_post_comp
    )
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "scene_weight_pre_only_RSA.csv",
        )
    )
    del temp_df

    category_pre_study_comp = np.corrcoef(
        category_weight_pre_comp, category_weight_study_comp
    )
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "scene_weight_pre_study_RSA.csv",
        )
    )
    del temp_df
    ...
    category_study_post_comp = np.corrcoef(
        category_weight_study_comp, category_weight_post_comp
    )
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "scene_weight_study_post_RSA.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "item_weight_study_post_RSA.csv",
        )
    )
    del temp_df
    ...
    for i in item_repress_removal_comp.keys():
        item_removal_post_comp[i] = np.corrcoef(
            item_repress_removal_comp[i], item_repress_post_comp[i]
        )[0][1:]
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "item_weight_removal_post_RSA.csv",
        )
    )
    del temp_df
    ...
    for i in item_repress_removal_comp.keys():
        item_pre_removal_comp[i] = np.corrcoef(
            item_repress_pre_comp[i], item_repress_removal_comp[i]
        )[0][1:]
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "item_weight_pre_removal_RSA.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "preexp_item_weight_pre_post_RSA.csv",
        )
    )
    del temp_df

    unweighted_pre_post_preexp_comp = np.corrcoef(
        non_weight_preexp_pre_comp, non_weight_preexp_post_comp
    )
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "preexp_unweighted_pre_post_RSA.csv",
        )
    )
    del temp_df
    ...
    if not os.path.exists(
        os.path.join(container_path, "sub-0%s" % subID, "RSA_%s" % brain_flag)
    ):
        os.makedirs(
            os.path.join(container_path, "sub-0%s" % subID, "RSA_%s" % brain_flag),
            exist_ok=True,
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "RSA_%s" % brain_flag,
            "scene_weighted_pre_vs_study.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "RSA_%s" % brain_flag,
            "scene_weighted_pre_vs_post.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "RSA_%s" % brain_flag,
            "scene_weighted_study_vs_post.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "RSA_%s" % brain_flag,
            "item_weighted_pre_vs_study.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "RSA_%s" % brain_flag,
            "item_weighted_study_vs_post.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "RSA_%s" % brain_flag,
            "preexp_item_weighted_pre_vs_post.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "RSA_%s" % brain_flag,
            "preexp_unweighted_pre_vs_post.png",
        )
    )
    plt.clf()
    ...
    itemw_pre_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "itemweighted_pre_post_fidelity.csv",
        )
    )
    ...
    unw_pre_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "unweighted_pre_post_fidelity.csv",
        )
    )

    itemw_preremoval_df_m = pd.DataFrame(
        data=np.array(list(preremoval_iw_maintain_dict.values()))
    )
    itemw_preremoval_df_m["operation"] = np.repeat("maintain", 30)
    itemw_preremoval_df_r = pd.DataFrame(
        data=np.array(list(preremoval_iw_replace_dict.values()))
    )
    itemw_preremoval_df_r["operation"] = np.repeat("replace", 30)
    itemw_preremoval_df_s = pd.DataFrame(
        data=np.array(list(preremoval_iw_suppress_dict.values()))
    )
    itemw_preremoval_df_s["operation"] = np.repeat("suppress", 30)
    itemw_preremoval_df = pd.DataFrame()
    itemw_preremoval_df = itemw_preremoval_df.append(
        itemw_preremoval_df_m, ignore_index=True
    )
    itemw_preremoval_df = itemw_preremoval_df.append(
        itemw_preremoval_df_r, ignore_index=True
    )
    itemw_preremoval_df = itemw_preremoval_df.append(
        itemw_preremoval_df_s, ignore_index=True
    )
    itemw_preremoval_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "itemweighted_pre_removal_fidelity.csv",
        )
    )

    itemw_remembered_df = pd.DataFrame(
        columns=["maintain", "replace", "suppress", "preexp"], index=range(0, 30)
    )
    itemw_forgot_df = pd.DataFrame(
        columns=["maintain", "replace", "suppress", "preexp"], index=range(0, 30)
    )

    itemw_study_post_remember_df = pd.DataFrame(
        columns=["maintain", "replace", "suppress"], index=range(0, 30)
    )
    itemw_study_post_forgot_df = pd.DataFrame(
        columns=["maintain", "replace", "suppress"], index=range(0, 30)
    )

    operation_list = np.repeat(["maintain", "replace", "suppress"], 30)

    itemw_removal_post_forgot_df = pd.DataFrame(
        index=range(0, 90), columns=[0, 1, 2, 3, 4, 5, 6, 7, 8, "operation"]
    )

    itemw_removal_post_remember_df = pd.DataFrame(
        index=range(0, 90), columns=[0, 1, 2, 3, 4, 5, 6, 7, 8, "operation"]
    )

    itemw_pre_removal_remember_df = pd.DataFrame(
        index=range(0, 90), columns=[0, 1, 2, 3, 4, 5, 6, 7, 8, "operation"]
    )

    itemw_pre_removal_forgot_df = pd.DataFrame(
        index=range(0, 90), columns=[0, 1, 2, 3, 4, 5, 6, 7, 8, "operation"]
    )
    ...
    for img_key in change_iw_maintain_dict.keys():
        ...
        if (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 1:
            itemw_remembered_df.loc[indexer_r]["maintain"] = change_iw_maintain_dict[
                "image ID: %s" % img_num
            ]
            itemw_study_post_remember_df.loc[indexer_f][
                "maintain"
            ] = modify_iw_maintain_dict["image ID: %s" % img_num]
            for i in range(0, 9):
                itemw_removal_post_remember_df.loc[indexer_r_agg][
                    i
                ] = removal_iw_maintain_dict["image ID: %s" % img_num][i]
                itemw_pre_removal_remember_df.loc[indexer_r_agg][
                    i
                ] = preremoval_iw_maintain_dict["image ID: %s" % img_num][i]
            ...
            itemw_forgot_df.loc[indexer_f]["maintain"] = change_iw_maintain_dict[
                "image ID: %s" % img_num
            ]
            itemw_study_post_forgot_df.loc[indexer_f][
                "maintain"
            ] = modify_iw_maintain_dict["image ID: %s" % img_num]
            for i in range(0, 9):
                itemw_removal_post_forgot_df.loc[indexer_f_agg][
                    i
                ] = removal_iw_maintain_dict["image ID: %s" % img_num][i]
                itemw_pre_removal_forgot_df.loc[indexer_f_agg][
                    i
                ] = preremoval_iw_maintain_dict["image ID: %s" % img_num][i]
    ...
    for img_key in change_iw_replace_dict.keys():
        ...
        if (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 1:
            itemw_remembered_df.loc[indexer_r]["replace"] = change_iw_replace_dict[
                "image ID: %s" % img_num
            ]
            itemw_study_post_remember_df.loc[indexer_f][
                "replace"
            ] = modify_iw_replace_dict["image ID: %s" % img_num]
            for i in range(0, 9):
                itemw_removal_post_remember_df.loc[indexer_r_agg][
                    i
                ] = removal_iw_replace_dict["image ID: %s" % img_num][i]
                itemw_pre_removal_remember_df.loc[indexer_r_agg][
                    i
                ] = preremoval_iw_replace_dict["image ID: %s" % img_num][i]
            ...
            itemw_forgot_df.loc[indexer_f]["replace"] = change_iw_replace_dict[
                "image ID: %s" % img_num
            ]
            itemw_study_post_forgot_df.loc[indexer_f][
                "replace"
    ...
    for img_key in change_iw_suppress_dict.keys():
        ...
        if (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 1:
            itemw_remembered_df.loc[indexer_r]["suppress"] = change_iw_suppress_dict[
                "image ID: %s" % img_num
            ]
            itemw_study_post_remember_df.loc[indexer_f][
                "suppress"
            ] = modify_iw_suppress_dict["image ID: %s" % img_num]
            for i in range(0, 9):
                itemw_removal_post_remember_df.loc[indexer_r_agg][
                    i
                ] = removal_iw_suppress_dict["image ID: %s" % img_num][i]
                itemw_pre_removal_remember_df.loc[indexer_r_agg][
                    i
                ] = preremoval_iw_suppress_dict["image ID: %s" % img_num][i]
            ...
            itemw_forgot_df.loc[indexer_f]["suppress"] = change_iw_suppress_dict[
                "image ID: %s" % img_num
            ]
            itemw_study_post_forgot_df.loc[indexer_f][
                "suppress"
    ...
    for img_key in change_iw_preexp_dict.keys():
        ...
        if (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 1:
            itemw_remembered_df.loc[indexer_r]["preexp"] = change_iw_preexp_dict[
                "image ID: %s" % img_num
            ]
            indexer_r = indexer_r + 1

        elif (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 0:
            itemw_forgot_df.loc[indexer_f]["preexp"] = change_iw_preexp_dict[
                "image ID: %s" % img_num
            ]
            indexer_f = indexer_f + 1
    ...
    for img_key in change_uw_maintain_dict.keys():
        ...
        if (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 1:
            uw_remembered_df.loc[indexer_r]["maintain"] = change_uw_maintain_dict[
                "image ID: %s" % img_num
            ]
            indexer_r = indexer_r + 1

        elif (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 0:
            uw_forgot_df.loc[indexer_f]["maintain"] = change_uw_maintain_dict[
                "image ID: %s" % img_num
            ]
            indexer_f = indexer_f + 1
    ...
    for img_key in change_uw_replace_dict.keys():
        ...
        if (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 1:
            uw_remembered_df.loc[indexer_r]["replace"] = change_uw_replace_dict[
                "image ID: %s" % img_num
            ]
            indexer_r = indexer_r + 1

        elif (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 0:
            uw_forgot_df.loc[indexer_f]["replace"] = change_uw_replace_dict[
                "image ID: %s" % img_num
            ]
            indexer_f = indexer_f + 1
    ...
    for img_key in change_uw_suppress_dict.keys():
        ...
        if (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 1:
            uw_remembered_df.loc[indexer_r]["suppress"] = change_uw_suppress_dict[
                "image ID: %s" % img_num
            ]
            indexer_r = indexer_r + 1

        elif (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 0:
            uw_forgot_df.loc[indexer_f]["suppress"] = change_uw_suppress_dict[
                "image ID: %s" % img_num
            ]
            indexer_f = indexer_f + 1
    ...
    for img_key in change_uw_preexp_dict.keys():
        ...
        if (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 1:
            uw_remembered_df.loc[indexer_r]["preexp"] = change_uw_preexp_dict[
                "image ID: %s" % img_num
            ]
            indexer_r = indexer_r + 1

        elif (memory_csv["memory"][memory_csv["image_num"] == img_num]).values[0] == 0:
            uw_forgot_df.loc[indexer_f]["preexp"] = change_uw_preexp_dict[
                "image ID: %s" % img_num
            ]
            indexer_f = indexer_f + 1
    ...
    itemw_remembered_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "itemweighted_pre_post_remember_fidelity.csv",
        )
    )
    itemw_forgot_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "itemweighted_pre_post_forgot_fidelity.csv",
        )
    )
    uw_remembered_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "unweighted_pre_post_remember_fidelity.csv",
        )
    )
    uw_forgot_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "unweighted_pre_post_forgot_fidelity.csv",
        )
    )

    itemw_study_post_forgot_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "itemweighted_study_post_forgot_fidelity.csv",
        )
    )
    itemw_study_post_remember_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "itemweighted_study_post_remember_fidelity.csv",
        )
    )

    itemw_removal_post_forgot_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "itemweighted_removal_post_forgot_fidelity.csv",
        )
    )
    itemw_removal_post_remember_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "itemweighted_removal_post_remembered_fidelity.csv",
        )
    )

    itemw_pre_removal_forgot_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "itemweighted_pre_removal_forgot_fidelity.csv",
        )
    )
    itemw_pre_removal_remember_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "itemweighted_pre_removal_remembered_fidelity.csv",
        )
    )
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Item_Weighted_forgot_only_study_post_summary.png",
        )
    )
    plt.clf()
    ...
    fig = sns.lineplot(
        data=itemw_removal_post_forgot_df, x="variable", y="value", hue="operation"
    )
    fig.set_xlabel("TR")
    fig.set_ylabel("Fidelity")
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Item_Weighted_forgot_only_removal_post_summary_timecourse.png",
        )
    )
    plt.clf()
    ...
    itemw_removal_post_remember_df = itemw_removal_post_remember_df.astype(
        {"value": float}
    )

    fig = sns.lineplot(
        data=itemw_removal_post_remember_df, x="variable", y="value", hue="operation"
    )
    fig.set_xlabel("TR")
    fig.set_ylabel("Fidelity")
    fig.set_ylim([-0.2, 0.2])
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Item_Weighted_remember_only_removal_post_summary_timecourse.png",
        )
    )
    plt.clf()
    ...
    fig = sns.lineplot(
        data=itemw_pre_removal_forgot_df, x="variable", y="value", hue="operation"
    )
    fig.set_xlabel("TR")
    fig.set_ylabel("Fidelity")
    fig.set_ylim([-0.2, 0.2])
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Item_Weighted_forgot_only_pre_removal_summary_timecourse.png",
        )
    )
    plt.clf()
    ...
    itemw_pre_removal_remember_df = itemw_pre_removal_remember_df.astype(
        {"value": float}
    )

    fig = sns.lineplot(
        data=itemw_pre_removal_remember_df, x="variable", y="value", hue="operation"
    )
    fig.set_xlabel("TR")
    fig.set_ylabel("Fidelity")
    fig.set_ylim([-0.2, 0.2])
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Item_Weighted_remember_only_pre_removal_summary_timecourse.png",
        )
    )
    plt.clf()
    ...
    itemw_study_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "itemweighted_study_post_fidelity.csv",
        )
    )
    ...
    catew_study_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "sceneweighted_study_post_fidelity.csv",
        )
    )
    ...
    unw_study_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "unweighted_study_post_fidelity.csv",
        )
    )
    ...
    itemw_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "itemweighted_pre_study_fidelity.csv",
        )
    )
    ...
    catew_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "sceneweighted_pre_study_fidelity.csv",
        )
    )
    ...
    unw_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "unweighted_pre_study_fidelity.csv",
        )
    )
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Item_Weighted_pre_post_summary.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Scene_Weighted_pre_post_summary.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Unweighted_pre_post_summary.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Item_Weighted_remember_only_pre_post_summary.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Unweighted_remember_only_pre_post_summary.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Item_Weighted_forgot_only_pre_post_summary.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Unweighted_forgot_only_pre_post_summary.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Item_Weighted_study_post_summary.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Scene_Weighted_study_post_summary.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Unweighted_study_post_summary.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Item_Weighted_pre_study_summary.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Scene_Weighted_pre_study_summary.png",
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Representational_Changes_%s" % brain_flag,
            "Unweighted_pre_study_summary.png",
        )
    )
    plt.clf()
...
for subID in subs:
    ...
    group_item_weighted_pre_post = group_item_weighted_pre_post.append(
        pd.read_csv(item_pre_post, usecols=[1, 2, 3, 4]), ignore_index=True
    )
    ...
    item_r_pre_post = os.path.join(
        data_path, "itemweighted_pre_post_remember_fidelity.csv"
    )
    group_item_remember_pre_post = group_item_remember_pre_post.append(
        pd.read_csv(item_r_pre_post, usecols=[1, 2, 3]), ignore_index=True
    )  # add column 4 if we want pre-exposed

    item_f_pre_post = os.path.join(
        data_path, "itemweighted_pre_post_forgot_fidelity.csv"
    )
    group_item_forgot_pre_post = group_item_forgot_pre_post.append(
        pd.read_csv(item_f_pre_post, usecols=[1, 2, 3]), ignore_index=True
    )
    ...
    group_by_sub_item_remember_pre_post = group_by_sub_item_remember_pre_post.append(
        pd.read_csv(item_r_pre_post, usecols=[1, 2, 3]).mean(), ignore_index=True
    )

    group_by_sub_item_forgot_pre_post = group_by_sub_item_forgot_pre_post.append(
        pd.read_csv(item_f_pre_post, usecols=[1, 2, 3]).mean(), ignore_index=True
    )
...
# plot and save the figures of the data - Pre vs Post
fig = sns.barplot(
    data=group_item_weighted_pre_post, ci=95, palette=["green", "blue", "red", "gray"]
)
fig.set_xlabel("Operations")
fig.set_ylabel("Fidelity")
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Representational_Changes_%s" % brain_flag,
        "Group_Item_Weighted_pre_post_summary.png",
    )
)
plt.clf()

fig = sns.barplot(
    data=group_item_remember_pre_post, ci=95, palette=["green", "blue", "red", "gray"]
)
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Representational_Changes_%s" % brain_flag,
        "Group_Item_Weighted_pre_post_remembered_summary.png",
    )
)
plt.clf()

fig = sns.barplot(
    data=group_item_forgot_pre_post, ci=95, palette=["green", "blue", "red", "gray"]
)
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Representational_Changes_%s" % brain_flag,
        "Group_Item_Weighted_pre_post_forgot_summary.png",
    )
)
plt.clf()

#####
# I am creating a temp dataframe manually called plotting_df
...
fig.set_title("Item Weighted - Pre vs. Post RSA", loc="center", wrap=True)
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Representational_Changes_%s" % brain_flag,
        "Group_Item_Weighted_pre_post_summary.svg",
    )
)
plt.clf()
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Representational_Changes_%s" % brain_flag,
        "Allsubs_Item_Weighted_pre_post_summary.svg",
    )
)
plt.clf()
