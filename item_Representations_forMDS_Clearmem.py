...
def item_representation_extract(subID):
    ...
    events['onset']=(events['onsets_volume']-1) #we need to be in TR timing and not seconds
    ...
    for image_id in np.unique(events['trial_type'].values):
        if image_id>0:
            ...
            trial_pattern=np.mean(img_clean.get_fdata()[:,:,:,(onset):(onset+3)],axis=3) #this is getting the 3 TRs for that trial's onset and then taking the average of it across the 4th dimension (time)
            

            output_name = os.path.join(out_folder, ('Sub-0%s_localizer_image%s_result.nii.gz' % (subID,(image_id))))
