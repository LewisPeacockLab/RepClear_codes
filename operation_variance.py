...
def organize_evidence(subID, space, task, save=True):
    ...
    all_operations = pd.concat(
        [all_maintain, all_replace, all_suppress]
    )  # combine all these into 1 dataframe to get the subject level z-score
    ...
    operation_var["Maintain"] = np.var(
        all_operations[all_operations["operation"] == "Maintain"]["evidence"]
    )
    operation_var["Replace"] = np.var(
        all_operations[all_operations["operation"] == "Replace"]["evidence"]
    )
    operation_var["Suppress"] = np.var(
        all_operations[all_operations["operation"] == "Suppress"]["evidence"]
    )
...
ax = sns.violinplot(
    data=combined_df, x="operation", y="evidence", palette=["green", "blue", "red"]
)
...
t_stat_maintain_replace, p_value_maintain_replace = ttest_rel(
    combined_var["Maintain"], combined_var["Replace"]
)
t_stat_maintain_suppress, p_value_maintain_suppress = ttest_rel(
    combined_var["Maintain"], combined_var["Suppress"]
)
t_stat_replace_suppress, p_value_replace_suppress = ttest_rel(
    combined_var["Replace"], combined_var["Suppress"]
)

# Print or save the t-statistics and p-values to a file or console
print(
    "Maintain vs Replace: t-statistic =",
    t_stat_maintain_replace,
    ", p-value =",
    p_value_maintain_replace,
)
print(
    "Maintain vs Suppress: t-statistic =",
    t_stat_maintain_suppress,
    ", p-value =",
    p_value_maintain_suppress,
)
print(
    "Replace vs Suppress: t-statistic =",
    t_stat_replace_suppress,
    ", p-value =",
    p_value_replace_suppress,
)
...
F_stat, P_value = f_oneway(
    temp_maintain, temp_replace, temp_suppress
)  # compare all groups

F_stat_maintain_replace, P_value_maintain_replace = f_oneway(
    temp_maintain, temp_replace
)  # compare maintain vs. replace
F_stat_maintain_suppress, P_value_maintain_suppress = f_oneway(
    temp_maintain, temp_suppress
)  # compare maintain vs. suppress
F_stat_replace_suppress, P_value_replace_suppress = f_oneway(
    temp_replace, temp_suppress
)
...
for i in range(iterations):
    ...
    results[i] = {
        "Maintain vs. Replace": ttest_ind(bootstrap_m_var, bootstrap_r_var),
        "Maintain vs. Suppress": ttest_ind(bootstrap_m_var, bootstrap_s_var),
        "Replace vs. Suppress": ttest_ind(bootstrap_r_var, bootstrap_s_var),
    }
...
plt.tight_layout()
plt.savefig(
    os.path.join(data_dir, "figs", "All_operation_trials_bootstrap_variance.svg")
)
plt.savefig(
    os.path.join(data_dir, "figs", "All_operation_trials_bootstrap_variance.png")
)
plt.clf()
