...
if steps["ph1_loading"]:
    ...
    logs(
        " | masked voxels: %s, n_trs: %s" % (str(xpats.shape[1]), str(xpats.shape[0])),
        xlog=xlog,
    )
...
if steps["ph2_feature_selection"]:
    ...
    for xsubject_id in subject_lists:
        print("#########################")
        print("separation %d / %d, subject: %s" % (xsep, xargs["n_seps"], xsubject_id))
    ...
    ######### load regressor
    xcat_reg = pd.read_pickle(
        os.path.join(xdirs["out"], "cat_regs_n%d.pkl" % xargs["n_subj"])
    )
    ...
    xf = os.path.join(
        xdirs["fmap"], "tmp_feat_fmap_sep%d_n%d.pkl" % (xsep, xargs["n_subj"])
    )
...
if steps["ph3_merging_data"]:
    ...
    if xmerge_fmap:
        ...
        for xsep in range(xargs["n_seps"]):
            xf = os.path.join(
                xdirs["fmap"],
                "tmp_feat_fmap_sep%d_n%d.pkl" % (xsep + 1, xargs["n_subj"]),
            )
    ...
    xthres_F = feat_sels["fmap"][np.where(feat_sels["pmap"] == xthres_p)[0]].values[0][
        0
    ]

    xfeat = xP < xthres_p
    logs(
        " | top %d voxels were selected P < %0.4f"
        % (np.where(xfeat == True)[0].shape[0], xthres_p),
        xlog,
    )
    ...
    for xsubject_id in subject_lists:
        ...
        logs(
            "TRs = %d, features = %d out of %d" % (n_trs, n_selvoxs, n_voxs), xlog=xlog
        )
    ...
    logs(
        " | whole brain mask: %d voxels"
        % np.where(nib.load(xmask).get_fdata() > 0)[0].shape[0],
        xlog=xlog,
    )
...
if xprepare_test:
    ...
    xtrain_pat = np.load(
        "/scratch/06873/zbretton/repclear_dataset/BIDS/derivatives/MVPA_cross_subjects/out/cat_pat_n50.npy"
    )

    xtrain_reg = pd.read_pickle(
        "/scratch/06873/zbretton/repclear_dataset/BIDS/derivatives/MVPA_cross_subjects/out/cat_regs_n50.pkl"
    )
...
xtrain_trim, xcat_regs_trim = remove_label(
    xtrain_pat, xcat_regs, 5
)  # this will drop the "clear" labels
...
classifier = LogisticRegression(
    penalty="l2", solver="lbfgs", max_iter=1000, multi_class="ovr", n_jobs=-1, verbose=1
)
