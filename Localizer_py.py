...
for TR_shift in TR_shifts:
    for mask_flag in masks:
        for num in range(len(subs)):
            ...
            run_list_stims_and_rest=run_list_stims_and_rest[(run_list_stims_and_rest<=2)|(run_list_stims_and_rest>=5)]
            ...
            group_even_stims_and_rest=np.full(int(run_list_stims_and_rest.size/4),2)
            ...
            groups_stims_and_rest=np.hstack((group_even_stims_and_rest,group_odd_stims_and_rest,group_even_stims_and_rest,group_odd_stims_and_rest))        
            ...
            groups_on=np.hstack((group_even_on,group_odd_on,group_even_on,group_odd_on)) 
        ...
        #Need to run a randomized classifier to test for validity
            ...
            for i in range(n_iters):
                ...
                for train, test in sp.split(localizer_bold_stims_and_rest,stim_on_rest,groups_stims_and_rest):
                    ...
                    selected_voxels=SelectFpr(f_classif,alpha=0.05).fit(train_data,permuted_labels[train]) #I compared this method to taking ALL k items in the F-test and filtering by p-value, so I assume this is a better feature selection method
            
                    # Train and test the classifier
                    classifier = LogisticRegression(penalty='l2',solver='liblinear',C=1)
            ...
            if rest=='off':
                print('L2 score - Only stimuli on: %s - Cost: %s - ROC/AUC: %s' % (L2_scores,L2_costs, L2_sig_scores))
            elif rest=='on':
                print('L2 With Rest Score: %s - Cost: %s - ROC/AUC: %s' % (L2_scores_on_rest, L2_costs_on_rest, L2_sig_scores_on_rest))
            print('----------------------------------')
            print('saving data...')            
