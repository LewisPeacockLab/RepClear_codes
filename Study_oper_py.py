        ...
        for num in range(len(subs)):
            ...
            if clear_data == 0:
                if os.path.exists(
                    os.path.join(
                        bold_path,
                        "sub-0%s_%s_study_%s_masked.npy"
                    ...
                    if mask_flag == "wholebrain":
                        study_run1 = apply_mask(
                            ...
                            target=(study_run1.get_data()),
                        )
                        study_run2 = apply_mask(
                            mask=(wholebrain_mask1.get_data()),
                            target=(study_run2.get_data()),
                        )
                        study_run3 = apply_mask(
                            mask=(wholebrain_mask1.get_data()),
                            target=(study_run3.get_data()),
                        )
                    ...
                    preproc_1 = clean(
                        study_run1, t_r=1, detrend=False, standardize="zscore"
                    )
                    preproc_2 = clean(
                        study_run2, t_r=1, detrend=False, standardize="zscore"
                    )
                    preproc_3 = clean(
                        study_run3, t_r=1, detrend=False, standardize="zscore"
                    )
                ...
                if os.path.exists(
                    ...
                    os.chdir(bold_path)
                    np.save(
                        "sub-0%s_%s_study_%s_masked_cleaned"
                        % (sub_num, brain_flag, mask_flag),
                        study_bold,
                    )
                    print(
                        "%s %s masked & cleaned data...saved" % (mask_flag, brain_flag)
                    )
            ...
            def L2_xval(data, labels, run_labels):
                ...
                for train_index, test_index in ps.split():
                    ...
                    L2 = LogisticRegression(
                        penalty="l2", C=C_best_i, solver="liblinear"
                    )
            ...
            (
                ...
                L2_chosenvoxels_nr,
                L2_sig_scores_nr,
            ) = L2_xval(study_bold_nr, stim_list_nr, run_list_nr)
