...
for TR_shift in TR_shifts:
    ...
    #3. suppress
        ...
        #now we need to use this normalized data to run the decoder
            ...
            L2_models_nr, L2_scores_nr, L2_predicts_nr, L2_trues_nr, L2_evidence_nr, L2_costs_nr, L2_chosenvoxels_nr, L2_sig_scores_nr = L2_xval(study_bold_nr, stim_list_nr, run_list_nr)
        ...
        #Need to run a randomized classifier to test for validity
            ...
            for i in range(n_iters):
                ...
                for train, test in sp.split():
                    ...
                    clf = classifier.fit(selected_voxels.transform(train_data), permuted_labels[train])
            ...
            print('L2 No Rest Score: %s - Cost: %s - ROC/AUC : %s' % (L2_scores_nr,L2_costs_nr, L2_sig_scores_nr))
        ...
        #need to save an output per subject here
            ...
            output_table = {
                ...
                # "L2 Average Scores" : L2_subject_score_mean,
                # "L2 Model" : L2_models,
                # "L2 Raw Scores" : L2_scores,
                      
                # "L2 Predictions" : L2_predicts,

                # "L2 True" : L2_trues,
                # "L2 Costs" : L2_costs,           

                # "Class List" : stim_list_shifted,           
                ...
                # "L2 Operation Average Scores (No Rest)" : L2_subject_score_operation_mean_nr,
                # "L2 Operation Model (No Rest)" : L2_models_operation_nr,
                # "L2 Operation Raw Scores (No Rest)" : L2_scores_operation_nr,
                      
                # "L2 Operation Predictions (No Rest)" : L2_predicts_operation_nr,

                # "L2 Operation True (No Rest)" : L2_trues_operation_nr,
                # "L2 Operation Costs (No Rest)" : L2_costs_operation_nr,

                # "Operation List (No Rest)" : oper_list_nr,

                # "L2 Operation Evidence (No Rest)" : L2_evidence_operation_nr, 

                # "Chosen voxels": L2_chosenvoxels,
                "Chosen voxels (No Rest)": L2_chosenvoxels_nr,
                "ROC/AUC" : L2_sig_scores_nr,
                # "Operation Chosen voxels (No Rest)": L2_chosenvoxels_operation_nr,

                # "Run List" : run_list,
                # "Run List w/ Rest": run_list_shifted,

                # "study Path" : study_files
                }
            
            import pickle
            if clear_data==0:
                os.chdir(os.path.join(container_path,sub))
                f = open("%s-studyoperation_%s_%s_%sTR lag_data.pkl" % (sub,brain_flag,mask_flag,TR_shift),"wb")
                pickle.dump(output_table,f)
                f.close()
            else:
                os.chdir(os.path.join(container_path,sub))
                f = open("%s-studyoperation_%s_%s_%sTR lag_data_cleaned.pkl" % (sub,brain_flag,mask_flag,TR_shift),"wb")
                pickle.dump(output_table,f)
                f.close()                            
            print('data saved!')
            print('Sub-0%s is now complete' % sub_num)
            print('this subject took:')
