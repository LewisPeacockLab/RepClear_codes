...
for num in range(len(subs)):
    ...
    events = pd.read_csv(
        "/scratch1/06873/zbretton/repclear_dataset/BIDS/task-study_events.tsv", sep="\t"
    )
    ...
    for contrast in contrasts:
        ...
        nib.save(
            thresholded_map,
            os.path.join(
                out_folder, f"{contrast}_{brain_flag}_thresholded_zmap.nii.gz"
            ),
        )
...
for contrast in contrasts:
    ...
    second_level_model = SecondLevelModel(
        smoothing_fwhm=None, mask_img=gm_mask_path[0], verbose=2, n_jobs=-1
    )
