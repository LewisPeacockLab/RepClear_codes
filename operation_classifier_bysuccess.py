...
def get_shifted_labels(task, shift_size_TR, rest_tag=0):
    ...
    )  # reset the index to use in the following loop
    ...
    for i in range(len(shifted_df)):
        ...
        if temp_image_id == 0:
            ...
            temp_memory = sub_memory_matrix[
                sub_memory_matrix["image_num"] == temp_image_id
            ]["memory"].values[0]
        shifted_df.loc[
            shifted_df["image_id"] == temp_image_id, "accuracy"
        ] = temp_memory
...
def fit_model(
    X, Y, runs, save=False, out_fname=None, v=False, balance=False, under_sample=False
):
    if v:
        print("\n***** Fitting model...")
    ...
    for train_inds, test_inds in skf.split(X, Y):
        ...
        if balance:
            ...
            lr = LogisticRegression(
                penalty="l2", solver=solver_s, C=best_Cs[-1], max_iter=1000
            )
        ...
        true_counts = np.asarray(
            [
                np.sum(y_test == i)
                for i in ["maintain_r", "maintain_f", "suppress_r", "suppress_f"]
            ]
        )
        cm = (
            confusion_matrix(
...
def sample_for_training(full_data, label_df, include_rest=False):
    ...
    for i in oper_inds:
        if (category_list.values[i] == 1) & (accuracy_list.values[i] == 1):
            ...
            operation_reg.append(
                "replace"
            )  # this means that later we will want to remove all replace's from the data to only look at maintain / suppress
...
def classification(subID):
    ...
    (
        ...
        y_scores,
        tested_inds,
    ) = fit_model(X, Y, runs, save=False, v=True, balance=False, under_sample=True)
    ...
    auc_df = pd.DataFrame(
        columns=["AUC", "Content", "Sub"],
        index=["Maintain_R", "Maintain_F", "Suppress_R", "Suppress_F"],
    )
    auc_df.loc["Maintain_R"]["AUC"] = roc_auc.loc[
        :, 0
    ...
    out_fname_template_auc = (
        f"sub-{subID}_{space}_{task}_{ROIs[0]}_operation_memoryoutcome_auc.csv"
    )
    ...
    evidence_df = pd.DataFrame(
        columns=["index", "runs", "operation", "image_id"]
    ...
    evidence_df["maintain_R_evi"] = np.vstack(evidence)[
        :, 0
    ]  # add in the evidence values for maintain_R
    evidence_df["maintain_F_evi"] = np.vstack(evidence)[
        :, 1
    ]  # add in the evidence values for maintain_F
    evidence_df["suppress_R_evi"] = np.vstack(evidence)[
        :, 2
    ]  # add in the evidence values for suppress_R
    evidence_df["suppress_F_evi"] = np.vstack(evidence)[
        :, 3
    ]  # add in the evidence values for suppress_F
    ...
    sub_dir = os.path.join(data_dir, f"sub-{subID}")
    out_fname_template = (
        f"sub-{subID}_{space}_{task}_{ROIs[0]}_operation_memoryoutcome_evidence.csv"
    )
    print("\n *** Saving evidence values with subject dataframe ***")
    evidence_df.to_csv(os.path.join(sub_dir, out_fname_template))

    sub_dir = os.path.join(data_dir, f"sub-{subID}")
    out_fname_template_cm = (
        f"sub-{subID}_{space}_{task}_{ROIs[0]}_operation_memoryoutcome_cm.csv"
    )
...
def binary_classification(subID, condition):
    ...
    (
        ...
        X, Y, runs, save=False, v=True, balance=False, under_sample=True
    )
    ...
    sub_dir = os.path.join(data_dir, f"sub-{subID}")
    out_fname_template_auc = (
        f"sub-{subID}_{space}_{task}_{ROIs[0]}_{condition}binary_memoryoutcome_auc.csv"
    )
    print("\n *** Saving AUC values with subject dataframe ***")
    auc_df.to_csv(os.path.join(sub_dir, out_fname_template_auc))
    ...
    evidence_df["operation"] = combined_tested_labels
    evidence_df["image_id"] = imgs
    evidence_df[f"{condition}_R_evi"] = np.vstack(
        evidences
    )  # add in the evidence values for suppress_R
    evidence_df[f"{condition}_F_evi"] = np.vstack(
        1 - evidences
    )  # add in the evidence values for suppress_F
    ...
    sub_dir = os.path.join(data_dir, f"sub-{subID}")
    out_fname_template_cm = (
        f"sub-{subID}_{space}_{task}_{ROIs[0]}_{condition}binary_memoryoutcome_cm.csv"
    )
    print("\n *** Saving confusion matrix with subject dataframe ***")
    cm_df = pd.DataFrame(data=cm.mean(axis=0))
    cm_df.to_csv(os.path.join(sub_dir, out_fname_template_cm))
...
def fit_binary_model(
    ...
    for train_inds, test_inds in skf.split(X, Y):
        ...
        if balance:
            print("balancing classes via class-weights")
            lr = LinearSVC(
                penalty="l2", C=best_Cs[-1], max_iter=1000, class_weight="balanced"
            )
        else:
        ...
        train_fpr, train_tpr, tr_thresholds = roc_curve(
            y_res, y_train_pred, pos_label=f"{condition}_r"
        )
        test_fpr, test_tpr, te_thresholds = roc_curve(
            y_test, y_test_pred, pos_label=f"{condition}_r"
        )
        ...
        true_counts = np.asarray(
            [np.sum(y_test == i) for i in [f"{condition}_r", f"{condition}_f"]]
        )
        cm = (
            confusion_matrix(y_test, preds, labels=[f"{condition}_r", f"{condition}_f"])
            / true_counts[:, None]
            * 100
        )
...
def organize_cms(space, task, ROI, save=True):
    ...
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_{ROIs}_operation_success_decoding.png",
        )
    )
    plt.clf()
...
def organize_binary_cms(condition, space, task, save=True):
    ...
    for subID in subIDs:
        try:
            ...
            sub_dir = os.path.join(data_dir, f"sub-{subID}")
            in_fname_template = (
                f"sub-{subID}_{space}_{task}_{condition}binary_memoryoutcome_cm.csv"
            )
    ...
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_{task}_{condition}binary_memoryoutcome_cm.png",
        )
    )
    plt.clf()
...
def organize_evidence(subID, space, task, ROI, save=True):
    ...
    sub_condition_list = sub_df["operation"][
        sub_index
    ].values  # so using the above indices, we will now grab what the operation is on each image
    ...
    for i in sub_condition_list:
        if i == "maintain_r":
            ...
            maintain_r_trials[temp_image] = sub_df[
                ["maintain_R_evi", "maintain_F_evi", "suppress_R_evi", "suppress_F_evi"]
            ][sub_index[counter] : sub_index[counter] + x].values
            maintain_r_trials_diff[temp_image] = [
                maintain_r_trials[temp_image][:, 1]
                - maintain_r_trials[temp_image][:, 0]
            ]
            counter += 1

        elif i == "maintain_f":
            temp_image = sub_images[counter]
            maintain_f_trials[temp_image] = sub_df[
                ["maintain_R_evi", "maintain_F_evi", "suppress_R_evi", "suppress_F_evi"]
            ][sub_index[counter] : sub_index[counter] + x].values
            maintain_f_trials_diff[temp_image] = [
                maintain_f_trials[temp_image][:, 1]
                - maintain_f_trials[temp_image][:, 0]
            ]
            counter += 1

        elif i == "suppress_r":
            temp_image = sub_images[counter]
            suppress_r_trials[temp_image] = sub_df[
                ["maintain_R_evi", "maintain_F_evi", "suppress_R_evi", "suppress_F_evi"]
            ][sub_index[counter] : sub_index[counter] + x].values
            suppress_r_trials_diff[temp_image] = [
                suppress_r_trials[temp_image][:, 3]
                - suppress_r_trials[temp_image][:, 2]
            ]
            counter += 1

        elif i == "suppress_f":
            temp_image = sub_images[counter]
            suppress_f_trials[temp_image] = sub_df[
                ["maintain_R_evi", "maintain_F_evi", "suppress_R_evi", "suppress_F_evi"]
            ][sub_index[counter] : sub_index[counter] + x].values
            suppress_f_trials_diff[temp_image] = [
                suppress_f_trials[temp_image][:, 3]
                - suppress_f_trials[temp_image][:, 2]
            ]
            counter += 1

    # now that the trials are sorted, we need to get the subject average for each condition:
    avg_maintain_r = pd.DataFrame(
        data=np.dstack(maintain_r_trials.values()).mean(axis=2)
    )
    avg_maintain_f = pd.DataFrame(
        data=np.dstack(maintain_f_trials.values()).mean(axis=2)
    )
    avg_maintain_r_diff = pd.DataFrame(
        data=np.dstack(maintain_r_trials_diff.values()).mean(axis=2)
    )
    avg_maintain_f_diff = pd.DataFrame(
        data=np.dstack(maintain_f_trials_diff.values()).mean(axis=2)
    )

    avg_suppress_r = pd.DataFrame(
        data=np.dstack(suppress_r_trials.values()).mean(axis=2)
    )
    avg_suppress_f = pd.DataFrame(
        data=np.dstack(suppress_f_trials.values()).mean(axis=2)
    )
    avg_suppress_r_diff = pd.DataFrame(
        data=np.dstack(suppress_r_trials_diff.values()).mean(axis=2)
    )
    avg_suppress_f_diff = pd.DataFrame(
        data=np.dstack(suppress_f_trials_diff.values()).mean(axis=2)
    )

    # now I will have to change the structure to be able to plot in seaborn:
    avg_maintain_r = (
        avg_maintain_r.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_r["sub"] = np.repeat(
        subID, len(avg_maintain_r)
    )  # input the subject so I can stack melted dfs
    avg_maintain_r["evidence_class"] = np.tile(
        ["maintain_r", "maintain_f", "suppress_r", "suppress_f"], x
    )  # add in the labels so we know what each data point is refering to
    avg_maintain_r.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_r[
        "condition"
    ] = "maintain_r"  # now I want to add in a condition label, since I can then stack all 3 conditions into 1 array per subject

    avg_maintain_f = (
        avg_maintain_f.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_f["sub"] = np.repeat(
        subID, len(avg_maintain_f)
    )  # input the subject so I can stack melted dfs
    avg_maintain_f["evidence_class"] = np.tile(
        ["maintain_r", "maintain_f", "suppress_r", "suppress_f"], x
    )  # add in the labels so we know what each data point is refering to
    avg_maintain_f.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_f[
        "condition"
    ] = "maintain_f"  # now I want to add in a condition label, since I can then stack all 3 conditions into 1 array per subject

    avg_suppress_r = (
        avg_suppress_r.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_r["sub"] = np.repeat(
        subID, len(avg_suppress_r)
    )  # input the subject so I can stack melted dfs
    avg_suppress_r["evidence_class"] = np.tile(
        ["maintain_r", "maintain_f", "suppress_r", "suppress_f"], x
    )  # add in the labels so we know what each data point is refering to
    avg_suppress_r.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_r[
        "condition"
    ] = "suppress_r"  # now I want to add in a condition label, since I can then stack all 3 conditions into 1 array per subject

    avg_suppress_f = (
        avg_suppress_f.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_f["sub"] = np.repeat(
        subID, len(avg_suppress_f)
    )  # input the subject so I can stack melted dfs
    avg_suppress_f["evidence_class"] = np.tile(
        ["maintain_r", "maintain_f", "suppress_r", "suppress_f"], x
    )  # add in the labels so we know what each data point is refering to
    avg_suppress_f.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_f[
        "condition"
    ] = "suppress_f"  # now I want to add in a condition label, since I can then stack all 3 conditions into 1 array per subject

    ### repeat for the difference plots ###
    avg_maintain_r_diff = (
        avg_maintain_r_diff.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_r_diff["sub"] = np.repeat(
        subID, len(avg_maintain_r_diff)
    )  # input the subject so I can stack melted dfs
    avg_maintain_r_diff["evidence_class"] = np.tile(
        ["maintain F-R"], x
    )  # add in the labels so we know what each data point is refering to
    avg_maintain_r_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_r_diff["TR"] = range(0, 9)
    avg_maintain_r_diff[
        "condition"
    ] = "maintain_r_diff"  # now I want to add in a condition label, since I can then stack all 3 conditions into 1 array per subject

    avg_maintain_f_diff = (
        avg_maintain_f_diff.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_maintain_f_diff["sub"] = np.repeat(
        subID, len(avg_maintain_f_diff)
    )  # input the subject so I can stack melted dfs
    avg_maintain_f_diff["evidence_class"] = np.tile(
        ["maintain F-R"], x
    )  # add in the labels so we know what each data point is refering to
    avg_maintain_f_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_f_diff["TR"] = range(0, 9)
    avg_maintain_f_diff[
        "condition"
    ] = "maintain_f_diff"  # now I want to add in a condition label, since I can then stack all 3 conditions into 1 array per subject

    avg_suppress_r_diff = (
        avg_suppress_r_diff.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_r_diff["sub"] = np.repeat(
        subID, len(avg_suppress_r_diff)
    )  # input the subject so I can stack melted dfs
    avg_suppress_r_diff["evidence_class"] = np.tile(
        ["suppress F-R"], x
    )  # add in the labels so we know what each data point is refering to
    avg_suppress_r_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_r_diff["TR"] = range(0, 9)
    avg_suppress_r_diff[
        "condition"
    ] = "suppress_r_diff"  # now I want to add in a condition label, since I can then stack all 3 conditions into 1 array per subject

    avg_suppress_f_diff = (
        avg_suppress_f_diff.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_f_diff["sub"] = np.repeat(
        subID, len(avg_suppress_f_diff)
    )  # input the subject so I can stack melted dfs
    avg_suppress_f_diff["evidence_class"] = np.tile(
        ["suppress F-R"], x
    )  # add in the labels so we know what each data point is refering to
    avg_suppress_f_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_f_diff["TR"] = range(0, 9)
    avg_suppress_f_diff[
        "condition"
    ] = "suppress_f_diff"  # now I want to add in a condition label, since I can then stack all 3 conditions into 1 array per subject

    avg_subject_df = pd.concat(
        [avg_maintain_r, avg_maintain_f, avg_suppress_r, avg_suppress_f],
        ignore_index=True,
        sort=False,
    )
    avg_subject_maintain_diff = pd.concat(
        [avg_maintain_r_diff, avg_maintain_f_diff], ignore_index=True, sort=False
    )
    avg_subject_suppress_diff = pd.concat(
        [avg_suppress_r_diff, avg_suppress_f_diff], ignore_index=True, sort=False
    )
    ...
    if save:
        sub_dir = os.path.join(data_dir, f"sub-{subID}")
        out_fname_template = (
            f"sub-{subID}_{space}_{task}_{ROIs}_4way_avg_evidence_dataframe.csv"
        )
        print(
        ...
        out_fname_template = (
            f"sub-{subID}_{space}_{task}_{ROIs}_maintain_diff_evidence_dataframe.csv"
        )
        print(
            f"\n Saving the sorted evidence dataframe for {subID} - phase: {task} - as {out_fname_template}"
        )
        ...
        out_fname_template = (
            f"sub-{subID}_{space}_{task}_{ROIs}_suppress_diff_evidence_dataframe.csv"
        )
        print(
            f"\n Saving the sorted evidence dataframe for {subID} - phase: {task} - as {out_fname_template}"
        )
...
def visualize_evidence():
    ...
    for subID in temp_subIDs:
        (
            temp_subject_df,
            temp_sub_maintain_diff,
            temp_sub_suppress_diff,
        ) = organize_evidence(subID, space, "study", ROI)
        group_evidence_df = pd.concat(
            [group_evidence_df, temp_subject_df], ignore_index=True, sort=False
        )
        group_evidence_maintain_diff = pd.concat(
            [group_evidence_maintain_diff, temp_sub_maintain_diff],
            ignore_index=True,
            sort=False,
        )
        group_evidence_suppress_diff = pd.concat(
            [group_evidence_suppress_diff, temp_sub_suppress_diff],
            ignore_index=True,
            sort=False,
        )
    ...
    ax = sns.lineplot(
        data=group_evidence_df.loc[
            (group_evidence_df["condition"] == "maintain_r")
            & (group_evidence_df["evidence_class"] == "maintain_r")
        ],
        x="TR",
        y="evidence",
        color="green",
    ...
    ax = sns.lineplot(
        data=group_evidence_df.loc[
            (group_evidence_df["condition"] == "maintain_r")
            & (group_evidence_df["evidence_class"] == "suppress_r")
        ],
        x="TR",
        y="evidence",
        color="red",
    ...
    ax.set_title(
        f"{space} - Memory-Based Operation Classifier during Remembered-Maintain (group average)",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_{ROI}_remember_evi_during_maintain_remember.png",
        )
    )
    plt.clf()
    ...
    ax = sns.lineplot(
        data=group_evidence_df.loc[
            (group_evidence_df["condition"] == "maintain_f")
            & (group_evidence_df["evidence_class"] == "maintain_f")
        ],
        x="TR",
        y="evidence",
        color="green",
    ...
    ax = sns.lineplot(
        data=group_evidence_df.loc[
            (group_evidence_df["condition"] == "maintain_f")
            & (group_evidence_df["evidence_class"] == "suppress_f")
        ],
        x="TR",
        y="evidence",
        color="red",
    ...
    ax.set(xlabel="TR", ylabel="Classifier Evidence")
    ax.set_title(
        f"{space} - Memory-Based Operation Classifier during Forgot-Maintain (group average)",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_{ROI}_remember_evi_during_maintain_forgot.png",
        )
    )
    plt.clf()
    ...
    ax = sns.lineplot(
        data=group_evidence_df.loc[
            (group_evidence_df["condition"] == "suppress_r")
            & (group_evidence_df["evidence_class"] == "maintain_r")
        ],
        x="TR",
        y="evidence",
    ...
    ax.set(xlabel="TR", ylabel="Classifier Evidence")
    ax.set_title(
        f"{space} - Memory-Based Operation Classifier during Remembered-Suppress (group average)",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_{ROI}_remember_evi_during_suppress_remember.png",
        )
    )
    plt.clf()
    ...
    ax = sns.lineplot(
        data=group_evidence_df.loc[
            (group_evidence_df["condition"] == "suppress_f")
            & (group_evidence_df["evidence_class"] == "maintain_f")
        ],
        x="TR",
        y="evidence",
    ...
    ax.set(xlabel="TR", ylabel="Classifier Evidence")
    ax.set_title(
        f"{space} - Memory-Based Operation Classifier during Forgot-Suppress (group average)",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_{ROI}_remember_evi_during_suppress_forgot.png",
        )
    )
    plt.clf()
    ...
    ax.set(xlabel="TR", ylabel="Classifier Evidence")
    ax.set_title(
        f"{space} - Memory-Based Suppress Operation Classifier during Remembered-Suppress (group average)",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_{ROI}_suppress_evi_during_suppress_remember.png",
        )
    )
    plt.clf()
    ...
    ax.set(xlabel="TR", ylabel="Classifier Evidence")
    ax.set_title(
        f"{space} - Memory-Based Suppress Operation Classifier during Forgot-Suppress (group average)",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_{ROI}_suppress_evi_during_suppress_forgot.png",
        )
    )
    plt.clf()
    ...
    ax = sns.lineplot(
        data=group_evidence_maintain_diff.loc[
            (group_evidence_maintain_diff["condition"] == "maintain_r_diff")
            & (group_evidence_maintain_diff["evidence_class"] == "maintain F-R")
        ],
        x="TR",
        y="evidence",
        color="green",
    ...
    ax = sns.lineplot(
        data=group_evidence_maintain_diff.loc[
            (group_evidence_maintain_diff["condition"] == "maintain_f_diff")
            & (group_evidence_maintain_diff["evidence_class"] == "maintain F-R")
        ],
        x="TR",
        y="evidence",
    ...
    ax.set_title(
        f"{space} - Memory-Based Operation Classifier difference during maintain (group average)",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_{ROI}_maintain_evi_diff_during_maintain.png",
        )
    )
    plt.clf()

    ax = sns.lineplot(
        data=group_evidence_suppress_diff.loc[
            (group_evidence_suppress_diff["condition"] == "suppress_r_diff")
            & (group_evidence_suppress_diff["evidence_class"] == "suppress F-R")
        ],
        x="TR",
        y="evidence",
        color="red",
    ...
    ax = sns.lineplot(
        data=group_evidence_suppress_diff.loc[
            (group_evidence_suppress_diff["condition"] == "suppress_f_diff")
            & (group_evidence_suppress_diff["evidence_class"] == "suppress F-R")
        ],
        x="TR",
        y="evidence",
    ...
    ax.set(xlabel="TR", ylabel="Classifier Evidence Difference (forgot-remember)")
    ax.set_title(
        f"{space} - Memory-Based Operation Classifier difference during suppress (group average)",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            f"group_level_{space}_{ROI}_suppress_evi_diff_during_suppress.png",
        )
    )
    plt.clf()
