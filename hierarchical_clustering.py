...
def plot_dendrogram(model, labels, save=False, tag="", out_fname="dendrogram", **kwargs):
    """
    ...
    class Node:
        ...
        def cate_from_children(self):
            ...
            # get self.cate
            sscores, scates = zip(*(sorted(zip(self.cate_score.values(), self.cate_score.keys()), reverse=True)))
    ...
    all_nodes = [Node(ID=i, children=[None, None], cate=lab) for i, lab in enumerate(labels)]
    ...
    dendrogram(linkage_matrix, ax=ax, labels=labels, link_color_func=label2color, **kwargs)
