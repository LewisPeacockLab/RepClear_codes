# code to bootstrap results for operation evidence on face evidence
import warnings
import sys
...
def organize_evidence(subID, space, task, condition, save=True):
    ...
    maintain_face_trials_mean = {}
    replace_face_trials_mean = {}
    suppress_face_trials_mean = {}
    ...
    category_sub_df["face_evi"] = stats.zscore(
        category_sub_df["face_evi"]
    )  # zscore the scene evidence to get a better sense of the relationship when across subjects
    ...
    for i in sub_condition_list:
        if (
            ...
            maintain_face_trials_mean[temp_image] = category_sub_df[["face_evi"]][
                category_sub_index[counter] + 2 : category_sub_index[counter] + 6
            ].values.mean()
            ...
            replace_face_trials_mean[temp_image] = category_sub_df[["face_evi"]][
                category_sub_index[counter] + 2 : category_sub_index[counter] + 6
            ].values.mean()
            ...
            suppress_face_trials_mean[temp_image] = category_sub_df[["face_evi"]][
                category_sub_index[counter] + 2 : category_sub_index[counter] + 6
            ].values.mean()
    ...
    all_face_maintain = pd.DataFrame(
        data=maintain_face_trials_mean.values(),
        index=maintain_face_trials_mean.keys(),
        columns=["evidence"],
    )
    all_face_replace = pd.DataFrame(
        data=replace_face_trials_mean.values(),
        index=replace_face_trials_mean.keys(),
        columns=["evidence"],
    )
    all_face_suppress = pd.DataFrame(
        data=suppress_face_trials_mean.values(),
        index=suppress_face_trials_mean.keys(),
        columns=["evidence"],
    )
    ...
    if condition == "suppress":
        ...
        new_category_sub_df = all_face_suppress.reset_index(
            drop=True
        )  # this removes the information of which item it was
        ...
        new_category_sub_df = all_face_maintain.reset_index(
            drop=True
        )  # this removes the information of which item it was
        ...
        new_category_sub_df = all_face_replace.reset_index(
            drop=True
        )  # this removes the information of which item it was
...
def group_bootstrap():
    ...
    for condition in conditions:
        ...
        group_evidence_df["face_evi"] = group_category_evidence_df["evidence"]
        bootstrap_mediation_df = pd.DataFrame(
            columns=["sample_id", "subject", "operation_evi", "face_evi", "memory"]
        )
        ...
        for i in range(iterations):
            ...
            for sub in temp_subIDs:
                temp_sub_df = pd.DataFrame(columns=["evidence", "memory", "face_evi"])
                ...
                temp_LR_category = LogisticRegression().fit(
                    temp_sub_df["face_evi"].values.reshape(-1, 1),
                    temp_sub_df["memory"].values.reshape(-1),
                ...
                temp_df["face_evi"] = [temp_sub_df["face_evi"].mean()]
                ...
                temp_df_mediation["face_evi"] = temp_sub_df["face_evi"].values
        ...
        # out_fname_template = f"bootstrap_{space}_study_{condition}_iteration_face_dataframe.csv"
        ...
        ci_value_1 = np.percentile(bootstrap_oper_pred_cate, 5)
        ci_value_2 = np.percentile(bootstrap_oper_pred_cate, 95)
        print(f"95% CI for face: {ci_value_1} - {ci_value_2}")
        # ci_value_3=np.percentile(bootstrap_category_beta,95)
        # print(f'95% CI for face on memory: {ci_value_3}')
        ...
        out_fname_template = f"bootstrap_{space}_study_{condition}_face_dataframe.csv"
        print(
