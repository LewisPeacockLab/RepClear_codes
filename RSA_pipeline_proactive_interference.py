...
for subID in subs:
    ...
    print(
        f"Running RSA - Proactive Interference (Version 1 & 2) analysis for sub0{subID}..."
    )
    ...
    sorted_study_scene_order = study_scene_order.sort_values(
        by=["trial_id"]
    )  # this organizes the trials based on the trial_id, since we will need this to assess the condition on the N-1 trial to sort
    ...
    for trial in sorted_study_scene_order["trial_id"].values:
        ...
        study_image_id = sorted_study_scene_order[
            sorted_study_scene_order["trial_id"] == trial
        ]["image_id"].tolist()[
            0
        ]  # this now uses the trial_id (study) to find the image_id (study)
        if (trial == 1) or (trial == 31) or (trial == 61):
            continue
        image_condition = sorted_study_scene_order[
            sorted_study_scene_order["trial_id"] == (trial - 1)
        ]["condition"].tolist()[0]
        trial_subcat = sorted_study_scene_order[
            sorted_study_scene_order["image_id"] == (study_image_id)
        ]["subcategory"].tolist()[0]
        ...
        if image_condition == 1:
            ...
            m_item_repress_pre_comp[m_counter] = masked_bolds_arr_1[
                pre_trial_num - 1, :
            ]

            m_item_weighted_study_comp[m_counter] = np.multiply(
                masked_bolds_arr_2[trial - 1, :],
                masked_weights_arr[pre_trial_num - 1, :],
            )
            m_item_weighted_pre_comp[m_counter] = np.multiply(
            ...
            r_item_repress_pre_comp[r_counter] = masked_bolds_arr_1[
                pre_trial_num - 1, :
            ]

            r_item_weighted_study_comp[r_counter] = np.multiply(
                masked_bolds_arr_2[trial - 1, :],
                masked_weights_arr[pre_trial_num - 1, :],
            )
            r_item_weighted_pre_comp[r_counter] = np.multiply(
                masked_bolds_arr_1[pre_trial_num - 1, :],
                masked_weights_arr[pre_trial_num - 1, :],
            )
            ...
            s_item_repress_pre_comp[s_counter] = masked_bolds_arr_1[
                pre_trial_num - 1, :
            ]

            s_item_weighted_study_comp[s_counter] = np.multiply(
                masked_bolds_arr_2[trial - 1, :],
                masked_weights_arr[pre_trial_num - 1, :],
            )
            s_item_weighted_pre_comp[s_counter] = np.multiply(
                masked_bolds_arr_1[pre_trial_num - 1, :],
                masked_weights_arr[pre_trial_num - 1, :],
            )
    ...
    all_item_repress_study_comp = np.vstack(
        (
            m_item_repress_study_comp,
            r_item_repress_study_comp,
            s_item_repress_study_comp,
        )
    )
    all_item_repress_pre_comp = np.vstack(
        (m_item_repress_pre_comp, r_item_repress_pre_comp, s_item_repress_pre_comp)
    )

    all_item_weighted_study_comp = np.vstack(
        (
            m_item_weighted_study_comp,
            r_item_weighted_study_comp,
            s_item_weighted_study_comp,
        )
    )
    all_item_weighted_pre_comp = np.vstack(
        (m_item_weighted_pre_comp, r_item_weighted_pre_comp, s_item_weighted_pre_comp)
    )

    all_item_pre_study_comp = np.corrcoef(
        all_item_repress_pre_comp, all_item_repress_study_comp
    )
    all_item_weighted_pre_study_comp = np.corrcoef(
        all_item_weighted_pre_comp, all_item_weighted_study_comp
    )
    ...
    for i in trials:
        ...
        diff_array = np.append(
            (all_item_pre_study_comp[i][84:index_interest]),
            (all_item_pre_study_comp[i][index_interest + 1 :]),
        )
        diff_iw_array = np.append(
            (all_item_weighted_pre_study_comp[i][84:index_interest]),
            (all_item_weighted_pre_study_comp[i][index_interest + 1 :]),
        )
    ...
    all_pro_inter_df = pd.DataFrame(
        data=all_uw_proactive_interference, columns=["Fidelity"]
    )
    all_pro_inter_df["Operation"] = np.repeat(["Maintain", "Replace", "Suppress"], 28)

    all_iw_pro_inter_df = pd.DataFrame(
        data=all_iw_proactive_interference, columns=["Fidelity"]
    )
    all_iw_pro_inter_df["Operation"] = np.repeat(
        ["Maintain", "Replace", "Suppress"], 28
    )
    ...
    m_item_weighted_pre_study_comp = np.corrcoef(
        m_item_weighted_pre_comp, m_item_weighted_study_comp
    )
    ...
    for i in trials:
        ...
        diff_index = (
            np.where(maintain_subcate != curr_subcate)[0] + 28
        )  # add 28, to index to the study items and not localizer items, and get the index of the items of the other subcategory
    ...
    r_item_weighted_pre_study_comp = np.corrcoef(
        r_item_weighted_pre_comp, r_item_weighted_study_comp
    )
    ...
    for i in trials:
        ...
        diff_index = (
            np.where(replace_subcate != curr_subcate)[0] + 28
        )  # add 28, to index to the study items and not localizer items, and get the index of the items of the other subcategory
    ...
    s_item_weighted_pre_study_comp = np.corrcoef(
        s_item_weighted_pre_comp, s_item_weighted_study_comp
    )
    ...
    for i in trials:
        ...
        diff_index = (
            np.where(suppress_subcate != curr_subcate)[0] + 28
        )  # add 28, to index to the study items and not localizer items, and get the index of the items of the other subcategory
    ...
    if not os.path.exists(
        os.path.join(
            container_path, "sub-0%s" % subID, "Proactive_interference_%s" % brain_flag
        )
    ):
        os.makedirs(
            os.path.join(
                container_path,
                "sub-0%s" % subID,
                "Proactive_interference_%s" % brain_flag,
            ),
            exist_ok=True,
        )
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "maintain_proactive_interference_unweighted.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "replace_proactive_interference_unweighted.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "suppress_proactive_interference_unweighted.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "maintain_unweighted_target_RSA.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "replace_unweighted_target_RSA.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "suppress_unweighted_target_RSA.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "maintain_proactive_interference_itemweighted.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "replace_proactive_interference_itemweighted.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "suppress_proactive_interference_itemweighted.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "maintain_itemweighted_target_RSA.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "replace_itemweighted_target_RSA.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "suppress_itemweighted_target_RSA.csv",
        )
    )
    del temp_df

    all_pro_inter_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "all_proactive_interference_unweighted.csv",
        )
    )
    all_pro_inter_mean_df = all_pro_inter_df.groupby(by="Operation").mean()
    all_pro_inter_mean_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "all_proactive_interference_mean_unweighted.csv",
        )
    )

    all_iw_pro_inter_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "all_proactive_interference_itemweighted.csv",
        )
    )
    all_iw_pro_inter_mean_df = all_pro_inter_df.groupby(by="Operation").mean()
    all_iw_pro_inter_mean_df.to_csv(
        os.path.join(
            container_path,
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "all_proactive_interference_mean_itemweighted.csv",
        )
    )
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "maintain_corrcoef_unweighted.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "replace_corrcoef_unweighted.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "suppress_corrcoef_unweighted.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "maintain_corrcoef_itemweighted.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "replace_corrcoef_itemweighted.csv",
        )
    )
    del temp_df
    ...
    temp_df.to_csv(
        os.path.join(
            ...
            "sub-0%s" % subID,
            "Proactive_interference_%s" % brain_flag,
            "suppress_corrcoef_itemweighted.csv",
        )
    )
    del temp_df
...
for subID in subs:
    ...
    all_mean_proactive = os.path.join(
        data_path, "all_proactive_interference_mean_unweighted.csv"
    )
    temp_df = pd.read_csv(all_mean_proactive)
    group_all_fidelity = group_all_fidelity.append(temp_df, ignore_index=True)
    del temp_df

    maintain_proactive = os.path.join(
        data_path, "maintain_proactive_interference_unweighted.csv"
    )
    temp_df = pd.read_csv(maintain_proactive, usecols=[1])
    temp_df2 = temp_df.mean()
    temp_df["Operation"] = "Maintain"
    group_fidelity_all_trials = group_fidelity_all_trials.append(
    ...
    replace_proactive = os.path.join(
        data_path, "replace_proactive_interference_unweighted.csv"
    )
    ...
    suppress_proactive = os.path.join(
        data_path, "suppress_proactive_interference_unweighted.csv"
    )
...
for subID in subs:
    ...
    all_mean_proactive = os.path.join(
        data_path, "all_proactive_interference_mean_itemweighted.csv"
    )
    temp_df = pd.read_csv(all_mean_proactive)
    ...
    maintain_proactive = os.path.join(
        data_path, "maintain_proactive_interference_itemweighted.csv"
    )
    temp_df = pd.read_csv(maintain_proactive, usecols=[1])
    temp_df2 = temp_df.mean()
    temp_df["Operation"] = "Maintain"
    iw_group_fidelity_all_trials = iw_group_fidelity_all_trials.append(
        temp_df, ignore_index=True
    )
    temp_df2["Operation"] = "Maintain"
    ...
    replace_proactive = os.path.join(
        data_path, "replace_proactive_interference_itemweighted.csv"
    )
    temp_df = pd.read_csv(replace_proactive, usecols=[1])
    temp_df2 = temp_df.mean()
    temp_df["Operation"] = "Replace"
    ...
    iw_suppress_proactive = os.path.join(
        data_path, "suppress_proactive_interference_itemweighted.csv"
    )
    ...
    temp_df2 = temp_df.mean()
    temp_df2["Operation"] = "Maintain"
    iw_group_all_target_RSA = iw_group_all_target_RSA.append(
        temp_df2, ignore_index=True
    )
    del temp_df, temp_df2
...
if not os.path.exists(
    os.path.join(
        container_path, "group_model", "Proactive_interference_%s" % brain_flag
    )
):
    os.makedirs(
        os.path.join(
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Proactive_interference_%s" % brain_flag,
        "Group_unweighted_proactive_interference.png",
    )
)
plt.clf()

fig = sns.barplot(
    data=group_fidelity_all_trials,
    x="Operation",
    y="Fidelity",
    ci=95,
    palette=["green", "blue", "red"],
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Proactive_interference_%s" % brain_flag,
        "Group_unweighted_proactive_interference_all_trials.png",
    )
)
plt.clf()

fig = sns.barplot(
    data=group_all_fidelity,
    x="Operation",
    y="Fidelity",
    ci=95,
    palette=["green", "blue", "red"],
...
fig.set_title(
    "Unweighted (Group Level) - Proactive Interference (compared to all stims)"
)
fig, test_results = add_stat_annotation(
    fig,
    data=group_all_fidelity,
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Proactive_interference_%s" % brain_flag,
        "Group_unweighted_proactive_interference_all_stims.png",
    )
)
plt.clf()

fig = sns.barplot(
    data=group_all_target_RSA,
    x="Operation",
    y="Fidelity",
    ci=95,
    palette=["green", "blue", "red"],
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Proactive_interference_%s" % brain_flag,
        "Group_unweighted_target_RSA.png",
    )
)
plt.clf()
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Proactive_interference_%s" % brain_flag,
        "Group_itemweighted_proactive_interference.png",
    )
)
plt.clf()

fig = sns.barplot(
    data=iw_group_fidelity_all_trials,
    x="Operation",
    y="Fidelity",
    ci=95,
    palette=["green", "blue", "red"],
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Proactive_interference_%s" % brain_flag,
        "Group_itemweighted_proactive_interference_all_trials.png",
    )
)
plt.clf()

fig = sns.barplot(
    data=iw_group_all_fidelity,
    x="Operation",
    y="Fidelity",
    ci=95,
    palette=["green", "blue", "red"],
...
fig.set_ylabel("Fidelity (Target - Related (all other stims))")
fig.set_title(
    "Item-weighted (Group Level) - Proactive Interference (compared to all stims)"
)
fig, test_results = add_stat_annotation(
    fig,
    data=iw_group_all_fidelity,
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Proactive_interference_%s" % brain_flag,
        "Group_itemweighted_proactive_interference_all_stims.png",
    )
)
plt.clf()

fig = sns.barplot(
    data=iw_group_all_target_RSA,
    x="Operation",
    y="Fidelity",
    ci=95,
    palette=["green", "blue", "red"],
...
plt.savefig(
    os.path.join(
        ...
        "group_model",
        "Proactive_interference_%s" % brain_flag,
        "Group_itemweighted_target_RSA.png",
    )
)
plt.clf()
