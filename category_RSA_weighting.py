...
for subID in subs:
    ...
    catew_remembered_df=pd.DataFrame(columns=['maintain','replace','suppress'],index=range(0,30))
    catew_forgot_df=pd.DataFrame(columns=['maintain','replace','suppress'],index=range(0,30))
    ...
    for img_key in change_cw_maintain_dict.keys():
        ...
        if (memory_csv['memory'][memory_csv['image_num']==img_num]).values[0]==1:
            catew_remembered_df.loc[indexer_r]['maintain']=change_cw_maintain_dict['image ID: %s' % img_num]
              
            indexer_r=indexer_r+1

        elif (memory_csv['memory'][memory_csv['image_num']==img_num]).values[0]==0:
            catew_forgot_df.loc[indexer_f]['maintain']=change_cw_maintain_dict['image ID: %s' % img_num]
    ...
    for img_key in change_cw_replace_dict.keys():
        ...
        if (memory_csv['memory'][memory_csv['image_num']==img_num]).values[0]==1:
            catew_remembered_df.loc[indexer_r]['replace']=change_cw_replace_dict['image ID: %s' % img_num]
                
            indexer_r=indexer_r+1

        elif (memory_csv['memory'][memory_csv['image_num']==img_num]).values[0]==0:
            catew_forgot_df.loc[indexer_f]['replace']=change_cw_replace_dict['image ID: %s' % img_num]
    ...
    for img_key in change_cw_suppress_dict.keys():
        ...
        if (memory_csv['memory'][memory_csv['image_num']==img_num]).values[0]==1:
            catew_remembered_df.loc[indexer_r]['suppress']=change_cw_suppress_dict['image ID: %s' % img_num]
         
            indexer_r=indexer_r+1

        elif (memory_csv['memory'][memory_csv['image_num']==img_num]).values[0]==0:
            catew_forgot_df.loc[indexer_f]['suppress']=change_cw_suppress_dict['image ID: %s' % img_num]
    ...
    catew_remembered_df.to_csv(os.path.join(container_path,"sub-0%s"  % subID,"Representational_Changes_%s" % brain_flag,'cateweighted_pre_post_remember_fidelity.csv'))
    catew_forgot_df.to_csv(os.path.join(container_path,"sub-0%s"  % subID,"Representational_Changes_%s" % brain_flag,'cateweighted_pre_post_forgot_fidelity.csv'))
...
for subID in subs:
    ...
    cate_r_pre_post=os.path.join(data_path,'cateweighted_pre_post_remember_fidelity.csv')
    group_catew_remember_pre_post=group_catew_remember_pre_post.append(pd.read_csv(cate_r_pre_post,usecols=[1,2,3]),ignore_index=True)

    cate_f_pre_post=os.path.join(data_path,'cateweighted_pre_post_forgot_fidelity.csv')
    group_catew_forgot_pre_post=group_catew_forgot_pre_post.append(pd.read_csv(cate_f_pre_post,usecols=[1,2,3]),ignore_index=True)
    ...
    group_by_sub_catew_remember_pre_post=group_by_sub_catew_remember_pre_post.append(pd.read_csv(cate_r_pre_post,usecols=[1,2,3]).mean(),ignore_index=True)

    group_by_sub_catew_forgot_pre_post=group_by_sub_catew_forgot_pre_post.append(pd.read_csv(cate_f_pre_post,usecols=[1,2,3]).mean(),ignore_index=True)   
...
grouped_catew_memory_pre_post=pd.concat((grouped_by_sub_catew_remember_pre_post,grouped_by_sub_catew_forgot_pre_post))

fig=sns.barplot(data=grouped_catew_memory_pre_post,x='variable',y='value',hue='memory',ci=95,palette=['gray','white'],edgecolor=".5")
fig.set_xlabel('Operations')
fig.set_ylabel('Fidelity of item-RSA')
fig.set_title('Category Weighted (Group Level) - Pre vs. Post RSA',loc='center', wrap=True)
