...
for num in range(len(subs)):
    ...
    for trial in range(face_trials):
        ...
        index = [i for i, n in enumerate(temp_events["trial_type"]) if n == "face"][
            trial
        ...
        contrast_array[
            np.where(model.design_matrices_[0].columns[:2].str.match("face_trial"))[0][
                0
            ]
        ] = 1
        contrast_array[
            np.where(model.design_matrices_[0].columns[:2].str.match("other"))[0][0]
        ] = -1

        contrasts = {
            "face_trial%s" % (trial + 1): pad_contrast(contrast_array, n_columns)
        }
    ...
    for trial in range(scene_trials):
        # this is a rough idea how I will create a temporary new version of the events file to use for the LSS
        temp_events = events.copy()
        index = [i for i, n in enumerate(temp_events["trial_type"]) if n == "scene"][
            trial
        ]  # this will find the nth occurance of a desired value in the list
        temp_events.loc[:, "trial_type"] = "other"
        ...
        # this means that regardless of where the 'scene_trial' or 'other' are in the design, it assigns the contrast properly
        contrast_array[
            np.where(model.design_matrices_[0].columns[:2].str.match("scene_trial"))[0][
                0
            ]
        ] = 1
        contrast_array[
        ...
        contrasts = {
            "scene_trial%s" % (trial + 1): pad_contrast(contrast_array, n_columns)
        }
