...
subIDs = ['002','003','004','005','006','007','008','009','010','011','012','013','014','015','016','017','018','023','024','025']
...
for subID in subIDs:
    memory_df=pd.read_csv(os.path.join(param_dir,'memory_and_familiar_sub-%s.csv' % subID))

    maintain_hits=memory_df[((memory_df['condition_num']==1) & (memory_df['resp']>=3))] #this is pulling all the index with correct answers
    replace_hits=memory_df[((memory_df['condition_num']==2) & (memory_df['resp']>=3))] #this is pulling all the index with correct answers
    suppress_hits=memory_df[((memory_df['condition_num']==3) & (memory_df['resp']>=3))] #this is pulling all the index with correct answers
    ...
    maintain_hit_zscore=stats.norm.ppf(m_hit_rate) #this calculates the z-score based on an inverse cumulative distribution function (hits / total)
    replace_hit_zscore=stats.norm.ppf(r_hit_rate) #this calculates the z-score based on an inverse cumulative distribution function (hits / total)
    suppress_hit_zscore=stats.norm.ppf(s_hit_rate) #this calculates the z-score based on an inverse cumulative distribution function (hits / total)
    pre_exp_hit_zscore=stats.norm.ppf(pe_hit_rate) #this calculates the z-score based on an inverse cumulative distribution function (hits / total)
    novel_fa_zscore=stats.norm.ppf(fa_rate) #this is over 60 since there are 60 total novel images
    ...
    def a_prime(hit_rate,false_alarm_rate):
        a_prime_value=(1 - ((1/4)*((false_alarm_rate/hit_rate) + ((1 - hit_rate)/(1-false_alarm_rate)))))
...
d_prime_df = d_prime_df[ np.isfinite( d_prime_df ).all( axis = 1) ] #because of some 0's and 1's, we get some 'inf' we need to remove, sadly that means removing that whole row
...
ax, test_results = add_stat_annotation(ax, data=d_prime_df,
                                   box_pairs=[("maintain", "replace"), ("maintain", "suppress"), ("maintain", "pre exposed"), ("suppress", "pre exposed"), ("replace", "pre exposed")],
...
for i in ax.containers:
    ax.bar_label(i,size=12,label_type='center')
ax, test_results = add_stat_annotation(ax, data=a_prime_df,
                                   box_pairs=[("maintain", "replace"), ("maintain", "suppress"), ("maintain", "pre exposed"), ("suppress", "pre exposed"), ("replace", "pre exposed")],
                                   test='t-test_paired', text_format='star', loc='inside', verbose=2)      
