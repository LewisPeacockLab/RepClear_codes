...
def item_representation_study(subID):
    ...
    if brain_flag == "T1w":
        mask_path = os.path.join(
            ...
            sub,
            "new_mask",
            "VVS_study_%s_mask.nii.gz" % brain_flag,
        )

        mask = nib.load(mask_path)
    else:
        mask_path = os.path.join(
            "/scratch/06873/zbretton/repclear_dataset/BIDS/derivatives/fmriprep/group_MNI_VTC_mask.nii.gz"
        )  # VTC
    ...
    if os.path.exists(
        os.path.join(bold_path, "sub-0%s_%s_studyX.nii.gz" % (subID, brain_flag))
    ):  # adding an X at the end to force it into the else to recreate the files
        img = nib.load(
            os.path.join(bold_path, "sub-0%s_%s_study.nii.gz" % (subID, brain_flag))
        )
        ...
        output_name = os.path.join(
            bold_path, "sub-0%s_%s_study.nii.gz" % (subID, brain_flag)
        )

        nib.save(img, output_name)  # Save the volume

        print(
            "%s Concatenated Study BOLD...saved" % (brain_flag)
    ...
    study_confounds = pd.concat(
        [confound_run1, confound_run2, confound_run3], ignore_index=False
    )
    ...
    if os.path.exists(
        os.path.join(
            bold_path,
            "sub-0%s_%s_study_%s_cleanedX.nii.gz" % (subID, brain_flag, mask_flag),
        )
    ):
        img_clean = nib.load(
            os.path.join(
                bold_path,
                "sub-0%s_%s_study_%s_cleaned.nii.gz" % (subID, brain_flag, mask_flag),
            )
        )

        print(
            "%s Concatenated, Cleaned and %s Masked Study BOLD...LOADED"
            % (brain_flag, mask_flag)
        )
        del img
    else:
        ...
        print(
            "%s Concatenated, Cleaned and %s Masked Study BOLD...saved"
            % (brain_flag, mask_flag)
        )
        del img
    ...
    for trial in range(len(scene_index)):
        ...
        out_folder = os.path.join(
            container_path, sub, "item_representations_%s" % brain_flag
        )
        ...
        )  # this is getting the 2 TRs for that trial's onset and then taking the average of it across the 4th dimension (time)
        removal_timecourse = img_clean.get_fdata()[:, :, :, (onset + 2) : (onset + 11)]

        output_name = os.path.join(
            out_folder,
            ("Sub-0%s_study_scene_trial%s_result.nii.gz" % (subID, (trial + 1))),
        )
        ...
        removal_output_name = os.path.join(
            out_folder,
            (
                "Sub-0%s_study_removal_timecourse_scene_trial%s_result.nii.gz"
                % (subID, (trial + 1))
            ),
        )
        removal_timecourse = removal_timecourse.astype(
            "double"
        )  # Convert the output into a precision format that can be used by other applications
        removal_timecourse[
            np.isnan(removal_timecourse)
        ] = 0  # Exchange nans with zero to ensure compatibility with other applications
        removal_pattern_nii = nib.Nifti1Image(
            removal_timecourse, affine_mat
        )  # create the volume image
