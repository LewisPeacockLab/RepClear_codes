...
for num in subs:
    ...
    category_labels = pd.read_csv(
        "%s_Operation_labels.csv" % brain_flag, delimiter=",", header=None
    )
    category_trials = pd.read_csv(
        "%s_Operation_trials.csv" % brain_flag, delimiter=",", header=None
    )
    ...
    for i in range(1, 31):
        ...
        replace_inds = np.where(
            (evi_df["operation"] == "replace") & (evi_df["operation_trials"] == i)
        )[0]
        suppress_inds = np.where(
            (evi_df["operation"] == "suppress") & (evi_df["operation_trials"] == i)
        )[0]
        maintain_inds = np.where(
            (evi_df["operation"] == "maintain") & (evi_df["operation_trials"] == i)
        )[0]
        # now we are going to add 5 indicies to the end to account for hemodynamics
        replace_inds = np.append(
            replace_inds, [i for i in range(replace_inds[-1] + 1, replace_inds[-1] + 6)]
        )
        suppress_inds = np.append(
            suppress_inds,
            [i for i in range(suppress_inds[-1] + 1, suppress_inds[-1] + 6)],
        )
        maintain_inds = np.append(
            maintain_inds,
            [i for i in range(maintain_inds[-1] + 1, maintain_inds[-1] + 6)],
        )

        if (evi_df["scenes"][replace_inds].values.size) > 14:
            fills = evi_df["scenes"][replace_inds].values.size - 14
            temp = evi_df["scenes"][replace_inds].values[
                :-fills
            ]  # matching array lengths
            temp_faces = evi_df["faces"][replace_inds].values[
                :-fills
            ]  # matching array lengths

            # we also want to see if this item was remembered or not
            trial_id = study_scene_order["image_id"][
                study_scene_order["condition"] == 2
            ].values[i - 1]
            trial_mem = mem_df[mem_df["image_num"] == trial_id]["memory"].values[0]
            if trial_mem == 1:
                r_replace_evi["trial %s" % i] = temp
                temp_df = {
                    "category_evi": temp[7:11].mean(),
                    "memory": 1,
                    "category_diff_evi": (
                        (temp[7:11].mean()) - (temp_faces[7:11].mean())
                    ),
                }

                temp_stim_df = {
                    "category_evi": temp[5:7].mean(),
                    "memory": 1,
                    "category_diff_evi": (
                        (temp[5:7].mean()) - (temp_faces[5:7].mean())
                    ),
                }

                mem_replace_evi = mem_replace_evi.append(temp_df, ignore_index=True)

                stim_replace_evi = stim_replace_evi.append(
                    temp_stim_df, ignore_index=True
                )
        ...
        if (evi_df["scenes"][maintain_inds].values.size) > 14:
            ...
            temp_faces = evi_df["faces"][maintain_inds].values[
                :-fills
            ]  # matching array lengths

            # we also want to see if this item was remembered or not
            trial_id = study_scene_order["image_id"][
                study_scene_order["condition"] == 1
            ].values[i - 1]
            trial_mem = mem_df[mem_df["image_num"] == trial_id]["memory"].values[0]
            if trial_mem == 1:
                ...
                stim_maintain_evi = stim_maintain_evi.append(
                    temp_stim_df, ignore_index=True
                )
        ...
        if (evi_df["scenes"][suppress_inds].values.size) > 14:
            ...
            if trial_mem == 1:
                ...
                stim_suppress_evi = stim_suppress_evi.append(
                    temp_stim_df, ignore_index=True
                )
    ...
    ax = sns.lineplot(
        data=plot_replace_df, x="x", y="y", color="blue", label="Replace-old", ci=68
    )
    ax = sns.lineplot(
        data=plot_otherreplace_df,
        x="x",
        y="y",
        color="skyblue",
        label="Replace-new",
    ...
    ax = sns.lineplot(
        data=plot_maintain_df, x="x", y="y", color="green", label="Maintain", ci=68
    )
    ax = sns.lineplot(
        data=plot_suppress_df, x="x", y="y", color="red", label="Suppress", ci=68
    )
    ax = sns.lineplot(
        data=plot_baseline_df, x="x", y="y", color="gray", label="Baseline", ci=68
    )

    ax.set(
        xlabel="TR (unshfited)",
        ylabel="Category Evidence",
        title="%s %s Category Decoding during Operations" % (brain_flag, sub),
    )
    ...
    plt.savefig(
        os.path.join(
            container_path,
            sub,
            "%s_%s_category_decoding_during_study.png" % (brain_flag, sub),
        )
    )
    plt.clf()
    ############
    ...
    mem_plot_df.to_csv(
        os.path.join(
            container_path, sub, "%s_%s_evidence_for_memory.csv" % (brain_flag, sub)
        )
    )
    ...
    stim_plot_df.to_csv(
        os.path.join(
            container_path, sub, "%s_%s_evidence_for_stimuli.csv" % (brain_flag, sub)
        )
    )
    ...
    plt.savefig(
        os.path.join(
            container_path,
            sub,
            "%s_%s_evidence_for_memory(by_condition).png" % (brain_flag, sub),
        )
    )
    plt.clf()

    sns.lmplot(
        data=mem_plot_df, y="memory", x="category_evi", logistic=True, y_jitter=0.03
    )
    plt.savefig(
        os.path.join(
            container_path,
            sub,
            "%s_%s_evidence_for_memory(combined).png" % (brain_flag, sub),
        )
    )
    plt.clf()
    ...
    plot_diffreplace_df["y"] = replace_evi.sub(
        maintain_evi.mean(axis=1), axis=0
    ).values.flatten()
    plot_diffreplace_df["l"] = np.tile(replace_evi.columns, 14)

    plot_diffsuppress_df["x"] = np.repeat(range(0, 14), 30)
    plot_diffsuppress_df["y"] = suppress_evi.sub(
        maintain_evi.mean(axis=1), axis=0
    ).values.flatten()
    plot_diffsuppress_df["l"] = np.tile(suppress_evi.columns, 14)

    ax = sns.lineplot(
        data=plot_diffreplace_df, x="x", y="y", color="blue", label="Replace", ci=68
    )
    ax = sns.lineplot(
        data=plot_diffsuppress_df, x="x", y="y", color="red", label="Suppress", ci=68
    )
    ...
    plt.savefig(
        os.path.join(
            container_path,
            sub,
            "%s_%s_category_decoding_minusMaintain_during_study.png"
            % (brain_flag, sub),
        )
    )
    plt.clf()
    ...
    group_diffreplace_df[num] = replace_evi.sub(maintain_evi.mean(axis=1), axis=0).mean(
        axis=1
    )
    group_diffsuppress_df[num] = suppress_evi.sub(
        maintain_evi.mean(axis=1), axis=0
    ).mean(axis=1)
...
group_mem_plot.to_csv(
    os.path.join(container_path, "%s_evidence_for_memory.csv" % (brain_flag))
)
group_stim_evi.to_csv(
    os.path.join(container_path, "%s_evidence_for_stimuli.csv" % (brain_flag))
)
...
plt.savefig(
    os.path.join(
        container_path, "%s_evidence_for_memory(by_condition).png" % (brain_flag)
    )
)
plt.clf()

sns.lmplot(
    data=group_mem_plot, y="memory", x="category_evi", logistic=True, y_jitter=0.03
)
plt.savefig(
    os.path.join(container_path, "%s_evidence_for_memory(combined).png" % (brain_flag))
)
plt.clf()

group_replace_evi.to_csv(
    os.path.join(container_path, "%s_group_category_decoding_replace.csv" % brain_flag)
)
group_other_replace_evi.to_csv(
    os.path.join(
        container_path, "%s_group_category_decoding_replace_new.csv" % brain_flag
    )
)
group_maintain_evi.to_csv(
    os.path.join(container_path, "%s_group_category_decoding_maintain.csv" % brain_flag)
)
group_suppress_evi.to_csv(
    os.path.join(container_path, "%s_group_category_decoding_suppress.csv" % brain_flag)
)
group_baseline_evi.to_csv(
    os.path.join(container_path, "%s_group_category_decoding_baseline.csv" % brain_flag)
)
...
ax = sns.lineplot(
    data=plot_group_replace_df, x="x", y="y", color="blue", label="Replace-old", ci=68
)

ax = sns.lineplot(
    data=plot_group_otherreplace_df,
    x="x",
    y="y",
    color="skyblue",
    label="Replace-new",
...
ax = sns.lineplot(
    data=plot_group_maintain_df, x="x", y="y", color="green", label="Maintain", ci=68
)

ax = sns.lineplot(
    data=plot_group_suppress_df, x="x", y="y", color="red", label="Suppress", ci=68
)

ax = sns.lineplot(
    data=plot_group_baseline_df, x="x", y="y", color="gray", label="Baseline", ci=68
)

ax.set(
    xlabel="TR (unshfited)",
    ylabel="Category Evidence",
    title="%s Group-Level Category Decoding during Operations" % brain_flag,
)


plt.savefig(
    os.path.join(
        container_path, "%s_group_category_decoding_during_study.png" % brain_flag
    )
)
plt.clf()
...
ax = sns.lineplot(
    data=plot_group_diffreplace_df, x="x", y="y", color="blue", label="Replace", ci=68
)
ax = sns.lineplot(
    data=plot_group_diffsuppress_df, x="x", y="y", color="red", label="Suppress", ci=68
)

ax.set(
    xlabel="TR (unshfited)",
    ylabel="Category Evidence (removal - maintain)",
    title="%s Group-Level Category Decoding during Operations" % brain_flag,
)
...
plt.savefig(
    os.path.join(
        container_path,
        "%s_group_category_decoding_minusMaintain_during_study.png" % brain_flag,
    )
)
plt.clf()
