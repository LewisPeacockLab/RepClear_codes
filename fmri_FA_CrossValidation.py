...
from sklearn.model_selection import PredefinedSplit, cross_validate, cross_val_predict, GridSearchCV, LeaveOneGroupOut, cross_val_score
...
for subID in subIDs:
    ...
    n_components = [2, 5, 10, 50, 100, 1000, np.shape(full_data)[1]] #last value is # of voxels
    ...
    for n in n_components:
        for X, Y, groups in subsample_by_runs(full_data, label_df): 
            ...
            fa_scores = cross_val_score(estimator=fa, X=X, y=Y, groups=groups, cv=logo, n_jobs=-1)
            ...
            pca_scores = cross_val_score(estimator=pca, X=X, y=Y, groups=groups, cv=logo, n_jobs=-1)
            ...
            for train_inds, test_inds in logo.split(X, Y, groups):
                ...
                auc_score = roc_auc_score(y_test, lr.predict_proba(X_test), multi_class='ovr')
        ...
        fa_decoding[n]={'score':itr_score,'auc':itr_auc,'confusion_matrix':itr_cms}

        pca_decoding[n]={'score':pca_itr_score,'auc':pca_itr_auc,'confusion_matrix':pca_itr_cms}
    ...
    pd.to_pickle(fa_df,'/scratch/06873/zbretton/repclear_dataset/BIDS/derivatives/fmriprep/sub-%s/VTC_subsample_LOGO_FA_CV_LL.pickle' % subID)
    pd.to_pickle(pca_df,'/scratch/06873/zbretton/repclear_dataset/BIDS/derivatives/fmriprep/sub-%s/VTC_subsample_LOGO_PCA_CV_LL.pickle' % subID)
    ...
    pca_mean, pca_std, fa_mean, fa_std = compute_scores(full_data) #running the 4-fold CV with all the data, similar to previous methods
    ...
    pd.to_pickle(fa_df,'/scratch/06873/zbretton/repclear_dataset/BIDS/derivatives/fmriprep/sub-%s/VTC_all_4fold_FA_CV_LL.pickle' % subID)
    pd.to_pickle(pca_df,'/scratch/06873/zbretton/repclear_dataset/BIDS/derivatives/fmriprep/sub-%s/VTC_all_4fold_PCA_CV_LL.pickle' % subID)
