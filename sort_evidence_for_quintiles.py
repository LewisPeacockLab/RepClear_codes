...
for num in subs:
    ...
    evi_df = pd.read_csv(
        os.path.join(
            container_path,
            sub,
            "sub-0%s_%s_trained-preremoval_tested-study_evidence.csv"
            % (num, brain_flag),
        ),
        delimiter=",",
    )
    evi_df = evi_df.iloc[
        :, 1:
    ]  # this will drop the first column which has an unused index
    # need to remove the extra columns that we arent using
    evi_df.drop(
        columns=[
            "phase",
            "trial",
    ...
    for i in range(1, 31):
        ...
        temp = evi_df["scenes"][
            replace_inds
        ].values  # grab scene evidence for these indicies
        temp_faces = evi_df["faces"][
            replace_inds
        ].values  # grab face evidence for these indicies
        ...
        if trial_mem == 1:
            r_replace_evi["trial %s" % i] = temp[
                :11
            ]  # this ensures all trials are equal in length, since this is the lowest TR length (2TR-stim + 4TR-operation + 5TR-ITI)
            temp_df = {
                "category_evi": temp[2:6].mean(),
                "memory": 1,
                "category_diff_evi": ((temp[2:6].mean()) - (temp_faces[2:6].mean())),
            }

            temp_stim_df = {
                "category_evi": temp[0:2].mean(),
                "memory": 1,
                "category_diff_evi": ((temp[0:2].mean()) - (temp_faces[0:2].mean())),
            }
        ...
        replace_evi["trial %s" % i] = temp[
            :11
        ]  # -normalize #only grabs the first 11 TRs so that everything is equal, across jittered ITIs
        ...
        # MAINTAIN########
        temp = evi_df["scenes"][maintain_inds].values
        temp_faces = evi_df["faces"][maintain_inds].values  # matching array lengths
        ...
        trial_mem = mem_df[mem_df["image_num"] == trial_id]["memory"].values[0]
        if trial_mem == 1:
            r_maintain_evi["trial %s" % i] = temp[:11]
            temp_df = {
                "category_evi": temp[2:6].mean(),
                "memory": 1,
                "category_diff_evi": ((temp[2:6].mean()) - (temp_faces[2:6].mean())),
            ...
            f_maintain_evi["trial %s" % i] = temp[:11]
            temp_df = {
                "category_evi": temp[2:6].mean(),
                "memory": 0,
                "category_diff_evi": ((temp[2:6].mean()) - (temp_faces[2:6].mean())),
        ...
        # SUPPRESS############
        temp = evi_df["scenes"][suppress_inds].values
        temp_faces = evi_df["faces"][suppress_inds].values
        ...
        trial_mem = mem_df[mem_df["image_num"] == trial_id]["memory"].values[0]
        if trial_mem == 1:
            r_suppress_evi["trial %s" % i] = temp[:11]
            temp_df = {
                "category_evi": temp[2:6].mean(),
                "memory": 1,
                "category_diff_evi": ((temp[2:6].mean()) - (temp_faces[2:6].mean())),
            ...
            f_suppress_evi["trial %s" % i] = temp[:11]
            temp_df = {
                "category_evi": temp[2:6].mean(),
                "memory": 0,
                "category_diff_evi": ((temp[2:6].mean()) - (temp_faces[2:6].mean())),
    ...
    plt.savefig(
        os.path.join(
            container_path,
            sub,
            "%s_%s_category_decoding_during_study_test.png" % (brain_flag, sub),
        )
    )
    plt.clf()
    ############
