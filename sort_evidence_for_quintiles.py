...
for num in subs:
    ...
    evi_df=pd.read_csv(os.path.join(container_path,sub,"sub-0%s_%s_trained-preremoval_tested-study_evidence.csv" % (num,brain_flag)), delimiter=",")
    evi_df=evi_df.iloc[:,1:] #this will drop the first column which has an unused index
    #need to remove the extra columns that we arent using
    evi_df.drop(columns=['phase','trial','order','plan_onset','onset','jitter','run','pre_post','category','subcategory','image_id','new_category','new_subcategory','new_image_id','response','accuracy','rt','concat_vol','stim_present'],inplace=True)
    ...
    for i in range(1,31):
        ...
        #REPLACE#######
        temp = evi_df['scenes'][replace_inds].values #grab scene evidence for these indicies
        temp_faces = evi_df['faces'][replace_inds].values #grab face evidence for these indicies
        ...
        if trial_mem==1:
            r_replace_evi['trial %s' % i]=temp[:11] #this ensures all trials are equal in length, since this is the lowest TR length (2TR-stim + 4TR-operation + 5TR-ITI)
            temp_df={'category_evi':temp[2:6].mean(), 'memory':1,'category_diff_evi':((temp[2:6].mean()) - (temp_faces[2:6].mean()))}

            temp_stim_df={'category_evi':temp[0:2].mean(), 'memory':1, 'category_diff_evi':((temp[0:2].mean()) - (temp_faces[0:2].mean()))}
            ...
            temp_df={'category_evi':temp[2:6].mean(), 'memory':0,'category_diff_evi':((temp[2:6].mean()) - (temp_faces[2:6].mean()))}

            temp_stim_df={'category_evi':temp[0:2].mean(), 'memory':0, 'category_diff_evi':((temp[0:2].mean()) - (temp_faces[0:2].mean()))}
