...
def make_tim_file():
    ...
    # output directory
    out_dir = (
        "/Users/zb3663/Desktop/School Files/Repclear_files/repclear_preprocessed/params"
    )
    ...
    phases = {1: "pre-exposure", 2: "pre-localizer", 3: "study", 4: "post-localizer"}
    ...
    for subID in subIDs:
        mat_dict = scipy.io.loadmat(
            f"{data_dir}/{projID}_{subID}/{fname.format(subID)}", simplify_cells=True
        )
        ...
        for phase in phases.keys():
            ...
            header = mat_dict["args"]["design"]["ph"][phase - 1][
                "header"
            ]  # this is by trial
            ...
            for imgid in phase_df["image_id"]:
                img_names.append(
                    img_df.iloc[np.where(img_df["id"] == imgid)[0][0]]["image_name"]
                )
                fam_means.append(
                    img_df.iloc[np.where(img_df["id"] == imgid)[0][0]][
                        "familiarity_mean"
                    ]
                )
                fam_ses.append(
                    img_df.iloc[np.where(img_df["id"] == imgid)[0][0]]["familiarity_se"]
                )
            ...
            for catei in range(1, 3):
                n_cate_trial = sum(
                    phase_df["category"] == catei
                )  # number of trials for this category in this phase
                phase_df.loc[phase_df["category"] == catei, "trial_id"] = (
                    np.arange(n_cate_trial) + 1
                )

            subdfs.append(phase_df)

            out_fname = (
                f"{out_dir}/sub-{subIDs_dict[subID]}_{phases[phase]}_tr_design.csv"
            )
