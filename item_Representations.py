...
def item_representation(subID):
    ...
    if os.path.exists(
        os.path.join(bold_path, "sub-0%s_%s_preremovalX.nii.gz" % (subID, brain_flag))
    ):  # adding "X" to the end to force it to remake these files since they keep erroring
        img = nib.load(
            os.path.join(
                bold_path, "sub-0%s_%s_preremoval.nii.gz" % (subID, brain_flag)
            )
        )
        ...
        print("%s Concatenated Localizer BOLD...saved" % (brain_flag))
    ...
    if os.path.exists(
        os.path.join(
            bold_path,
            "sub-0%s_%s_preremoval_%s_cleanedX.nii.gz" % (subID, brain_flag, mask_flag),
        )
    ):
        img_clean = nib.load(
            os.path.join(
                bold_path,
                "sub-0%s_%s_preremoval_%s_cleaned.nii.gz"
                % (subID, brain_flag, mask_flag),
            )
        )

        print(
            "%s Concatenated, Cleaned and %s Masked Localizer BOLD...LOADED"
        ...
        output_name = os.path.join(
            bold_path,
            "sub-0%s_%s_preremoval_%s_cleaned.nii.gz" % (subID, brain_flag, mask_flag),
        )

        nib.save(img_clean, output_name)  # Save the volume

        print(
            "%s Concatenated, Cleaned and %s Masked Localizer BOLD...saved"
            % (brain_flag, mask_flag)
        )
        del img
    ...
    for trial in range(len(face_index)):
        ...
        output_name = os.path.join(
            out_folder,
            ("Sub-0%s_pre_face_trial%s_result.nii.gz" % (subID, (trial + 1))),
        )
    ...
    for trial in range(len(scene_index)):
        ...
        output_name = os.path.join(
            out_folder,
            ("Sub-0%s_pre_scene_trial%s_result.nii.gz" % (subID, (trial + 1))),
        )
        trial_pattern = trial_pattern.astype(
            "double"
        )  # Convert the output into a precision format that can be used by other applications
