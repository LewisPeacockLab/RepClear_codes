...
def clearmem_cate_classification(subID="006", xval=False, C=0.01):
    ...
    if xval: 
        ...
        lr = LogisticRegression(penalty='l2', solver='liblinear', C=C, verbose=5, n_jobs=32)
        lr.fit(X_sub, Y)
        model = lr 
        ...
        cm = confusion_matrix(Y, preds, labels=list(stim_labels.keys())) / true_counts[:,None] * 100
        ...
        # save
        out_path = os.path.join(results_dir, "category_clf", f"sub-{subID}_clearmem_task-{task}_space-{space}_VTC_final-lr.pkl")
        print(f"Saving results to {out_path}...")
...
def clearmem_study_decode(subID="006"):
    ...
    # === load model
    file_path = os.path.join(results_dir, "category_clf", f"sub-{subID}_clearmem_task-localizer_space-{space}_VTC_final-lr.pkl")
    ...
    # decode metrics
    evid = (1. / (1. + np.exp(-model.decision_function(data_sub))))  # TR x class
    ...
    out_fname = os.path.join(results_dir, "category_clf", f"sub-{subID}_final-lr-decode.pkl")
