...
for num in range(len(subs)):
    ...
    face_mask_path = os.path.join(
        container_path, sub, "results", "face_v_other_mask.nii.gz"
    )
    fruit_mask_path = os.path.join(
        container_path, sub, "results", "fruit_v_other_mask.nii.gz"
    )
    scene_mask_path = os.path.join(
        container_path, sub, "results", "scene_v_other_mask.nii.gz"
    )
    ...
    face_localizer_run1 = apply_mask(
        mask=(face_mask.get_data()), target=(localizer_run1.get_data())
    )
    face_localizer_run2 = apply_mask(
        mask=(face_mask.get_data()), target=(localizer_run2.get_data())
    )
    face_localizer_run3 = apply_mask(
        mask=(face_mask.get_data()), target=(localizer_run3.get_data())
    )
    face_localizer_run4 = apply_mask(
        mask=(face_mask.get_data()), target=(localizer_run4.get_data())
    )
    face_localizer_run5 = apply_mask(
        mask=(face_mask.get_data()), target=(localizer_run5.get_data())
    )

    fruit_localizer_run1 = apply_mask(
        mask=(fruit_mask.get_data()), target=(localizer_run1.get_data())
    )
    fruit_localizer_run2 = apply_mask(
        mask=(fruit_mask.get_data()), target=(localizer_run2.get_data())
    )
    fruit_localizer_run3 = apply_mask(
        mask=(fruit_mask.get_data()), target=(localizer_run3.get_data())
    )
    fruit_localizer_run4 = apply_mask(
        mask=(fruit_mask.get_data()), target=(localizer_run4.get_data())
    )
    fruit_localizer_run5 = apply_mask(
        mask=(fruit_mask.get_data()), target=(localizer_run5.get_data())
    )

    scene_localizer_run1 = apply_mask(
        mask=(scene_mask.get_data()), target=(localizer_run1.get_data())
    )
    scene_localizer_run2 = apply_mask(
        mask=(scene_mask.get_data()), target=(localizer_run2.get_data())
    )
    scene_localizer_run3 = apply_mask(
        mask=(scene_mask.get_data()), target=(localizer_run3.get_data())
    )
    scene_localizer_run4 = apply_mask(
        mask=(scene_mask.get_data()), target=(localizer_run4.get_data())
    )
    scene_localizer_run5 = apply_mask(
        mask=(scene_mask.get_data()), target=(localizer_run5.get_data())
    )

    wholebrain_mask1 = nib.load(brain_mask_path[0])

    # unsure if I need to z-score or not, will check on this
    face_preproc_1 = clean(
        face_localizer_run1, t_r=0.46, detrend=False, standardize="zscore"
    )
    face_preproc_2 = clean(
        face_localizer_run2, t_r=0.46, detrend=False, standardize="zscore"
    )
    face_preproc_3 = clean(
        face_localizer_run3, t_r=0.46, detrend=False, standardize="zscore"
    )
    face_preproc_4 = clean(
        face_localizer_run4, t_r=0.46, detrend=False, standardize="zscore"
    )
    face_preproc_5 = clean(
        face_localizer_run5, t_r=0.46, detrend=False, standardize="zscore"
    )

    fruit_preproc_1 = clean(
        fruit_localizer_run1, t_r=0.46, detrend=False, standardize="zscore"
    )
    fruit_preproc_2 = clean(
        fruit_localizer_run2, t_r=0.46, detrend=False, standardize="zscore"
    )
    fruit_preproc_3 = clean(
        fruit_localizer_run3, t_r=0.46, detrend=False, standardize="zscore"
    )
    fruit_preproc_4 = clean(
        fruit_localizer_run4, t_r=0.46, detrend=False, standardize="zscore"
    )
    fruit_preproc_5 = clean(
        fruit_localizer_run5, t_r=0.46, detrend=False, standardize="zscore"
    )

    scene_preproc_1 = clean(
        scene_localizer_run1, t_r=0.46, detrend=False, standardize="zscore"
    )
    scene_preproc_2 = clean(
        scene_localizer_run2, t_r=0.46, detrend=False, standardize="zscore"
    )
    scene_preproc_3 = clean(
        scene_localizer_run3, t_r=0.46, detrend=False, standardize="zscore"
    )
    scene_preproc_4 = clean(
        scene_localizer_run4, t_r=0.46, detrend=False, standardize="zscore"
    )
    scene_preproc_5 = clean(
        scene_localizer_run5, t_r=0.46, detrend=False, standardize="zscore"
    )

    face_localizer_bold = np.concatenate(
        (face_preproc_1, face_preproc_2, face_preproc_3, face_preproc_4, face_preproc_5)
    )
    fruit_localizer_bold = np.concatenate(
        (
            fruit_preproc_1,
            fruit_preproc_2,
            fruit_preproc_3,
    ...
    scene_localizer_bold = np.concatenate(
        (
            scene_preproc_1,
            scene_preproc_2,
            scene_preproc_3,
        ...
        if x <= 18:
            ...
            for run in range(1, 6):
                try:
                    face_items[x].append(
                        events.loc[
                            (events["trial_type"] == x) & (events["run"] == run)
                        ].onsets_volume.values[0]
                    )  # this is how we search for the item level times
                except:
                    pass
            for y in range(len(face_items[x])):
                beta_path = os.path.join(
                    ...
                    sub,
                    "results",
                    "item%s_v_other_face_stat_map.nii.gz" % x,
                )
                temp_beta = nib.load(beta_path)
                temp_beta_masked = apply_mask(
                    mask=face_mask.get_data(), target=temp_beta.get_data()
                )
                temp.update(
                    {
                        "run%s"
                        % (y + 1): (
                            face_localizer_bold[
                                (face_items[x][y] + 9) : (face_items[x][y] + 18)
                            ].mean(axis=0)
                        )
                        * temp_beta_masked
                    }
                )  # take the mean of the 3 TR where the item is presented
                del beta_path, temp_beta_masked, temp_beta
            face_data[x] = temp
            del temp
            df_face_data = pd.DataFrame.from_dict(
                {
                    (i, j): face_data[i][j]
            ...
            for y in range(len(fruit_items[x])):
                beta_path = os.path.join(
                    ...
                    sub,
                    "results",
                    "item%s_v_other_fruit_stat_map.nii.gz" % x,
                )
                temp_beta = nib.load(beta_path)
                temp_beta_masked = apply_mask(
                    mask=fruit_mask.get_data(), target=temp_beta.get_data()
                )
                temp.update(
                    {
                        "run%s"
                        % (y + 1): (
                            fruit_localizer_bold[
                                (fruit_items[x][y] + 9) : (fruit_items[x][y] + 18)
                            ].mean(axis=0)
                        )
                        * temp_beta_masked
                    }
            ...
            fruit_data[x] = temp
            del temp
            df_fruit_data = pd.DataFrame.from_dict(
                {
                    (i, j): fruit_data[i][j]
            ...
            for y in range(len(scene_items[x])):
                beta_path = os.path.join(
                    ...
                    sub,
                    "results",
                    "item%s_v_other_scene_stat_map.nii.gz" % x,
                )
                temp_beta = nib.load(beta_path)
                temp_beta_masked = apply_mask(
                    mask=scene_mask.get_data(), target=temp_beta.get_data()
                )
                temp.update(
                    {
                        "run%s"
                        % (y + 1): (
                            scene_localizer_bold[
                                (scene_items[x][y] + 9) : (scene_items[x][y] + 18)
                            ].mean(axis=0)
                        )
                        * temp_beta_masked
                    }
            ...
            scene_data[x] = temp
            del temp
            df_scene_data = pd.DataFrame.from_dict(
                {
                    (i, j): scene_data[i][j]
...
ax = sns.heatmap(
    data=(np.nan_to_num(sub_mean_face, posinf=1)), cmap="jet", vmin=-0.2, vmax=0.9
)
