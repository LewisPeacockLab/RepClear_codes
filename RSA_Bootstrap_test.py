...
for i in range(n_iterations):
    ...
    for sub in range(len(subs)):
        xboot_sub_r = random.sample(
            list(group_item_remember_pre_post["suppress"].dropna()), n_trials
        )
        iteration_supersub_r = iteration_supersub_r + xboot_sub_r

        xboot_sub_f = random.sample(
            list(group_item_forgot_pre_post["suppress"].dropna()), n_trials
        )
...
if Average(bootstrap_remember) > Average(bootstrap_forgot):
    diff = np.array(bootstrap_remember) - np.array(
        bootstrap_forgot
    )  # calcuate the difference of all iterations

    precent_boot = (
        len(np.where(diff > 0)[0]) / 1000
    ) * 100  # calculate the % of iterations where the difference is greater than 0
...
melt_remember.rename(
    columns={"variable": "Operation", "value": "Fidelity"}, inplace=True
)
...
melt_remember["Subject"] = (
    np.tile((range(len(subs) - 4)), 3) + 1
)  # repeats 1-18 3 times, for the sub list
