...
for num in range(len(subs)):
    ...
    if brain_flag == "MNI":
        gm_mask_path = os.path.join(
            "/scratch/06873/zbretton/repclear_dataset/BIDS/derivatives/fmriprep/group_%s_GM_mask.nii.gz"
            % brain_flag
        )
    ...
    model_operations = FirstLevelModel(
        t_r=1,
        hrf_model="glover",
        drift_model=None,
        high_pass=None,
    ...
    contrasts = {
        "maintain": pad_contrast([2, -1, 0, -1], n_columns),
    ...
    for contrast in contrasts:
        z_map_s = model_operations.compute_contrast(
            contrasts[contrast], output_type="z_score"
        )

        nib.save(
            z_map_s, os.path.join(out_folder, f"{contrast}_{brain_flag}_zmap.nii.gz")
        )

        t_map_s = model_operations.compute_contrast(
            contrasts[contrast], stat_type="t", output_type="stat"
        )

        nib.save(
            t_map_s, os.path.join(out_folder, f"{contrast}_{brain_flag}_tmap.nii.gz")
        )

        file_data_s = model_operations.generate_report(
            contrasts[contrast], bg_img=mean_img_
        )

        file_data_s.save_as_html(
            os.path.join(out_folder, f"{contrast}_{brain_flag}_report.html")
        )

#######

# alternative lvl2 with FDR:

container_path_save = (
    "/Users/zb3663/Library/CloudStorage/Box-Box/LewPeaLabBox/STUDY/RepClear/univariate"
)
container_path = "/Volumes/zbg_eHD/Zachary_Data/processed"
...
out_dir = os.path.join(container_path_save, "group_model", "group_operation_lvl2")
...
for contrast in contrasts:
    ...
    maps = [
        nib.load(
            os.path.join(
                container_path,
                sub,
                "study_lvl1_%s" % brain_flag,
                f"{contrast}_{brain_flag}_zmap.nii.gz",
            )
        )
        for sub in subs
    ]
    ...
    second_level_model = SecondLevelModel(
        smoothing_fwhm=None,
        mask_img="/Volumes/zbg_eHD/Zachary_Data/processed/group_mask/group_mask.nii.gz",
        verbose=2,
        n_jobs=-1,
    )
    ...
    file_data.save_as_html(
        os.path.join(out_dir, f"group+{contrast}_{brain_flag}_fdr_report.html")
    )
    # use this threshold to look at the second-level results
    nib.save(
        thresholded_map,
        os.path.join(
            out_dir, f"group+{contrast}_{brain_flag}_thresholded_fdr_zmap.nii.gz"
        ),
    )
