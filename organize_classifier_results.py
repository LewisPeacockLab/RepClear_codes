...
for subID in subIDs:
    in_file = os.path.join(
        data_dir, f"sub-{subID}_task-preremoval_space-T1w_VVS_lrxval.npz"
    )  # get numpy file name

    temp_dict = np.load(in_file)  # load in numpy dict

    group_aucs.append(
        temp_dict["auc_scores"].mean()
    )  # take the mean of the x-valdiation AUCs and append to group level table for stats
...
for subID in subIDs:
    in_file = os.path.join(
        data_dir, f"sub-{subID}", f"sub-{subID}_T1w_study_operation_auc.csv"
    )  # get dataframe file name
...
in_file = os.path.join(
    data_dir, "btwnsub_MNI_study_operation_auc.csv"
)  # get dataframe file name
