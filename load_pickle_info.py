    ...
    for TR_shift in TR_array:
        ...
        if phase == "localizer":
            sub_dict = pickle.load(
                open(
                    "%s-preremoval_%s_%s_%sTR lag_data.pkl"
        ...
        print("Running %s..." % sub)
        print(
            "TR lag = %s  |  Brain-Space = %s  |  Mask = %s"
            % (TR_shift, brain_flag, mask_flag)
        )
        print("-----------------------------")
        ...
        if phase == "localizer":
            ...
            print(
                "Classifier Predictions = %s , %s"
                % (predicts[0], predicts[1], predicts[2])
            )
        print("-----------------------------")
