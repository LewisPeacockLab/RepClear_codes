...
def item_level_RSA(subID="002", phase="pre", weight_source="LSS", stats_test="tmap"):
    ...
    print(
        f"Running item level RSA & MDS for sub{subID}, {phase}, {weight_source}, {stats_test}..."
    )
    ...
    for cateID in sub_cates.keys():
        if weight_source == "LSA":
            cate_weights_fnames = glob.glob(
                f"{weights_dir}/{cateID}*full*{stats_test}*"
            )
        elif weight_source == "LSS":
        ...
        weights_arr.append(
            np.stack([cate_weights[i] for i in sorted(cate_weights.keys())])
        )
    ...
    out_fname = os.path.join(
        results_dir, "MDS", f"sub-{subID}_{phase}_{weight_source}_{stats_test}_mdss"
    )
    if not os.path.exists(os.path.join(results_dir, "MDS")):
        os.makedirs(os.path.join(results_dir, "MDS"), exist_ok=True)
    _ = select_MDS(item_repress, save=True, out_fname=out_fname)

    # ===== perform the correlation
    corr_matrix = np.corrcoef(item_repress)
    out_name = os.path.join(
        results_dir, "RSM", f"sub-{subID}_{phase}_{weight_source}_{stats_test}_rsm"
    )
    if not os.path.exists(os.path.join(results_dir, "RSM")):
        os.makedirs(os.path.join(results_dir, "RSM"), exist_ok=True)
...
def prepare_plot(subID):
    ...
    data = np.load(
        os.path.join(data_path, "sub-%s_pre_LSS_tmap_mdss.npz" % subID),
        allow_pickle=True,
    )
    mds = data["mdss"][1]
    return mds, labels
...
def prepare_plot_clearmem(subID):
    ...
    labels = np.array(labels)
    data = np.load(
        os.path.join(data_path, "sub-%s_LSS_mdss.npz" % subID), allow_pickle=True
    )
    mds = data["mdss"][1]
    return mds, labels
...
def item_RSA_compare(
    subID="002", phase1="pre", phase2="post", weight_source="BOTH", stats_test="tmap"
):
    """
    ...
    print(
        f"Running item level RSA for sub{subID}, {phase1} comapred to {phase2}, {weight_source}, {stats_test}..."
    )
    ...
    if phase1 == "pre":
        if weight_source == "LSA":
            weights_dir_1 = os.path.join(
                data_dir, f"sub-{subID}", "localizer_item_level"
            )
        elif weight_source == "LSS":
    ...
    if phase2 == "post":
        if weight_source == "LSA":
            weights_dir_2 = os.path.join(
                data_dir, f"sub-{subID}", "localizer_item_level"
            )
        elif weight_source == "LSS":
            weights_dir_2 = os.path.join(
                data_dir, f"sub-{subID}", "post_localizer_LSS_lvl1"
            )
        elif weight_source == "BOTH":
            LSS_dir_2 = os.path.join(
                data_dir, f"sub-{subID}", "post_localizer_LSS_lvl1"
            )
        else:
            raise ValueError("Weight source must be LSA, LSS or BOTH")
    ...
    for cateID in sub_cates.keys():
        if weight_source == "LSA":
            ...
            cate_weights_fnames_1 = glob.glob(
                f"{weights_dir_1}/{cateID}*full*{stats_test}*"
            )
            ...
            cate_weights_1 = {
                i: cate_weights_1[i] for i in sorted(cate_weights_1.keys())
            }
            all_weights_1[cateID] = cate_weights_1
            weights_arr_1.append(
                np.stack([cate_weights_1[i] for i in sorted(cate_weights_1.keys())])
            )
            ...
            cate_weights_2 = {
                i: cate_weights_2[i] for i in sorted(cate_weights_2.keys())
            }
            ...
            weights_arr_2.append(
                np.stack([cate_weights_2[i] for i in sorted(cate_weights_2.keys())])
            )
            ...
            LSS_arr_1.append(
                np.stack([LSS_data_1[i] for i in sorted(LSS_data_1.keys())])
            )
            ...
            try:
                LSS_arr_2.append(
                    np.stack([LSS_data_2[i] for i in sorted(LSS_data_2.keys())])
                )
            except:
    ...
    if phase1 == "pre":
        pre_face_order, pre_scene_order = sort_trials_by_categories(
            subID=subID, phase=2
        )
    if phase2 == "post":
    ...
    for trial in range(len(pre_scene_order)):
        trial_index = pre_scene_order.index[
            pre_scene_order["trial_id"] == (trial + 1)
        ].tolist()[
            0
        ]  # adding 1 to trial since index starts at 1, then finding that index in the scene_order DF
        image_id = pre_scene_order.loc[
            trial_index, "image_id"
        ...
        post_trial_index = post_scene_order.index[
            post_scene_order["image_id"] == image_id
        ].tolist()[0]
        post_trial_num = post_scene_order.loc[
            post_trial_index, "trial_id"
        ]  # now we used the image ID to find the proper trial in the post condition to link to

        LSS_trial_fidelity = np.corrcoef(
            item_LSS_pre[trial + len(pre_face_order), :],
            item_LSS_post[(post_trial_num - 1), :],
        )
        LSA_trial_fidelity = np.corrcoef(
            item_repress_pre[trial + len(pre_face_order), :],
            item_repress_post[(post_trial_num - 1), :],
        )
    ...
    print(
        "Average LSS Fidelity of unoperated items: %s | std: %s"
        % (
            np.array(list(LSS_unoperated_dict.values())).mean(),
            np.array(list(LSS_unoperated_dict.values())).std(),
        )
    )
    print(
        "Average LSS Fidelity of maintained items: %s | std: %s"
        % (
            np.array(list(LSS_maintain_dict.values())).mean(),
            np.array(list(LSS_maintain_dict.values())).std(),
        )
    )
    print(
        "Average LSS Fidelity of replaced items: %s | std: %s"
        % (
            np.array(list(LSS_replace_dict.values())).mean(),
            np.array(list(LSS_replace_dict.values())).std(),
        )
    )
    print(
        "Average LSS Fidelity of suppressed items: %s | std: %s"
        % (
            np.array(list(LSS_suppress_dict.values())).mean(),
            np.array(list(LSS_suppress_dict.values())).std(),
        )
    )

    if not os.path.exists(
        os.path.join(data_dir, "sub-%s" % subID, "Representational_Changes")
    ):
        os.makedirs(
            os.path.join(data_dir, "sub-%s" % subID, "Representational_Changes"),
            exist_ok=True,
    ...
    LSS_df.to_csv(
        os.path.join(
            data_dir, "sub-%s" % subID, "Representational_Changes", "LSS_fidelity.csv"
        )
    )

    pd.DataFrame(LSS_maintain_dict, index=[0]).to_csv(
        os.path.join(
            data_dir,
            "sub-%s" % subID,
            "Representational_Changes",
            "LSS_maintain_fidelity.csv",
        )
    )
    pd.DataFrame(LSS_replace_dict, index=[0]).to_csv(
        os.path.join(
            data_dir,
            "sub-%s" % subID,
            "Representational_Changes",
            "LSS_replace_fidelity.csv",
        )
    )
    pd.DataFrame(LSS_suppress_dict, index=[0]).to_csv(
        os.path.join(
            data_dir,
            "sub-%s" % subID,
            "Representational_Changes",
            "LSS_suppress_fidelity.csv",
        )
    )
    pd.DataFrame(LSS_unoperated_dict, index=[0]).to_csv(
        os.path.join(
            data_dir,
            "sub-%s" % subID,
            "Representational_Changes",
            "LSS_unoperated_fidelity.csv",
        )
    )
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-%s" % subID,
            "Representational_Changes",
            "LSS_fidelity_violin_summary.png",
        )
    )
    plt.clf()
    ...
    LSA_df.to_csv(
        os.path.join(
            data_dir, "sub-%s" % subID, "Representational_Changes", "LSA_fidelity.csv"
        )
    )
    ...
    plt.savefig(
        os.path.join(
            ...
            "sub-%s" % subID,
            "Representational_Changes",
            "LSA_fidelity_violin_summary.png",
        )
    )
    plt.clf()
    ...
    dict_file = open(
        os.path.join(
            data_dir, "sub-%s" % subID, "Representational_Changes", "LSS_unoperated.pkl"
        ),
        "wb",
    )
    ...
    dict_file = open(
        os.path.join(
            data_dir, "sub-%s" % subID, "Representational_Changes", "LSA_unoperated.pkl"
        ),
        "wb",
    )
    ...
    dict_file = open(
        os.path.join(
            data_dir, "sub-%s" % subID, "Representational_Changes", "LSS_Maintain.pkl"
        ),
        "wb",
    )
    ...
    dict_file = open(
        os.path.join(
            data_dir, "sub-%s" % subID, "Representational_Changes", "LSA_Maintain.pkl"
        ),
        "wb",
    )
    ...
    dict_file = open(
        os.path.join(
            data_dir, "sub-%s" % subID, "Representational_Changes", "LSS_Replace.pkl"
        ),
        "wb",
    )
    ...
    dict_file = open(
        os.path.join(
            data_dir, "sub-%s" % subID, "Representational_Changes", "LSA_Replace.pkl"
        ),
        "wb",
    )
    ...
    dict_file = open(
        os.path.join(
            data_dir, "sub-%s" % subID, "Representational_Changes", "LSS_Suppress.pkl"
        ),
        "wb",
    )
    ...
    dict_file = open(
        os.path.join(
            data_dir, "sub-%s" % subID, "Representational_Changes", "LSA_Suppress.pkl"
        ),
        "wb",
    )
