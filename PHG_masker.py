...
for num in range(len(subs)):
    ...
    def new_mask(subject_path):
        ...
        for roi in [lh_parahippocampal_anterior,rh_parahippocampal_anterior,lh_parahippocampal,rh_parahippocampal]:
                os.system('fslmaths %s -thr %s -uthr %s %s'%(aparc_aseg, roi[1], roi[1], roi[0]))
        out_mask = os.path.join(outdir, 'PHG_%s_%s_mask.nii.gz' % (task_flag,brain_flag))
                
        os.system('fslmaths %s -add %s -add %s -add %s -bin %s'%(lh_parahippocampal[0], rh_parahippocampal[0], lh_parahippocampal_anterior[0], rh_parahippocampal_anterior[0],out_mask))
