...
for num in range(len(subs)):
    ...
    model_face.fit(
        run_imgs=img[:2], events=events_list[:2], confounds=localizer_confounds[:2]
    )
    ...
    for contrast in contrasts:
        z_map_f = model_face.compute_contrast(
            contrasts[contrast], output_type="z_score"
        )
        nib.save(
            z_map_f,
            os.path.join(out_folder, f"face_{contrast}_{brain_flag}_zmap.nii.gz"),
        )
        ...
        nib.save(
            t_map_f,
            os.path.join(out_folder, f"face_{contrast}_{brain_flag}_tmap.nii.gz"),
        )
        file_data_f = model_face.generate_report(contrasts[contrast], bg_img=mean_img_)
        file_data_f.save_as_html(
            os.path.join(out_folder, f"face_{contrast}_{brain_flag}_report.html")
        )

        z_map_s = model_scene.compute_contrast(
            contrasts[contrast], output_type="z_score"
        )
        nib.save(
            z_map_s,
            os.path.join(out_folder, f"scene_{contrast}_{brain_flag}_zmap.nii.gz"),
        )
        ...
        nib.save(
            t_map_s,
            os.path.join(out_folder, f"scene_{contrast}_{brain_flag}_tmap.nii.gz"),
        )
        file_data_s = model_scene.generate_report(contrasts[contrast], bg_img=mean_img_)
        file_data_s.save_as_html(
            os.path.join(out_folder, f"scene_{contrast}_{brain_flag}_report.html")
        )
...
for contrast in contrasts:
    ...
    maps = [
        nib.load(
            os.path.join(
                container_path,
                sub,
                "preremoval_lvl1_%s" % brain_flag,
                f"{contrast}_stimuli_{brain_flag}_zmap.nii.gz",
            )
        )
        for sub in subs
    ]
    ...
    nib.save(
        thresholded_map,
        os.path.join(out_dir, f"group+{contrast}_{brain_flag}_thresholded_zmap.nii.gz"),
    )
