...
def load_existing_data(
    subID, task, space, mask_ROIS, load=False
...
def fit_model(X, Y, groups, save=False, out_fname=None, v=False):
    ...
    for train_inds, test_inds in logo.split(X, Y, groups):
        ...
        trained_models.append(
            lr
        )  # this alllows me to extract the trained model from this function, so I can then use this trained classifier to decode another phase
        fprs.append(
            fpr
        )  # this allows me to to extract the feature selection, since if I want to carry this trained classifier into another phase, I will need to feature select that data in the same way
...
def decode(training_runs, train_data, train_labels, test_data, test_labels):
    ...
    predictions = lr.predict(
        X_test_sub
    )  # get the predicted categories from running the model
    evidence = 1.0 / (
        1.0 + np.exp(-lr.decision_function(X_test_sub))
    )  # MAIN: collect the evidence values of all timepoints (3 columns: 0-rest, 1-faces, 2-scenes)
    true = y_test
...
def classification(subID):
    ...
    scores = np.mean(
        scores, axis=1
    )  # get the mean of each iteration (since theyre in pairs for the train/test pair)
    auc_scores = np.mean(
        auc_scores, axis=1
    )  # get the auc mean of each iteration (since theyre in pairs for the train/test pair)
    cms = np.stack(cms)
    ...
    best_model = np.where(auc_scores == max(auc_scores))[0][
        0
    ]  # finding the max via auc_score
    ...
    )  # this will decode the data using the already trained model and feature selection method

    # need to then save the evidence:
    evidence_df = pd.DataFrame(data=label_df2)  # take the study DF for this subject
    evidence_df.drop(
        columns=evidence_df.columns[0], axis=1, inplace=True
...
def organize_memory_evidence(subID, save=True):
    ...
    if save:
        sub_dir = os.path.join(data_dir, f"sub-{subID}")
        out_fname_template = (
            f"sub-{subID}_{space}_{task2}_remember_evidence_dataframe.csv"
        )
        print(
        ...
        out_fname_template2 = (
            f"sub-{subID}_{space}_{task2}_forgot_evidence_dataframe.csv"
        )
        print(
...
def visualize_evidence():
    ...
    ax = sns.lineplot(
        data=group_evidence_df.loc[
            ...
            & (group_evidence_df["evidence_class"] == "scenes")
        ],
        x="TR",
        y="evidence",
        color="green",
    ...
    ax.set(
        xlabel="TR",
        ylabel="Category Classifier Evidence",
        title="T1w - Category Classifier (group average)",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir, "figs", "group_level_category_decoding_during_removal_v2.svg"
        )
    )
    plt.savefig(
        os.path.join(
            data_dir, "figs", "group_level_category_decoding_during_removal_v2.png"
        )
    )
    plt.clf()

    # want to get the trajectory for removal of an item in WM (classifier evidence of removal - maintain):
    maintain_df = group_evidence_df.loc[
        (group_evidence_df["condition"] == "maintain")
        & (group_evidence_df["evidence_class"] == "scenes")
    ]
    replace_df = group_evidence_df.loc[
        (group_evidence_df["condition"] == "replace")
        & (group_evidence_df["evidence_class"] == "scenes")
    ]
    suppress_df = group_evidence_df.loc[
        (group_evidence_df["condition"] == "suppress")
        & (group_evidence_df["evidence_class"] == "scenes")
    ]

    replace_df["evidence"] = (
        replace_df["evidence"].values - maintain_df["evidence"].values
    )
    suppress_df["evidence"] = (
        suppress_df["evidence"].values - maintain_df["evidence"].values
    )

    from scipy.stats import ttest_1samp

    # loop over each condition separately
    for condition in ["replace", "suppress"]:
        if condition == "replace":
            # loop over each TR separately
            for tr in range(0, 14):
                # filter data to only include the current TR
                tr_df = replace_df[replace_df["TR"] == tr]

                # perform one-sample t-test against 0 on all subjects
                t_statistic, p_value = ttest_1samp(
                    tr_df["evidence"], 0, nan_policy="omit"
                )

                # print the result
                print(
                    f"Condition: {condition}, TR: {tr}, T-statistic: {t_statistic}, p-value: {p_value}"
                )
            ...
            for tr in range(0, 14):
                # filter data to only include the current TR
                tr_df = suppress_df[suppress_df["TR"] == tr]
    ...
    for condition in ["replace", "suppress"]:
        if condition == "replace":
            # loop over each non-overlapping TR pair separately
            for tr in range(0, 13, 2):
                # filter data to only include the current TR pair
                tr_pair_df = replace_df[
                    (replace_df["TR"] == tr) | (replace_df["TR"] == tr + 1)
                ]

                # perform one-sample t-test against 0 on all subjects
                t_statistic, p_value = ttest_1samp(
                    tr_pair_df["evidence"], 0, nan_policy="omit"
                )

                # print the result
                print(
                    f"Condition: {condition}, TR pair: ({tr}, {tr+1}), T-statistic: {t_statistic}, p-value: {p_value}"
                )
            ...
            for tr in range(0, 13, 2):
                # filter data to only include the current TR pair
                tr_pair_df = suppress_df[
                    (suppress_df["TR"] == tr) | (suppress_df["TR"] == tr + 1)
                ]
        ...
        ax.set_title(
            "T1w - Category Classifier - Trajectory for removal of an item from WM",
            loc="center",
            wrap=True,
        )
        plt.tight_layout()
        plt.savefig(
            os.path.join(
                data_dir,
                "figs",
                "group_level_category_decoding_removal_minus_maintain_v2.svg",
            )
        )
        plt.savefig(
            os.path.join(
                data_dir,
                "figs",
                "group_level_category_decoding_removal_minus_maintain_v2.png",
            )
        )
        plt.clf()

        # adding in the smooth curve, which will be done via scipy.interpolate
        n_points = 100  # how many data points for the curve?
        x_new = np.linspace(
            group_evidence_df["TR"].min(), group_evidence_df["TR"].max(), n_points
        )
    ...
    for condition in group_evidence_df["condition"].unique():
        if condition == "maintain":
            for sub in subIDs:
                ...
                temp_df["TR"] = x_new
                temp_df["evidence"] = gaussian_filter(y_smooth, sigma=5, mode="wrap")
    ...
    ax = sns.lineplot(
        data=new_df.loc[(new_df["condition"] == "maintain")],
        x="TR",
        y="evidence",
        color="green",
        label="Maintain",
    ...
    ax = sns.lineplot(
        data=new_df.loc[(new_df["condition"] == "replace")],
        x="TR",
        y="evidence",
        color="blue",
        label="Replace",
    ...
    ax = sns.lineplot(
        data=new_df.loc[(new_df["condition"] == "suppress")],
        x="TR",
        y="evidence",
        color="red",
        label="Suppress",
    ...
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            "group_level_category_decoding_during_removal_v2_smoothed.svg",
        )
    )
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            "group_level_category_decoding_during_removal_v2_smoothed.png",
        )
    )
    plt.clf()

    new_diff_replace = new_df.loc[(new_df["condition"] == "replace")].copy()
    new_diff_replace["evidence"] = (
        new_df.loc[(new_df["condition"] == "replace")]["evidence"]
        - new_df.loc[(new_df["condition"] == "maintain")]["evidence"]
    )

    new_diff_suppress = new_df.loc[(new_df["condition"] == "suppress")].copy()
    new_diff_suppress["evidence"] = (
        new_df.loc[(new_df["condition"] == "suppress")]["evidence"]
        - new_df.loc[(new_df["condition"] == "maintain")]["evidence"]
    )

    ax = sns.lineplot(
        data=new_diff_replace,
        x="TR",
        y="evidence",
        color="blue",
        label="replace",
    ...
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            "group_level_category_decoding_removal_minus_maintain_v2_smoothed.svg",
        )
    )
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            "group_level_category_decoding_removal_minus_maintain_v2_smoothed.png",
        )
    )
    plt.clf()
...
def check_anova_stats():
    ...
    for i in scene_evis.TR.unique():
        ...
        ANOVA_results[f"TR {i}"] = AnovaRM(
            data=temp_df, depvar="evidence", subject="sub", within=["condition"]
        ).fit()
        print(f"TR {i}: ")
...
def classification_post(subID):
    ...
    # get labels
    label_df2 = get_shifted_labels(
        task2, 5, rest_tag
    )  # switched it to 4 since it seemed 1 TR off when looking at the evidence values
    print(f"Category label shape: {label_df2.shape}")
    ...
    )  # have to reset the index since its wrong after the shift
    maintain_ind = np.where(
        (evidence_df["old_novel"] == 1)
        & (evidence_df["condition"] == 1)
        & (evidence_df["stim_present"] == 1)
    )
    maintain_df = evidence_df.loc[maintain_ind]
    maintain_imgs = np.unique(maintain_df["image_id"].values)
    maintain_post_evi = {}
    for img in maintain_imgs:
        maintain_post_evi[img] = maintain_df.loc[
            maintain_df["image_id"] == img
        ].scene_evi.mean()  # this takes the average of the scene evidence during the 2 TR of viewing

    replace_ind = np.where(
        (evidence_df["old_novel"] == 1)
        & (evidence_df["condition"] == 2)
        & (evidence_df["stim_present"] == 1)
    )
    ...
    for img in replace_imgs:
        replace_post_evi[img] = replace_df.loc[
            replace_df["image_id"] == img
        ].scene_evi.mean()

    suppress_ind = np.where(
        (evidence_df["old_novel"] == 1)
        & (evidence_df["condition"] == 3)
        & (evidence_df["stim_present"] == 1)
    )
    ...
    for img in suppress_imgs:
        suppress_post_evi[img] = suppress_df.loc[
            suppress_df["image_id"] == img
        ].scene_evi.mean()

    preexp_ind = np.where(
        (evidence_df["old_novel"] == 3) & (evidence_df["stim_present"] == 1)
    )
    preexp_df = evidence_df.loc[preexp_ind]
    preexp_imgs = np.unique(preexp_df["image_id"].values)
    preexp_post_evi = {}
    for img in preexp_imgs:
        preexp_post_evi[img] = preexp_df.loc[
            preexp_df["image_id"] == img
        ].scene_evi.mean()
    ...
    post_removal_df = pd.concat(
        [temp_maintain, temp_replace, temp_suppress, temp_preexp]
    )

    sub_dir = os.path.join(data_dir, f"sub-{subID}")
    out_fname_template = (
        f"sub-{subID}_{space}_trained-{task}_tested-{task2}_evidence_sorted.csv"
    )
    print("\n *** Saving evidence values with subject dataframe ***")
    ...
    subject_avg = {
        "maintain": maintain_avg,
        "replace": replace_avg,
        "suppress": suppress_avg,
        "pre-exp": preexp_avg,
    }
...
def group_post():
    ...
    melt_group_df.rename(
        columns={"variable": "operation", "value": "evidence"}, inplace=True
    )
    ax = sns.barplot(
        data=melt_group_df,
        x="operation",
        y="evidence",
        palette=["green", "blue", "red", "gray"],
        ci=68,
    )
    ax.set(
        xlabel="Operation",
        ylabel="Category Classifier Evidence",
        title="T1w - Category Classifier (group average) - Post Removal",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir, "figs", "group_level_category_decoding_during_postremoval.png"
        )
    )
    plt.clf()
...
def nminus1(subID, save=True):
    ...
    for x in range(len(sub_condition_list)):
        ...
        if (
            ...
            if temp_memory == 1:
                maintain_remember_trials[temp_image] = sub_df[
                    ["rest_evi", "scene_evi", "face_evi"]
                ][sub_index[x] - 1 : sub_index[x] + 1].values
            elif temp_memory == 0:
                maintain_forgot_trials[temp_image] = sub_df[
                    ["rest_evi", "scene_evi", "face_evi"]
                ][sub_index[x] - 1 : sub_index[x] + 1].values
    ...
    avg_remember_maintain_diff = (
        avg_remember_maintain[1] - avg_remember_maintain[2]
    )  # want to also look at difference between Scenes and Faces to evaluate fidelity
    ...
    avg_forgot_maintain_diff = (
        avg_forgot_maintain[1] - avg_forgot_maintain[2]
    )  # want to also look at difference between Scenes and Faces to evaluate fidelity
    ...
    avg_maintain_diff["sub"] = np.repeat(
        subID, len(avg_maintain_diff)
    )  # input the subject so I can stack melted dfs
    avg_maintain_diff["evidence_class"] = "diff"
    avg_maintain_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_maintain_diff[
        "condition"
    ...
    avg_replace_diff = (
        avg_replace_diff.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_replace_diff["sub"] = np.repeat(
        subID, len(avg_replace_diff)
    )  # input the subject so I can stack melted dfs
    avg_replace_diff["evidence_class"] = "diff"
    avg_replace_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_replace_diff[
        "condition"
    ...
    avg_suppress_diff = (
        avg_suppress_diff.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_suppress_diff["sub"] = np.repeat(
        subID, len(avg_suppress_diff)
    )  # input the subject so I can stack melted dfs
    avg_suppress_diff["evidence_class"] = "diff"
    avg_suppress_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_suppress_diff[
        "condition"
    ...
    avg_subject_diff_df = pd.concat(
        [avg_maintain_diff, avg_replace_diff, avg_suppress_diff],
        ignore_index=True,
        sort=False,
    )
    ...
    avg_remember_maintain_diff["sub"] = np.repeat(
        subID, len(avg_remember_maintain_diff)
    )  # input the subject so I can stack melted dfs
    avg_remember_maintain_diff["evidence_class"] = "diff"
    avg_remember_maintain_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_remember_maintain_diff[
        "condition"
    ...
    avg_remember_replace_diff = (
        avg_remember_replace_diff.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_remember_replace_diff["sub"] = np.repeat(
        subID, len(avg_remember_replace_diff)
    )  # input the subject so I can stack melted dfs
    avg_remember_replace_diff["evidence_class"] = "diff"
    avg_remember_replace_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_remember_replace_diff[
        "condition"
    ...
    avg_remember_suppress_diff = (
        avg_remember_suppress_diff.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_remember_suppress_diff["sub"] = np.repeat(
        subID, len(avg_remember_suppress_diff)
    )  # input the subject so I can stack melted dfs
    avg_remember_suppress_diff["evidence_class"] = "diff"
    avg_remember_suppress_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_remember_suppress_diff[
        "condition"
    ...
    avg_subject_remember_diff_df = pd.concat(
        [
            avg_remember_maintain_diff,
            avg_remember_replace_diff,
            avg_remember_suppress_diff,
    ...
    avg_forgot_maintain_diff["sub"] = np.repeat(
        subID, len(avg_forgot_maintain_diff)
    )  # input the subject so I can stack melted dfs
    avg_forgot_maintain_diff["evidence_class"] = "diff"
    avg_forgot_maintain_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_forgot_maintain_diff[
        "condition"
    ...
    avg_forgot_replace_diff = (
        avg_forgot_replace_diff.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_forgot_replace_diff["sub"] = np.repeat(
        subID, len(avg_forgot_replace_diff)
    )  # input the subject so I can stack melted dfs
    avg_forgot_replace_diff["evidence_class"] = "diff"
    avg_forgot_replace_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_forgot_replace_diff[
        "condition"
    ...
    avg_forgot_suppress_diff = (
        avg_forgot_suppress_diff.T.melt()
    )  # now you get 2 columns: variable (TR) and value (evidence)
    avg_forgot_suppress_diff["sub"] = np.repeat(
        subID, len(avg_forgot_suppress_diff)
    )  # input the subject so I can stack melted dfs
    avg_forgot_suppress_diff["evidence_class"] = "diff"
    avg_forgot_suppress_diff.rename(
        columns={"variable": "TR", "value": "evidence"}, inplace=True
    )  # renamed the melted column names
    avg_forgot_suppress_diff[
        "condition"
    ...
    avg_subject_forgot_diff_df = pd.concat(
        [avg_forgot_maintain_diff, avg_forgot_replace_diff, avg_forgot_suppress_diff],
        ignore_index=True,
        sort=False,
    )
    ...
    if save:
        sub_dir = os.path.join(data_dir, f"sub-{subID}")
        out_fname_template = (
            f"sub-{subID}_{space}_{task2}_nminus1_evidence_dataframe.csv"
        )
        print(
            f"\n Saving the sorted evidence dataframe for {subID} - phase: {task2} - as {out_fname_template}"
        )
        ...
        out_fname_template_diff = (
            f"sub-{subID}_{space}_{task2}_nminus1_evidence_difference_dataframe.csv"
        )
        print(
        ...
        avg_subject_remember_diff_df.to_csv(
            os.path.join(sub_dir, out_fname_template_diff)
        )
        ...
        avg_subject_forgot_diff_df.to_csv(
            os.path.join(sub_dir, out_fname_template_diff)
        )

        sub_dir = os.path.join(data_dir, f"sub-{subID}")
        out_fname_template = (
            f"sub-{subID}_{space}_{task2}_nminus1_remember_evidence_dataframe.csv"
        )
        print(
            f"\n Saving the sorted remebered evidence dataframe for {subID} - phase: {task2} - as {out_fname_template}"
        )
        ...
        out_fname_template2 = (
            f"sub-{subID}_{space}_{task2}_nminus1_forgot_evidence_dataframe.csv"
        )
        print(
            f"\n Saving the sorted forgot evidence dataframe for {subID} - phase: {task2} - as {out_fname_template}"
        )
    ...
    return (
        avg_subject_df,
        avg_subject_diff_df,
        avg_remember_subject_df,
        avg_forgot_subject_df,
        avg_subject_remember_diff_df,
        avg_subject_forgot_diff_df,
    )
...
def visualize_nminus1_evidence():
    ...
    for subID in subIDs:
        (
            temp_subject_df,
            temp_subject_diff_df,
            temp_subject_remember,
            temp_subject_forgot,
            group_remember_evidence_diff,
            group_forgot_evidence_diff,
        ) = nminus1(subID)

        group_evidence_df = pd.concat(
            [group_evidence_df, temp_subject_df], ignore_index=True, sort=False
        )
        group_evidence_diff_df = pd.concat(
            [group_evidence_diff_df, temp_subject_diff_df],
            ignore_index=True,
            sort=False,
        )
        group_remember_evidence_df = pd.concat(
            [group_remember_evidence_df, temp_subject_remember],
            ignore_index=True,
            sort=False,
        )
        group_forgot_evidence_df = pd.concat(
            [group_forgot_evidence_df, temp_subject_forgot],
            ignore_index=True,
            sort=False,
        )
        group_remember_evidence_diff_df = pd.concat(
            [group_remember_evidence_diff_df, group_remember_evidence_diff],
            ignore_index=True,
            sort=False,
        )
        group_forgot_evidence_diff_df = pd.concat(
            [group_forgot_evidence_diff_df, group_forgot_evidence_diff],
            ignore_index=True,
            sort=False,
        )
    ...
    temp_diff_df = pd.concat(
        [group_remember_evidence_diff_df, group_forgot_evidence_diff_df]
    )

    ax = sns.barplot(
        data=group_evidence_df.loc[(group_evidence_df["evidence_class"] == "scenes")],
        x="TR",
        y="evidence",
        hue="condition",
        ci=68,
    ...
    ax.set(
        xlabel="TR",
        ylabel="Category Classifier Evidence",
        title="T1w - Average Category Classifier evidence of N-1 (group average)",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(data_dir, "figs", "group_level_category_decoding_nminus1_TR.png")
    )
    plt.clf()

    ax = sns.barplot(
        data=group_evidence_diff_df.loc[
            (group_evidence_diff_df["evidence_class"] == "diff")
        ],
        x="TR",
        y="evidence",
        hue="condition",
    ...
    ax.set_title(
        "T1w - Average Category Classifier evidence of N-1 - Scene minus Face evidence (group average)",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir, "figs", "group_level_category_decoding_nminus1_diff_TR.png"
        )
    )
    plt.clf()
    ...
    ax.set(
        xlabel="Operation of N-1",
        ylabel="Category Classifier Evidence",
        title="T1w - Average Category Classifier evidence of N-1 (group average)",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(data_dir, "figs", "group_level_category_decoding_nminus1_avg.png")
    )
    plt.clf()
    ...
    ax.set(
        xlabel="Operation of N-1",
        ylabel="Category Classifier Evidence (Scenes - Faces)",
    )
    ax.set_title(
        "T1w - Average Category Classifier evidence of N-1 - Scene minus Face evidence (group average)",
        loc="center",
    ...
    plt.savefig(
        os.path.join(
            data_dir, "figs", "group_level_category_decoding_nminus1_diff_avg.png"
        )
    )
    plt.clf()

    ax = sns.barplot(
        data=temp_df.loc[(temp_df["evidence_class"] == "scenes")],
        x="condition",
        y="evidence",
        hue="memory",
        ci=68,
    ...
    ax.set_title(
        "T1w - Average Category Classifier evidence of N-1 - sorted by outcome (group average)",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir, "figs", "group_level_category_decoding_nminus1_memory_avg.png"
        )
    )
    plt.clf()

    ax = sns.barplot(
        data=temp_diff_df.loc[(temp_diff_df["evidence_class"] == "diff")],
        x="condition",
        y="evidence",
        hue="memory",
        ci=68,
    ...
    ax.set_title(
        "T1w - Average Category Classifier evidence of N-1 - Scene minus Face evidence - sorted by outcome (group average)",
        loc="center",
        wrap=True,
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir,
            "figs",
            "group_level_category_decoding_nminus1_diff_memory_avg.png",
        )
    )
    plt.clf()
