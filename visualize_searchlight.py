...
for subID in subIDs:
    ...
    for operation in operations:
        img = nib.load(
            os.path.join(
                searchlight_dir, "Sub-%s_SL_%s_result.nii.gz" % (subID, operation)
            )
        )
        plotting.plot_glass_brain(
            ...
            plot_abs=False,
            output_file=(
                os.path.join(searchlight_dir, "%s_glass_brain.pdf" % operation)
            ),
        )
        ...
        plotting.plot_glass_brain(
            ...
            plot_abs=False,
            output_file=(
                os.path.join(searchlight_dir, "%s_glass_brain_full.pdf" % operation)
            ),
        )
