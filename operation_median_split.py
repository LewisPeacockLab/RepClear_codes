...
def organize_evidence(subID, space, task, save=True):
    ...
    for i in sub_condition_list:
        if (
            ...
            memory_outcome[temp_image] = sub_df[sub_df["image_id"] == temp_image][
                "memory"
            ].values[0]
            counter += 1
    ...
    all_maintain = pd.DataFrame(
        data=maintain_trials_mean.values(), index=maintain_trials_mean.keys()
    )
    all_replace = pd.DataFrame(
        data=replace_trials_mean.values(), index=replace_trials_mean.keys()
    )
    ...
    df_high, df_low = [
        x
        for _, x in all_suppress.groupby(
            all_suppress["evidence"] < all_suppress.evidence.median()
        )
    ]
    ...
    if save:
        sub_dir = os.path.join(data_dir, f"sub-{subID}")
        out_fname_template = (
            f"sub-{subID}_{space}_{task}_mediansplit_evidence_dataframe.csv"
        )
        print(
...
def group_post():
    ...
    ax.set(
        xlabel="Suppress Median Split",
        ylabel="Mean Memory outcome",
        title="Suppress Evidence Median-Split: Prediction of Memory Outcome",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            data_dir, "figs", f"{space}_group_level_suppress_median_split_memory.png"
        )
    )
    plt.clf()
...
high_memory_evi = group_evidence_df[group_evidence_df["split"] == "high"][
    "memory"
].values  # high suppress evidence - should be low memory
low_memory_evi = group_evidence_df[group_evidence_df["split"] == "low"][
    "memory"
].values  # low suppress evidence - should be high memory
